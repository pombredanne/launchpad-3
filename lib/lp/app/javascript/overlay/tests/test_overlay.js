...
YUI.add('lp.overlay.test', function (Y) {
    ...
    tests.suite.add(new Y.Test.Case({
        ...
        test_getTabNodes_types: function() {
            // tabable nodes include <a>, <button>, <input>, <select>.
        ...
        test_getTabNodes_visibility: function() {
            // tabable nodes that are hidden are excluded.
            // Remember that the 0 button is the close button in the header.
            this.overlay = new Y.lazr.PrettyOverlay({
                headerContent: 'Fnord',
                bodyContent: [
        ...
        test_handleTab_last_to_first: function() {
            // Tabing from the last navigatable node moves focus to the first.
            // Remember that the 0 button is the close button in the header.
            this.overlay = new Y.lazr.PrettyOverlay({
                headerContent: 'Fnord',
            ...
            var focused = false;
            nodes.item(0).focus = function() {focused = true;};
            this.overlay._handleTab(e);
        ...
        test_handleTab_first_to_last: function() {
            ...
            var focused = false;
            nodes.item(2).focus = function() {focused = true;};
            this.overlay._handleTab(e);
            Y.Assert.isTrue(halted);
            Y.Assert.isTrue(focused);
        },

        test_handleTab_within_tab_rnge: function() {
            // Tab from one element to another that is not beyond the
            // for or last element does nothing special.
            // Remember that the 0 button is the close button in the header.
            this.overlay = new Y.lazr.PrettyOverlay({
                headerContent: 'Fnord',
                bodyContent: '<div><a href="#">1</a> <a href="#">2</a></div>'
