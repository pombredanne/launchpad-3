    ...
    */
    ...
    Y.extend(PrettyOverlay, Y.Overlay, {
        ...
        */
        ...
        /**
         * Bind an event handler to ensure the Tab key cycles through
         ...
         * @method _bindTabKey
         */
        _bindTabKey: function(e) {
            var TAB = 9;
            var bounding_box = this.get(BOUNDING_BOX);
            var action_selector = 'a, button, input, select, textarea';
            Y.delegate("keydown", function(e) {
                if (e.keyCode === TAB) {
                    var links = bounding_box.all(action_selector);
                    var all_links = new Y.NodeList([]);
                    links.each(function(item, index, node_list) {
                        if (item.getX() !== 0) {
                            // The browser rendered the link.
                            all_links.push(item);
                        }
                    });
                    var max_index = all_links.size() - 1;
                    var next = (e.shiftKey) ? -1 : 1;
                    var next_index = all_links.indexOf(e.currentTarget) + next;
                    if (next_index > max_index) {
                        all_links.item(0).focus();
                        e.halt();
                    } else if (next_index < 0) {
                        all_links.item(max_index).focus();
                        e.halt();
                    }
                }
            }, bounding_box, action_selector);
        },
