...
var ESCAPE = 27,
    ...
    BINDUI = "bindUI";
    TABABLE_SELECTOR = 'a, button, input, select, textarea';
        ...
        */
         ...
         */
         _getTabNodes: function() {
            var bounding_box = this.get(BOUNDING_BOX);
            var all_tab_nodes = bounding_box.all(TABABLE_SELECTOR);
            var tab_nodes = new Y.NodeList([]);
            all_tab_nodes.each(function(item, index, node_list) {
                if (item.getY() !== 0) {
                    // The node has a position so a user can tab to it.
                    // Note that textarea's can have 0 X but have height.
                    tab_nodes.push(item);
                }
            });
            return tab_nodes;
