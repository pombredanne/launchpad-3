...
function Expander (icon_node, content_node, config) {
    ...
    this.config = config;
    this.loaded = !Y.Lang.isValue(config.loader);
...
namespace.Expander.prototype = {
    ...
    setExpanded: function (is_expanded) {
        if (is_expanded) {
            this.content_node.addClass(this.css_classes.expanded);
        } else {
            this.content_node.removeClass(this.css_classes.expanded);
        }
    },
    ...
    foldContentNode: function (expand) {
        if (expand) {
            this.content_node.removeClass(this.css_classes.unseen);
        } else {
            this.content_node.addClass(this.css_classes.unseen);
        }
    },
    ...
    addSpinner: function () {
        ...
        this.content_node.prepend(spinner_node);
    },

    /*
     * Show spinner and invoke the loader.  Arrange for the loader to invoke
     * receiveContent when done.
     */
    load: function () {
        this.addSpinner();
        this.setLoaded();
    ...
    /*
     * Toggle the visibility of the content node and the visual of the
     * expander icon.
     */
    toggle: function () {
        var expand = !this.isExpanded();
        this.foldContentNode(expand);
        this.setIcon(expand);
        if (expand && !this.isLoaded()) {
            this.load();
        }
    ...
    /*
     * Turn the icon "connection point" into an expander icon.
     */
    renderIcon: function (is_expanded) {
        this.setIcon(is_expanded);
        this.icon_node.removeClass(this.css_classes.unseen);
    },
    ...
    setUp: function () {
        ...
        var is_expanded = this.isExpanded();
        this.foldContentNode(is_expanded);
        this.renderIcon(is_expanded);
 ...
 *     each expander that is set up.  Must take an Expander as its argument,
 *     create a Node or NodeList with the output to be displayed, and feed
 *     the output to the expander's "receive" method.
 */
function createByCSS(widget_select, icon_select, content_select, loader) {
    ...
    Y.all(widget_select).each(function(widget) {
        var expander = new Expander(
            widget.one(icon_select),
            widget.one(content_select),
            config);
        expander.setUp();
    });
}
