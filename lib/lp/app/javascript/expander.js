...
namespace.STATE_CHANGED = STATE_CHANGED;
...
Y.extend(ExpanderRadioController, Y.Base, {
    ...
    _stateChangeListener: function(active_expander) {
        var controller = this;
        return function(e) {
            var group_id = e.details[0];
            var new_state = e.details[1];
            controller.expanders[group_id].forEach(function(expander) {
    ...
    registerExpander: function(group_id, expander) {
        if (!this.expanders.hasOwnProperty(group_id)) {
            this.expanders[group_id] = [];
        }
        this.expanders[group_id].push(expander);
        var controller = this;
        expander.on(STATE_CHANGED, this._stateChangeListener(expander));
    },

    deregisterExpander: function(group_id, expander) {
        if (this.expanders.hasOwnProperty(group_id)) {
        ...
        expander.detach(STATE_CHANGED, this._stateChangeListener);
    }
});

var expanderRadioController = new ExpanderRadioController();
namespace.expanderRadioController = expanderRadioController;
...
function Expander(icon_node, content_node, config) {
    ...
    if (Y.Lang.isValue(this.config.group_id)) {
        expanderRadioController.registerExpander(
            this.config.group_id, this);
    }
...
Y.extend(Expander, Y.Base, {
    ...
    destructor: function() {
        if (Y.Lang.isValue(this.config.group_id)) {
            expanderRadioController.deregisterExpander(
                this.config.group_id, this);
        }
    },
    ...
    setExpanded: function(is_expanded) {
        ...
        if (state_changed && Y.Lang.isValue(this.config.group_id)) {
            this.fire(
                ...
                is_expanded?EXPANDED:COLLAPSED);
        }
    },
