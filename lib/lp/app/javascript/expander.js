 ...
 *
 * @param content_node The Node holding the content of the expander.
 * @param expand Are we expanding?  If not, we must be collapsing.
 */
function foldContentNode(content_node, expand) {
    if (expand) {
 ...
 */
function setIcon(icon_node, expand) {
    icon_node.addClass('sprite');
    icon_node.removeClass(nameSprite(!expand)).addClass(nameSprite(expand));
}
...
function toggleExpander(icon_node, content_node, loader) {
    var expand = !isExpanded(content_node);
    foldContentNode(content_node, expand);
    setIcon(icon_node, expand);
    if (expand) {
        if (Y.Lang.isValue(loader) && !isLoaded(content_node)) {
            load(icon_node, content_node, loader);
        }
    }
    setExpanded(content_node, expand);
}

/*
 * Turn an icon "connection point" into an expander.
 */
function enhanceExpanderIcon(icon_node, content_node, is_expanded, loader) {
    icon_node.on('click', function(e) {
        e.preventDefault();
        toggleExpander(icon_node, content_node, loader);
    });
    setIcon(icon_node, is_expanded);
...
function createExpander(icon_node, content_node, loader) {
    ...
    foldContentNode(content_node, is_expanded);
    enhanceExpanderIcon(icon_node, content_node, is_expanded, loader);
}
