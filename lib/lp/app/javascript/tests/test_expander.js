...
YUI().use('lp.testing.runner', 'base', 'test', 'console', 'node',
    ...
    var ExpanderTestMixin = {
        ...
        tearDown: function() {
            if (this.expander !== undefined) {
                ...
                //this.expander.detach(module.STATE_CHANGED);
            }
        },
    ...
    suite.add(new Y.Test.Case(
        ...
        test_no_expand_event_unless_group_defined: function() {
            ...
            var root = this.makeExpanderHooks({expanded: true});
            var expander = this.makeExpander(root);
            expander.on(module.STATE_CHANGED, function(e) {
                event_fired = true;
            });
            expander.icon_node.simulate('click');
        ...
        test_expand_event_fired: function() {
            ...
            expander.on(
                module.STATE_CHANGED, function(e) {
                    Y.Assert.areEqual('group id', e.details[0]);
                    Y.Assert.areEqual(module.EXPANDED, e.details[1]);
                    event_fired = true;
                });
            expander.icon_node.simulate('click');
        ...
        test_collapse_event_fired: function() {
            ...
            expander.on(
                module.STATE_CHANGED, function(e) {
                    Y.Assert.areEqual('group id', e.details[0]);
                    Y.Assert.areEqual(module.COLLAPSED, e.details[1]);
                    event_fired = true;
                });
            expander.icon_node.simulate('click');
            Y.Assert.isTrue(event_fired);
