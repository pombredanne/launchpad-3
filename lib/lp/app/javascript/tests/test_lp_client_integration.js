...
suite.add(new Y.Test.Case({
  ...
  test_get_html_representation_escaped: function() {
    var data = serverfixture.setup(this, 'create_user_with_html_display_name');
  ...
  // test_hosted_file: function() {
    // If we want to support Chrome the hosted file support in
    // lp_client is fairly useless for us.  That's because we redirect
    // to the librarian, which is cross-domain, which the browser will
    // not accept.
    // var data = serverfixture.setup(this, 'create_bug_and_login');
    // var client = new Y.lp.client.Launchpad({sync: true});
    // var config = makeTestConfig({parameters: {
    //   comment: 'A new attachment',
    //   content_type: 'text/plain',
    //   data: 'Some data.',
    //   filename: 'foo.txt'}});
    // client.named_post(data.bug.self_link, 'addAttachment', config);
    // Y.Assert.isTrue(config.successful);
    //
    // This is where the redirection to the librarian happens (in follow_link).
    //
    // var attachment = config.result;
    // config = makeTestConfig();
    // attachment.follow_link('data', config);
    // Y.Assert.isTrue(config.successful);
    // var hosted_file = config.result;
    // Y.Assert.areEqual('foo.txt', hosted_file.filename);
    // Y.Assert.areEqual('Some data.', hosted_file.contents);
    // Y.Assert.areEqual('text/plain', hosted_file.content_type);
    //
    // We cannot PUT or DELETE this file, and we have no file that
    // supports it AIUI.  Otherwise these would belong in separate
    // tests.
    //
    // hosted_file.filename = 'bar.json';
    // hosted_file.contents = "['I am JSON, hear me roar']";
    // hosted_file.content_type = 'application/json';
    // config = makeTestConfig();
    // hosted_file.lp_save(config);
    // Y.Assert.isTrue(config.successful);
    // config = makeTestConfig();
    // hosted_file.lp_delete(config);
    // Y.Assert.isTrue(config.successful);
  // }
}));
