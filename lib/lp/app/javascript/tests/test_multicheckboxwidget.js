...
suite.add(new Y.Test.Case({
    ...
    test_reference_display: function() {
        // Test that values which are for references are handled correctly.
        this.createWidget();
        var items = Y.one('[id="multicheckboxtest.items"]');
        var mapping = {0: 'Item0', 1: 'Item1'};
        var selected = getSelectedItems(items, mapping, '');
        Y.Assert.areEqual('Item0', selected.pop());
    },
