...
var BugListingNavigator = Y.lp.bugs.buglisting.BugListingNavigator;
...
suite.add(new Y.Test.Case({
    ...
    get_render_navigator: function() {
        ...
        var template = "{{#bugtasks}}{{#show_foo}}{{foo}}{{/show_foo}}" +
            "{{/bugtasks}}";
        var navigator =  new BugListingNavigator({
            cache: lp_cache,
            template: template,
            target: this.target
        });
...
var get_navigator = function(url, config) {
    ...
    return new BugListingNavigator(navigator_config);
};
...
suite.add(new Y.Test.Case({
    ...
    test_get_pre_fetch_configs: function(){
        ...
        Y.each(configs, function(value){
            batch_keys.push(module.ListingNavigator.get_batch_key(value));
        });
    ...
    test_pre_fetch_batches: function(){
        ...
        Y.Assert.areSame(
            ...
            '/bar/+bugs/++model++?orderby=&memo=57&start=56');
    },
    ...
    test_pre_fetch_on_update_from_new_model: function(){
        ...
        Y.Assert.areSame(
            io_provider.last_request.url,
            '/bar/+bugs/++model++?orderby=baz&memo=pi&start=314');
    }
}));
...
suite.add(new Y.Test.Case({
    ...
    test_dealias_batches_next: function(){
        ...
        var prev_batch_key = module.ListingNavigator.get_batch_key(
            prev_batch_config);
    ...
    test_dealias_batches_prev: function(){
        ...
        var next_batch_key = module.ListingNavigator.get_batch_key(
            next_batch_config);
