...
suite.add(new Y.Test.Case({
    ...
    test_getHTMLOrDefault: function() {
        // Make sure that the getHTMLOrDefault method works as expected.
        var entry_repr = {
          'first': "Hello",
          'second': "World<boo/>",
          'self_link': Y.lp.client.get_absolute_uri("a_self_link"),
        ...
        var entry = new Y.lp.client.Entry(null, entry_repr, "a_self_link");
        Assert.areEqual("<p>Hello</p>", entry.getHTMLOrDefault('first'));
        // If there is no html representation, the raw cache value should be
        // properly escaped.
        Assert.areEqual("World&lt;boo&#x2F;&gt;", entry.getHTMLOrDefault('second'));
      },
    ...
    test_update_cache_raises_attribute_events: function() {
        ...
        Assert.areEqual('<p>World html</p>', first_event.new_value_html);
