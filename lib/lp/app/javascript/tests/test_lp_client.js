...
suite.add(new Y.Test.Case({
    ...
    test_load_cache: function(){
        var recorder = new IORecorder();
        Assert.isNull(recorder.url);
        var mylist = [];
        var config = {
        ...
        Y.lp.client.load_cache(context, '+myview', config);
        Assert.areEqual('/context/+myview/++model++', recorder.url);
        recorder.success('{}', {'Content-Type': 'application/json'});
        var expected = Y.lp.client.Entry(client, {}, null);
        for (k in mylist[0]){
            if (mylist[0].hasOwnProperty(k)){
                Assert.areEqual(mylist[0][k], expected[k], k);
            }
        }
    }
}));
...
IORecorder.prototype.success = function(value, headers){
    var response = new MockHttpResponse();
    response.setResponseHeader('Content-Type', headers['Content-Type']);
    response.responseText = value;
    this.config.on.success(null, response, this.config['arguments']);
};
