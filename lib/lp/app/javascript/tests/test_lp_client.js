...
suite.add(new Y.Test.Case({
    ...
    test_getHTMLOrDefault: function() {
        ...
        var entry = new Y.lp.client.Entry(null, entry_repr, "a_self_link");
        Assert.areEqual("Hello", entry.getHTMLOrDefault('first').get('innerHTML'));
        // If there is no html representation, the raw cache value should be
        // properly wrapped inside a node.
        Assert.areEqual('World', entry.getHTMLOrDefault('second').get('innerHTML'));
      },
