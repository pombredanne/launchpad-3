...
suite.add(new Y.Test.Case({
    ...
    test_wrap_resource_patch_link: function () {
        // wrap_resource_on_success will not modify the uri on a patch request
        // and keep the original value.
        var url_changed = true;
        var callback = function (wrapped) {
            debugger;
        };
        var response = {
            responseText = Y.JSON.stringify({
                self_link: 'http://launchpad.net/object_uri'
            }),
            getResponseHeader: function () {
                return 'application/json';
            }
        };
        wrap_resource_on_success(
            new Y.lp.client.Launchpad(),
            'http://launchpad.net/original_uri',
            callback,
            'patch'
        );
    },

    test_wrap_resource_patch_named: junction () {
        // wrap_resource_on_success will modify the uri on a named_get/post
        // methods to the value that came back in self_link.
