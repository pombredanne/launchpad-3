...
YUI.add('lp.app.inlinehelp.test', function (Y) {
    ...
    suite.add(new Y.Test.Case({
        ...
        test_click_gets_overlay: function () {
            ...
            Y.Assert.areEqual(
                'Help',
                Y.one('.yui3-widget-hd').get('text'),
                'Should have a header content set of "Help"'
            );
        },
        ...
        test_click_get_content: function () {
            ...
            Y.Assert.areEqual(
                ...
                'The body content should be what we set into the object'
            );

            Y.lp.app.inlinehelp._show_help = orig_show_help;
        },

        test_failed_loading_gets_default: function () {
            // if we can't load the ajax content we should get something that
            // says we failed nicely
            Y.lp.app.inlinehelp.init_help();
            Y.one('a[target="help"]').simulate('click');
            ...
            this.wait(function () {
                Y.Assert.areEqual(
                    'Failed',
                    Y.one('.yui3-widget-bd').get('text').substr(0, 6),
                    'The body should start with "Failed..."'
                );
            }, 100);
        }
    }));
