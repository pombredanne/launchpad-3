 ...
 * @param {Object} yesFn The function to call if the user answers yes.
 * @param {Object} noFn The function to call if the user answers no.
 */
namespace.yesno_save_confirmation = function(picker, content, yesFn, noFn) {
    var ask_user_node = Y.Node.create(
            "<div class='transparent validation-node'/>");
    ...
    yes_button.on('click', function(e) {
        e.halt();
        ask_user_node.get('parentNode').removeChild(ask_user_node);
        if (Y.Lang.isFunction(yesFn) ) {
            yesFn();
        }
        form_reset(picker);
    });
    ...
    // Insert the validation elements into the form and animate their
    // appearance.
    content_node.insert(ask_user_node, 'after');
    var message_fade_in = new Y.Anim({
        node: ask_user_node,
        to: {opacity: 1},
        duration: 0.3
    });
    var content_fade_out = new Y.Anim({
        node: content_node,
        to: {opacity: 0},
        duration: 0.3
    });
    content_fade_out.run();
    content_fade_out.subscribe('end', function() {
        var steps_node = picker.get('contentBox').one('.steps');
        if (steps_node != null)
            steps_node.hide();
        content_node.setStyles({opacity: 1, display: 'none'});
        message_fade_in.run();
    });
};
...
function form_reset(picker) {
    var content_node = picker.get('contentBox').one('.yui3-widget-bd');
    content_node.setStyles({opacity: 1, display: 'block'});
    var steps_node = picker.get('contentBox').one('.steps');
    if (steps_node != null)
        steps_node.show();
    var ask_user_node = picker.get('contentBox').one('.validation-node');
    if (ask_user_node != null)
        ask_user_node.get('parentNode').removeChild(ask_user_node);
}
...
namespace.create = function (vocabulary, config, activator) {
    ...
    picker.subscribe('save', function (e) {
        ...
        if (Y.Lang.isFunction(validate_callback)) {
            activator.renderProcessing();
            validate_callback(picker, picker_result, do_save, undefined, form_reset);
        } else {
            do_save();
        }
    });
