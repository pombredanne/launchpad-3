...
namespace.yesno_save_confirmation = function(
    ...
    var buttons = Y.Node.create('<div class="extra_form_buttons"/>');
    ...
    ask_user_node.setContent(content);
    ask_user_node.appendChild(buttons);
    validation_content_container.appendChild(ask_user_node);

    picker.get('contentBox').one('.yui3-widget-bd').insert(
            validation_content_container, 'before');
