...
Y.extend(InlineEditor, Y.Widget, {
    ...
    initializer: function(cfg) {
        ...
        /**
         * Store the cfg so we can pass things to composed elements like the
         * ResizingTextarea. In this way we can change settings of that plugin
         * wihle creating and dealing with this widget
         */
    ...
    bindUI: function() {
        ...
        if (!this.get(MULTILINE)) {
            // if this is not a multi-line then set a max size to current size
            cfg.max_height = this.get(INPUT_EL).getComputedStyle('height');
        ...
        this.get(INPUT_EL).plug(Y.lp.app.formwidgets.ResizingTextarea, cfg);
    ...
    setInput: function(value) {
        ...
        // Normally, an event is caught on the change of the input, but if we
        // programatically set the input, it won't always catch
...
Y.EditableText = EditableText;
           ...
           "requires": ["oop", "anim", "event", "node", "widget",
                        "lp.anim", "lazr.base", "lp.app.formwidgets.resizing_textarea"]});
