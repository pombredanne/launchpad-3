...
function SubscribersLoader(config) {
    ...
    if (Y.Lang.isString(config.subscribe_someone_else_link)) {
        if (!Y.Lang.isString(config.subscribe_someone_else_level)) {
            Y.error(
                "No config.subscribe_someone_else_level specified.");
        }
 ...
 */
SubscribersLoader.prototype._loadSubscribersFromList = function(
    details) {
        ...
        loader.lp_client.get(person_uri, {
            on: {
                success: function(person) {
                    loader._subscribeSomeoneElse(person, level);
                },
 ...
 * @param level {String} Level of the subscription.
 */
SubscribersLoader.prototype._subscribeSomeoneElse = function(person, level) {
