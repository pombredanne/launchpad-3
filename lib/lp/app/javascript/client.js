...
Entry.prototype.getHTMLOrDefault = function(key) {
    ...
    // escaped and wrapped inside a node. But we don't escape links.
    if (key.substr(-5) !== '_link' && typeof default_value == 'string') {
      var escaped_value = Y.Escape.html(default_value);
      default_value = Y.Node.create("<span/>");
      default_value.setContent(escaped_value);
    }
