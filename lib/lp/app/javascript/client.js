...
function escape_if_string(value)
{
  ...
  for (var name in cache) {
    ...
    if (name != 'lp_html') {
      if (old_value != new_value) {
        ...
        var new_value_html = entry.getHTMLOrDefault(name);
        var escaped_new_value = escape_if_string(new_value);
        var escaped_old_value = escape_if_string(old_value);
        var event = {
          'name': name,
          'old_value': escaped_old_value,
