    ...
    if (callback) {
        ...
        if (media_type.substring(0,16) === 'application/json') {
            ...
            if (Y.Lang.isValue(representation) &&
                ...
                // If, during a patch request, the resource 'moves' and has a different
                // uri than the self_link of the response, make sure we trigger a resource
                // change event.
                if (method === module.PATCH &&
                    module.get_absolute_uri(uri) !== representation.self_link) {
                    Y.fire('lp:resource:changed', {
                        new_value: representation.self_link
                    });
                } else {
