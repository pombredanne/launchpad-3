...
function display_notifications(notifications) {
    ...
    function remove_notifications(notification_class) {
        var nodes = Y.all('div'+notification_class);
        nodes.each(function(node) {
            var parent = node.get('parentNode');
            parent.removeChild(node);
        });
    }
    remove_notifications('.error.message');
    remove_notifications('.warning.message');
    remove_notifications('.informational.message');
    remove_notifications('.debug.message');
    ...
    var notification_info = {
        ...
        'informational message': infos,
        'warning message': warnings,
        'error message': errors
    };
    Y.each(notification_info, function(notifications, div_class) {
...
Entry.prototype.getHTMLOrDefault = function(key) {
    ...
    // We need to ensure that any default non-html string value is properly
    // escaped. But we don't escape links.
    if (key.substr(-5) !== '_link' && typeof default_value == 'string')
      default_value = Y.Escape.html(default_value);
...
Y.lp.client.plugins.PATCHPlugin = PATCHPlugin;
  ...
  }, "0.1", {"requires": ["plugin", "dump", "escape", "lazr.editor",
    "lp.client"]});
