...
var namespace = Y.namespace('lp.app.listing_navigator');
...
Y.extend(namespace.ListingNavigator, Y.Base, {
     ...
     */
     handle_new_batch: function(batch) {
        var key, i;
        var batch_key = namespace.get_batch_key(batch);
    ...
    update: function(config) {
        ...
        var key = namespace.get_batch_key(config);
 ...
 */
namespace.get_feature_flag = function(flag_name){
 ...
 */
namespace.get_batch_key = function(config) {
...
namespace.get_batch_key_list = function(batch){
    ...
    if (Y.Lang.isNull(prev_config)){
        ...
        keys.push(namespace.get_batch_key(prev_config));
    }
    keys.push(namespace.get_batch_key(batch));
    if (Y.Lang.isNull(next_config)){
        keys.push(null);
    } else {
        keys.push(namespace.get_batch_key(next_config));
    }
    return keys;
};
