...
namespace.addMultiCheckboxPatcher = function (
    ...
    // The activator will flash the content node but that is not sufficient
    // if the contents are not simple text - only a small bit of white space
    // get flashed. So we ensure the actual widget text is flashed.
    var widget_node = Y.one('#'+attribute_name);
    if (widget_node != null) {
        Y.after(function() {
            var anim = Y.lazr.anim.green_flash({node: widget_node});
            anim.run();
        }, activator, 'renderSuccess');
        Y.after(function() {
            var anim = Y.lazr.anim.red_flash({node: widget_node});
            anim.run();
        }, activator, 'renderCancellation');
        Y.after(function() {
            var anim = Y.lazr.anim.red_flash({node: widget_node});
            anim.run();
    ...
    var save = function(editform, selected_items) {
        var choice_nodes = Y.one('[id="'+attribute_name+'.items"]');
        var result = getSelectedItems(
                choice_nodes, selected_items, attribute_type);
        ...
        var parameters = {};
        parameters[attribute_name] = result;
...
/**
  ...
  *
  * @requires dom, dump, lazr.overlay, lazr.overlay
  * @method create
  * @param {String} attribute_name The attribute on the resource being
  *                                modified.
...
namespace.create = function (attribute_name, items, help_text, config) {
    ...
    content.appendChild(
            Y.Node.create("<div class='yui3-lazr-formoverlay-form-header'/>"));
    ...
    Y.Array.each(items, function(data, i) {
        ...
        item_values[data.token] = data.value;
    }, this);
    ...
    var save_button = Y.Node.create(
            '<button class="overlay-close-button lazr-pos lazr-btn">Ok</button>');
    var close_button = Y.Node.create(
            '<button class="overlay-close-button lazr-neg lazr-btn">Cancel</button>');
    save_button.on('click', function(e) {
        e.halt();
        editform.hide();
        config.save(editform, item_values);
    });
 ...
 */
function getSelectedItems(choice_nodes, item_values, attribute_type) {
    var result = [];
    choice_nodes.all('.checkboxType').each(function(item) {
        if (item.get("checked")) {
            var item_token = item.getAttribute("value");
            var item_value = item_values[item_token];
...
function marshall(value, attribute_type) {
    ...
    "io", "dom", "dump", "lazr.overlay", "lazr.activator", "lp.client"
    ]});
