...
/**
 * Produces a slide-out animation as an Y.Anim object.
 *
 * Ensures one can pause and continue an animation without restarting
 * from the beginning, thus allowing nicer reversing of the animation.
 *
 * The target node obtains the 'lazr-opened' CSS class when open,
 * 'lazr-closed' when closed.
 *
 * @method restartable_slide_out
 * @public
 ...
 */
namespace.restartable_slide_out = function(node, user_cfg) {
    ...
    // CSS class should be set properly).
    node.setStyles({
        height:   cfg.from.height,
        overflow: 'hidden'
    });
    anim.is_expanded = false;
    ...
    anim.on('directionChange', function() {
        ...
        // Reversing an animation direction always pauses it if running.
        //this.pause();
    });
    ...
    anim.on('prause', function() {
        // Animation was paused, store the current height as appropriate
        // for the direction it's animating in.
        if (this.is_expanded) {
            this._animation.set(
                'to', { height: this.content_node.getStyle('height') });
        } else {
            this._animation.set(
                'from', { height: this.content_node.getStyle('height') });
        }
...
namespace.slide_out = function(node, user_cfg) {
    ...
    // Set what we need to calculate the new content's scrollHeight.
    // XXX 20110704 Danilo: setting display: block messes up the display
    // if node is a TR for instance.
    node.setStyles({
        height:   cfg.from.height,
        overflow: 'hidden',
        display:  'block'
