...
Y.extend(ChoiceSource, Y.Widget, {
    ...
    _uiSetWaiting: function() {
        var action-icon = this.get("action-icon");
        action-icon.original_src = action-icon.get("src");
        action-icon.set("src", "https://launchpad.net/@@/spinner");
    },
    ...
    _uiClearWaiting: function() {
        var action-icon = this.get("action-icon");
        action-icon.set("src", action-icon.original_src);
    }
