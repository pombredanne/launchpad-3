...
Y.mix(ChoiceListWidget, {
    ...
    ATTRS: {
        ...
        choices: {
            getter: function() {
                // TODO: Make this return object choices.
                return this.fieldNode.all("li > input").get("value");
            },
            setter: function(value, name) {
                var convert = Y.bind(this._convertChoice, this);
                var compare = Y.bind(this._compareChoices, this);
                var choices = Y.Array(value).map(convert).sort(compare);
                var create = Y.bind(this._createChoice, this);
...
Y.extend(ChoiceListWidget, FormRowWidget, {
    ...
    remove_choices: function(choices) {
        var convert = Y.bind(this._convertChoice, this);
        Y.Array(choices).map(convert).forEach(
            function(choice) {
                this.fieldNode.all(
                    "li > input[value=" + choice.value + "]").each(
                        function(li_input) {
    ...
    add_choices: function(choices) {
        ...
        choices = this.get("choices").map(function(choice) {
            if (choicemap.hasOwnProperty(choice.value)) {
        ...
        choices = choices.concat(Y.Object.values(choicemap));
        // Set back again.
