...
Y.extend(ResizingTextarea, Y.Plugin.Base, {
     ...
     */
    _one_line_height: function (val) {
        var clone_one = this.t_area.cloneNode(true);
        clone_one.setStyles({
            width: this.t_area.getComputedStyle('width'),
            resize: 'none',
            height: '1em'
        });

        clone_one.removeAttribute('id');
        clone_one.removeAttribute('name');
        clone_one.generateID();
        clone_one.setAttrs({
            'tabIndex': -1
        });

        // add to the dom
        this.t_area.get('parentNode').appendChild(clone_one);
        var empty_height = clone_one.get('scrollHeight');
    ...
    initializer : function(cfg) {
        ...
        // get the single line height info
        this.one_line_height = this._one_line_height();
    ...
    resize: function() {
        ...
        var one_line = this.get('one_line');

        if (this.get('one_line') && this.one_line_height === scroll_height) {
