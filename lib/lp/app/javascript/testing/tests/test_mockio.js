...
suite.add(new Y.Test.Case({
    ...
    test_success_helper__own_status: function() {
        // The failure can define its own non-5xx status.
        var mockio = this._make_mockio(),
            status = 302;
    ...
    test_success_helper__status_override: function() {
        // A status that is 5xx is overridden to be 200.
    ...
    test_failure_helper__own_status: function() {
        // The failure can define its own 5xx status.
        var mockio = this._make_mockio(),
            status = 503;
    ...
    test_failure_helper__status_override: function() {
        // A status that is not 5xx is overridden to be 500.
        // This is to guard against foot shooting.
        var mockio = this._make_mockio(),
            own_status = 200,
            real_status = 500;
...
suite.add(new Y.Test.Case({
    ...
    test_get_io_provider__default: function() {
        var mockio = undefined,
            io_provider = module.get_io_provider(mockio);
    ...
    test_get_io_provider_from_config__default: function() {
        // If no io_provider is configured, Y is the io_provider.
        var io_provider = module.get_io_provider_from_config({});
        Y.Assert.areSame(Y, io_provider);
    },

    test_get_io_provider_from_config__default_undefined: function() {
        // If no configuration is provided, Y is the io_provider.
        var io_provider = module.get_io_provider_from_config();
        Y.Assert.areSame(Y, io_provider);
    },

    test_get_io_provider_from_config__mockio: function() {
        // If io_provider is configured,  it is picked as the io_provider.
        var mockio = new module.MockIo(),
            io_provider = module.get_io_provider_from_config(
                {io_provider: mockio});
        Y.Assert.areSame(mockio, io_provider);
    },

    test_get_io_provider_from_config__different_key: function() {
