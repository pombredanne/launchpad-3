...
Y.extend(Picker, Y.lazr.PrettyOverlay, {
     ...
     * @param data a json data object with the details to render
     */
    _renderBadgeUI: function(data) {
        if (data.badge) {
            var badge_url = data.badge.url;
            var badge_alt = data.badge.alt;
            ...
            affiliation.appendChild(Y.Node.create(badge_alt));
            return affiliation;
        }
        return null;
    },
    ...
    _syncResultsUI: function() {
        ...
        Y.Array.each(results, function(data, i) {
            // Sort out the badge div.
            var li_badge = this._renderBadgeUI(data);
