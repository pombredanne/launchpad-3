...
Y.extend(Picker, Y.lazr.PrettyOverlay, {
    ...
    _syncResultsUI: function() {
        ...
        Y.Array.each(results, function(data, i) {
            ...
            var icon_node = Y.Node.create('<div></div>');
            icon_node.appendChild(li_title);
            icon_node.appendChild(li_desc);

            var content_node = Y.Node.create(
                '<a class="yui-picker-assign-me-button bg-image" ' +
                'href="javascript:void(0)">' +
                'Select</a>');
            content_node.on('click', function () { this.fire('save') }, this); 
            ...
            li.appendChild(icon_node);
            li.appendChild(content_node);
