...
Y.extend(Picker, Y.lazr.PrettyOverlay, {
    ...
    _renderDetailsUI: function(data) {
        ...
        var select_link = Y.Node.create(
            '<a class="save" href="#">Select ' + data.title + '</a>');
        select_link.on('click', function (e, picker) {
            picker.fire('save');
            }, select_link, this);
        var details_link = this._text_or_link(
            'View details', data.alt_title_link, data.link_css);
        //details_node.append(data.details);
        details_node.append(select_link);
        details_node.append(Y.Node.create('<span>&nbsp;&nbsp;</span>'));
        details_node.append(details_link);
        return details_node;
    },
    ...
    _syncResultsUI: function() {
        ...
        Y.Array.each(results, function(data, i) {
            ...
            // XXX sinzui 2011-07-27: expander with external link.
