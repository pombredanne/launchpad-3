...
suite.add(new Y.Test.Case({
    ...
    test_title_badge: function () {
        ...
        var bb = this.picker.get('boundingBox');
        var li = bb.one('.yui3-picker-results li');
        var img_node = li.one('div.badge img.badge');
        Assert.areEqual(
            badge_info.url, img_node.getAttribute('src'),
            'Unexpected badge url');
        Assert.areEqual(
            badge_info.alt, img_node.get('alt'),
            'Unexpected badge alt text');
        Assert.areEqual(
            badge_info.alt, img_node.get('title'),
            'Unexpected badge title text');
        var badge_node = li.one('div.badge');
        Assert.areEqual(
            badge_info.alt, badge_node.get('text'), 'Unexpected badge text');
    },
