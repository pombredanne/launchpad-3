...
suite.add(new Y.Test.Case({
    ...
    create_picker: function(yio) {
        var config = {
            "step_title": "Choose something",
            "header": "Pick something",
            "validate_callback": null
            };
        if (Y.Lang.isValue(yio)) {
            config.yio = yio;
        }
    ...
    test_automated_search_results_ignored_if_user_has_searched: function() {
        // If an automated search (like loading suggestions) returns results
        // and the user has submitted a search, then the results are ignored
        ...
        Assert.areEqual(null, picker.get('results'));
