...
function setup_mute_link_handlers() {
    ...
    mute_link.on('click', function(e) {
        ...
        if (! is_muted) {
            Y.log("Muting subscription.");
            mute_subscription.set(
                'bug_notification_level', "Nothing");
            success_callback = function() {
                Y.log("Success callback - mute");
            ...
            Y.log("Unmuting subscription.");
            success_callback = function() {
                Y.log("Success callback - unmute");
...
function update_subscription_after_mute_or_unmute(mute_subscription) {
    Y.log("Updating subscription link.");
    ...
    if (mute_subscription.get('is_muted')) {
        ...
        subscription.disable_spinner(subscription_labels.EDIT);
    }
}
