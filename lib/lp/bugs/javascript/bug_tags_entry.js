...
namespace.setup_tag_entry = function(available_official_tags) {
    ...
    namespace.setup_tag_completer(
        '#tag-input', available_official_tags, false);
};
 ...
 *
 * @method setup_tag_completer
 */
namespace.setup_tag_completer = function(input, official_tags, render) {
    // Disable the browser autocomplete so that it does not conflict.
    Y.log('setting up BugTasCompleter.');
    Y.one(input).set('autocomplete', 'off');
    Y.log('Disabled browser autocomplete.');
    Y.one('body').appendChild(
        '<div id="tags-autocomplete"><div id="tags-autocomplete-content">' +
        '</div></div>');
    Y.log('Added tag menu.');
    ...
    if (render) {
        autocomplete.render();
        Y.log('Rendered the AutoComplete widget.');
    }
};
}, "0.1", {
    "requires": [
