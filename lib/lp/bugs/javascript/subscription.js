...
function render_reason(reason) {
    ...
    var vars = {
        // XXX This should be .id, but it's not exposed.
        'bug_id': 'XXX' //LP.cache.context.id
    };
    for (var name in reason.vars) {
        vars[name] =
            '<a href="' + reason.vars[name].url + '">' +
            reason.vars[name].title + '</a>';
    }
    return Y.substitute(text, vars);
}
...
function show_subscription_description(config) {
    ...
    if (direct === null) {
        // There are no direct subscriptions.
        if (subs.length > 0) {
        ...
        for (var index in subs) {
            ...
            other_node.appendChild(
                Y.Node.create('<div></div>')
                    ...
                    .set('innerHTML', render_reason(sub)));
        }
