...
function make_action_link_function (node, sprite_class, method_name) {
    var config = {
        on: {success:
            function (maybe_sub) {
    ...
    node.on(
        ...
        function (e) {
            ...
            var method_call = function() {
                make_action_link_function(node, sprite_class, method_name);
            }
            // If this a private bug, and the user is unsubscribing
            // themselves, give them a warning.
            if (method_name === 'unsubscribe') {
                var co = new Y.lp.app.confirmoverlay.ConfirmationOverlay({
                    submit_fn: method_call,
                    form_content: 'Are you sure?',
                    headerContent: 'Confirm unsubscription'
                });
                co.show()
            } else {
                method_call();
