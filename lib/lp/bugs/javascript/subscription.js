...
function gather_subscriptions_by_role(
    ...
    var results = [];
    var work = [{subscriptions: category.as_team_member,
                 config: team_config},
                {subscriptions.as_team_admin,
    ...
    for (var work_index in work) {
        ...
        if (subscriptions.length > 0) {
            ...
            for (var index in subscriptions) {
                ...
                var team = team_subscription.principal;
                if (!Y.isValue(team_map[team])) {
                    var link_data = get_link_data(team);
                    team_map[team] = link_data;
    ...
    if (category.personal.length > 0) {
        subscriptions.push(
            { reason: reasons.YOU_ASSIGNED,
              vars: {},
              actions: actions.CHANGE_ASSIGNEES });
    }
...
function gather_subscriptions_as_supervisor(category) {
    ...
    for (var index in category.personal) {
        ...
        subscriptions.push({
            ...
            actions: actions.SET_BUG_SUPERVISOR
        });
    }
    ...
    if (category.personal.length > 0) {
        ...
        var sub = choose_by_number(
            ...
            { reason: reasons.YOU_SUBSCRIBED_TO_DUPLICATE,
              ...
              actions: actions.UNSUBSCRIBE_DUPLICATE},
            { reason: reasons.YOU_SUBSCRIBED_TO_DUPLICATES,
              vars: { duplicate_bugs: dupes },
              actions: actions.UNSUBSCRIBE_DUPLICATES});
        sub['actions'] = actions.UNSUBSCRIBE_DUPLICATES
        subscriptions.push(sub);
    }
