...
Y.mix(reasons, {
    ...
    TEAMS_SUBSCRIBED_TO_DUPLICATE: (
        _BECAUSE_TEAMS_ARE + _SUBSCRIBED_PLURAL_TO_DUPLICATE),
    TEAMS_SUBSCRIBED_TO_DUPLICATES: (
    ...
    ADMIN_TEAMS_SUBSCRIBED_TO_DUPLICATES: (
...
/**
 * Return appropriate string based on the number.
 *
 * @method choose_string_by_number.
 * @param {Integer} number Number used in the string.
 * @param {String} singular String to return when number == 1.
 * @param {String} plural String to return when number != 1.
 */
function choose_string_by_number(number, singular, plural) {
    if (number == 1) {
        return singular;
    } else {
        return plural;
...
function get_assignee_reason(category) {
    ...
    if (category.personal.length > 0) {
        ...
        reason = choose_string_by_number(
