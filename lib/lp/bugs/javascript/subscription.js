...
/**
 * Convert a team object to a { title, url } object for use in web pages.
 * Additionally, accepts a string as well and returns it unmodified.
 */
function get_team_link_data(team) {
    // For testing, we take strings as well.
    if (typeof(team) == 'string') {
        return team;
    } else {
        return ObjectLink(team.display_name, team.web_link);
    }
}
...
function gather_subscriptions_as_assignee(category) {
    ...
    if (category.as_team_member.length > 0) {
        ...
        for (var index in category.as_team_member) {
            ...
            teams.push(get_team_link_data(team_subscription.principal));
        }
...
function gather_subscriptions_as_supervisor(category) {
    ...
    for (var index in category.as_team_member) {
        ...
        subscriptions.push({
            ...
            vars: {
                team: get_team_link_data(team_subscription.principal),
                pillar: team_subscription.pillar
            }
        });
    }
...
function get_subscription_reason(config) {
    ...
    var subs = gather_subscriptions_as_assignee(info.as_assignee);
    console.log(subs);
