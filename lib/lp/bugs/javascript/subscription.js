...
function add_url_element_to_link(link, url_suffix) {
    return ObjectLink(
        link.self,
        link.title,
        link.url + url_suffix);
...
function add_url_element_to_links(links, url_suffix) {
    ...
    for (var index in links) {
        result.push(add_url_element_to_link(links[index], url_suffix));
    }
    return result;
}
...
var actions = {
    CHANGE_ASSIGNEES: function () {
        return Y.Node.create('<a>Change assignees for this bug</a>')
            .set('href', LP.cache.context.web_link); // XXX Make testable.
    },
    UNSUBSCRIBE_DUPLICATES: function (args) {
        var node = Y.Node.create(
            '<a href="#" class="sprite modify remove js-action"></a>'),
            client = lp_client(),
            handler = new Y.lp.client.ErrorHandler(),
            subscriptions = [];
        if (Y.Lang.isValue(args.teams) && args.teams.length > 0) {
            // Unsubscribe team.
    ...
    CHANGE_TEAM_SUBSCRIPTIONS: function (args) {
        ...
        var teams = [],
            ...
            handler = new Y.lp.client.ErrorHandler();
        if (args.teams.length > 1) {
            node.set('text', 'Unsubscribe all of these teams');
        } else {
            node.set('text', 'Unsubscribe this team');
        }
        ...
        for (var index in args.teams) {
            teams.push(args.teams[index].self.self_link);
        }
        node.on('click', function (e) {
            e.halt();
            var callback = function () {
