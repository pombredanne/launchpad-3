...
/**
 * The URL of the bug filing guidelines view.
 */
var filebug_guidelines_url;
/**
 * The URL of the bug filing optional extras view.
 */
var filebug_optional_extras_url;
...
function set_product_urls()
{
    ...
    if (Y.Lang.isValue(product_field)) {
        ...
        filebug_guidelines_url =
            filebug_base_url + product + '/+filebug-reporting-guidelines';
        filebug_optional_extras_url =
            filebug_base_url + product + '/+filebug-extra-options';
        search_url_base =
...
function reload_filebug_form() {
    var guidelines_config = {
        on: {
            success: function(transaction_id, response, args) {
                ...
                Y.one('#extra-filebug-details').set(
                        INNER_HTML, response.responseText);
    ...
    var optional_fields_config = {
        on: {
            success: function(transaction_id, response, args) {
                if (response.responseText=='')
                    return;
                var form_info = Y.JSON.parse(response.responseText);
                var show_fields = form_info['show_fields'];
                var hide_fields = form_info['hide_fields'];

                // We assume the fields are rendered using the Launchpad
                // form infrastructure, so that the field widgets are
                // contained inside a table row. We show/hide the row.
                var find_field_node_parant = function(field_name) {
                    var node = Y.one(Y.DOM.byId('field.'+field_name));
                    if (node != null)
                        return node.ancestor('tr');
                };

                Y.each(show_fields, function(field_name){
                    var node = find_field_node_parant(field_name);
                    if (node !=null)
                        node.setStyle(DISPLAY, BLOCK);
                });
                Y.each(hide_fields, function(field_name){
                    var node = find_field_node_parant(field_name);
                    if (node != null)
    ...
    YIO.io(filebug_guidelines_url, guidelines_config);
    YIO.io(filebug_optional_extras_url, optional_fields_config);
}
