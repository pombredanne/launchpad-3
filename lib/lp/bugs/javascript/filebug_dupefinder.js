    ...
    var config = {
        on: {
            success: function(transaction_id, response, args) {
                ...
                extra_filebug_details.set(INNER_HTML, response.responseText);

                // Clear the contents of the search results.
                Y.one('#possible-duplicates').set(INNER_HTML, '');

                if (filebug_form_was_visible) {
                    // Change the label on the search button to inform the user
                    // they can re-check for duplicates against the currently
                    // selected project.
                    search_button.set('value', 'Check again');
                } else {
