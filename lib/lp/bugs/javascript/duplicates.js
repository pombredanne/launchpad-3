...
namespace.MarkBugDuplicate = Y.Base.create("markBugDupeWidget", Y.Widget, [], {
    ...
    renderUI: function() {
        ...
        this.duplicate_form.loadFormContentAndRender(mark_dupe_form_url);
    },
        ...
        var config = {
            on: {
                ...
                failure: function(id, response) {
                    that.duplicate_form.showError(response.responseText);
                }
            },
            data: qs_data
        };
    ...
    _bug_details_template: function() {
        ...
        '      <a href="{{bug_url}}" class="bugtitle sprite new-window">{{bug_summary}}</a>',
        '      <div class="buginfo-extra">',
    ...
    _show_bug_details_node: function(new_bug_details_node) {
        var form = this.duplicate_form.form_node.one('.form');
    ...
    _hide_bug_details_node: function() {
        ...
        this.duplicate_form.clearError();
    ...
    _update_bug_duplicate_failure: function(response, old_dup_url, new_dup_id) {
        ...
        this.duplicate_form.showError(error_msg);
        this._xaxis_centre();
    },
