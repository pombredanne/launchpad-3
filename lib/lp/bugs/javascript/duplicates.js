...
namespace.MarkBugDuplicate = Y.Base.create("markBugDupeWidget", Y.Widget, [], {
    ...
    bindUI: function () {
        ...
        // We the overlay is hidden, we need to reset the form.
        this.duplicate_form_overlay.after('visibleChange', function() {
            if (!this.get('visible')) {
    ...
    /**
     * Show a spinner next to the bug entry field.
     *
     * @method _showBugSpinner
     */
    ...
    /**
     * Hide the bug search spinner.
     *
     * @method _hideBugSpinner
     */
    _hide_bug_spinner: function(node) {
        if( node === null) {
            return;
        }
        var spinner = this.duplicate_form_overlay.get('contentBox').one('.spinner');
        if (spinner !== null) {
            spinner.remove();
        }
    },
    ...
    _find_duplicate_bug: function(data) {
        ...
        var bug_id = this.duplicate_form_overlay.get('contentBox')
            .one('[id="field.duplicateof"]');
        var config = {
            on: {
                start: function() {
                    that._show_bug_spinner(bug_id);
                    that.duplicate_form_overlay.clearError();
                },
                end: function() {
                    that._hide_bug_spinner(bug_id);
                },
                ...
                failure: function(id, response) {
                    ...
                    if (response.status === 404) {
                        error_msg = new_dup_id +
                            ' is not a valid bug number or nickname.'
                    } else {
                        error_msg = response.responseText;
                    }
                    that.duplicate_form_overlay.showError(error_msg);
                }
            },
            data: qs_data
        };
    ...
    _bug_confirmation_form_template: function() {
        return [
            '<div class="confirmation-node yui3-lazr-formoverlay-form" style="margin-top: 5px;">',
            '{{> bug_details}}',
    ...
    _confirm_selected_bug: function(bug_data) {
        ...
        var duplicate_form = this.duplicate_form_overlay.form_node;
        this.form_actions =
            duplicate_form.one('.yui3-lazr-formoverlay-actions');
        this.form_errors = this.duplicate_form_overlay.error_node;
        this.form_widgets = duplicate_form.one('>:first-child');
...
//                that._hide_confirm_node(confirm_node);
        ...
        confirm_node.one(".cancel_button")
            .on('click', function(e) {
                e.halt();
                that.duplicate_form_overlay.hide();
            });
    },

    _show_confirm_node: function(confirm_node) {
//        this.form_widgets.addClass('hidden');
//        this.form_actions.addClass('hidden');
        this.duplicate_form_overlay.form_node.addClass('hidden');
        this.duplicate_form_overlay.form_header_node.appendChild(confirm_node);
//        this.form_errors.insert(confirm_node, 'after');
    },

    _hide_confirm_node: function() {
        this.duplicate_form_overlay.form_node.removeClass('hidden');
        var confirmation_node = this.duplicate_form_overlay.form_header_node.one('.confirmation-node');
        if (Y.Lang.isValue(confirmation_node)) {
            confirmation_node.remove(true);
        }
//        var confirm_node = this.duplicate_form_overlay.form_node
//            .one('.confirmation-node');
//        if (Y.Lang.isValue(confirm_node)) {
//            confirm_node.remove(true);
//        }
//        if (Y.Lang.isValue(this.form_widgets)) {
//            this.form_widgets.removeClass('hidden');
//            this.form_actions.removeClass('hidden');
//        }
    },
    ...
    _update_bug_duplicate_failure: function(response, old_dup_url, new_dup_id) {
        ...
        this.duplicate_form_overlay.get('contentBox').one('.yui3-lazr-formoverlay-errors')
            .setContent(error_msg);
        this.duplicate_form_overlay.get('contentBox').one(".yes_button").addClass('hidden');
//        this.duplicate_form_overlay.showError(error_msg);
...
//        this.duplicate_form_overlay.hide();
        ...
        var dupe_span = this.get('dupe_span');
        var that = this;
        var submit_btn = this.duplicate_form_overlay.get('contentBox').one(".yes_button");
        var config = {
            on: {
                start: function() {
