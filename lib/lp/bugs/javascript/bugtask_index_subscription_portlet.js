...
function setup_subscription_link_handlers() {
    ...
    // level.  For each subscription level ("Discussion," "Details" and
    // "Lifecycle") we create both an action link and a span.  When we
    // draw an overlay, we decide which of them is appropriate for each level.
    // When you are first creating a new subscription, we show all links, and
    // no spans.  When you are changing an existing subscription, we display
    // the (non-active) span for the current level, and links for the other
    // two levels.
    ...
    // has an existing direct subscription.  We have two variants.  If the
    // user has subscriptions to this bug other than the direct subscription,
    // we show a version that includes a warning, and encourages muting as
    // a more effective alternative.  Otherwise, we show a simple unsubscribe
    // link.
