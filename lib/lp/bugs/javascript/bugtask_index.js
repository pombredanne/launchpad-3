...
namespace.setup_bugtask_row = function(conf) {
    ...
    if (Y.Lang.isValue(assignee_content)) {
        var validate_assignee = function(picker, value, saveFn, cancelFn, resetFn) {
            if (value == null)
                return true;
            ...
            var result = lp_client.named_get(
                    conf.bugtask_path, "isContributor", config);
            var success = result.status == 200 && result.responseText == 'true';
            if (!success) {
                // Handle assignment to non contributor
                var yesno_content = "<p>Marilize did not previously have any assigned bugs in Mozilla Firefox.</p>" +
                        "<br/>Do you really want to assign them to this bug?";

                return Y.lp.app.picker.yesno_save_confirmation(picker, yesno_content, saveFn, cancelFn, resetFn);
            } else {
                if (Y.Lang.isFunction(saveFn))
                    saveFn();
            }
            return success;
        };
