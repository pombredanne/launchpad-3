...
namespace.setup_bugtask_index = function() {
    ...
    Y.on('load', function() {
        ...
        if (update_dupe_link) {
            ...
            var form_header = '<p>Marking the bug as a duplicate will, by default, ' +
            ...
            if (has_dupes !== null) {
                form_header = form_header +
                    ...
                    'class="large-warning">This bug also has duplicates. ' +
...
function setup_subscribe_someone_else_handler(subscription) {
    ...
    var picker = Y.lp.picker.create('ValidPersonOrTeam', config);
}
...
function setup_link_branch_picker() {
    ...
    if (Y.Lang.isValue(link_branch_link)) {
        ...
        var picker = Y.lp.picker.create('Branch', config);
    }
}
...
function load_subscriber_ids() {
    ...
    Y.io(url, config);
                      ...
                      "lazr.choiceedit", "lp.picker", "lp.client.plugins",
                      "lp.bugs.subscriber", "lp.errors"]});
