 ...
 * interactions.
 *
 * @method setup_subscription_overlay
 * @param subscription {Object} A Y.lp.bugs.subscriber.Subscription object.
 */
function setup_subscription_overlay(subscription) {
...
function setup_subscription_link_handlers() {
    ...
    if (namespace.use_advanced_subscriptions) {
        // If we're using the advanced subscriptions UI, setup the
        // overlay here so that when the user clicks the link they won't
        // have to wait for the form to be fetched from the server.
        var subscription_overlay = setup_subscription_overlay(
            subscription)
    }
