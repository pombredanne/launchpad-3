 ...
 */
namespace._render_bugtask_table = function(table_node) {
...
namespace._process_bugtask_delete_response = function(response, row_id) {
    ...
    if( content_type === 'application/json' ) {
        ...
        window.location.replace(redirect.bugtask_url);
        return;
    }
    // We have received HTML, so we replace the current bugtask table with a
    // new one.
    var new_table = Y.Node.create(response.responseText);
    var on = {
        end: function() {
            namespace._render_bugtask_table(new_table);
            Y.lp.client.display_notifications(
                response.getResponseHeader('X-Lazr-Notifications'));
        }
    };
