...
var update_subscription_portlet = function() {
    var portal_node = Y.one('#portlet-subscription');
    ...
    var config = {
        on: {
            success: function (id, response) {
                ...
                var new_content = Y.Node.create(details.html);
                portal_node.replace(new_content);
                var item;
                for (item in details.cache_data) {
                    if (details.cache_data.hasOwnProperty(item)) {
                        Y.log(item + "="+details.cache_data[item]);
                        LP.cache[item] = details.cache_data[item];
                    }
                }
