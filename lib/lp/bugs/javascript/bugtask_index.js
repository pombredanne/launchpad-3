...
namespace.showDeleteSpinner = function(delete_link) {
...
namespace.hideDeleteSpinner = function(delete_link) {
...
namespace.render_bugtask_table = function(table_node) {
    var bugtask_table = Y.one('#affected-software');
    bugtask_table.replace(table_node);
    namespace.setup_bugtask_table();
};

namespace.confirm_bugtask_delete = function(delete_link, conf) {
    var delete_text = [
        ...
        conf.targetname,
        '.<br><br>Please confirm you really want to do this.</p>'
        ].join('');
...
namespace.delete_bugtask = function (delete_link, conf) {
    ...
    var y_config = {
        ...
        on: {
            start:
                function() {
                    namespace.showDeleteSpinner(delete_link);
                },
            end:
                function() {
                    namespace.hideDeleteSpinner(delete_link);
                },
            failure:
                ...
                function(id, response) {
                    Y.lp.anim.red_flash({
                        ...
                        on: {
                                ...
                                var new_table = Y.Node.create(
                                    response.responseText);
                                namespace.render_bugtask_table(new_table);
                                Y.lp.client.display_notifications(
...
namespace.setup_bugtask_row = function(conf) {
    ...
    if (Y.Lang.isValue(LP.links.me) && Y.Lang.isValue(delete_link)
        ...
        delete_link.on('click', (function (e) {
            e.preventDefault();
            namespace.confirm_bugtask_delete(delete_link, conf);
        }));
    }
