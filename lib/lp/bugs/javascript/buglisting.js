...
Y.extend(namespace.ListingNavigator, Y.Base, {
    initializer: function(config) {
        ...
        config.target.plug(Y.lp.indicator.IndicatorPlugin);
    ...
    set_pending: function (is_pending) {
        var indicator = this.get('target').indicator;
        if (is_pending){
            indicator.show();
        } else {
            indicator.hide();
...
}, "0.1", {"requires": ["history", "node", 'lp.client', 'lp.app.errors', 'lp.indicator']});
