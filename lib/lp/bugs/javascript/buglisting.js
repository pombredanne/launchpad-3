...
namespace.ListingNavigator.prototype.getBatchKey = function(order_by){
    return JSON.stringify([order_by]);
};
...
namespace.ListingNavigator.prototype.update_from_model = function(model){
    this.batches[this.getBatchKey(model.order_by)] = model.mustache_model;
...
namespace.ListingNavigator.prototype.get_batch_query = function(config){
    ...
    if (config.forward !== undefined && !config.forward){
...
namespace.ListingNavigator.prototype.change_ordering = function(order_by){
    var query;
    var cached_batch = this.batches[this.getBatchKey(order_by)];
    if (Y.Lang.isValue(cached_batch)){
        this.update_from_model(cached_batch);
        return;
    }
    this.load_model({
        order_by: order_by, start: null, forward: true, memo: null});
};
...
namespace.ListingNavigator.prototype.next_batch = function(order_by){
    this.load_model({
        forward: true,
...
namespace.ListingNavigator.prototype.prev_batch = function(order_by){
    this.load_model({
        forward: false,
