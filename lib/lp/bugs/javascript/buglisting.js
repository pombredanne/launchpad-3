...
Y.extend(namespace.ListingNavigator, Y.Base, {
     ...
     */
    update_from_model: function(fetch_only, model) {
    ...
    get_pre_fetch_configs: function(){
        configs = [];
        configs.push(this.next_batch_config());
        //configs.push(this.first_batch_config());
        return configs;
    },
    /**
    ...
    load_model: function(config) {
        ...
        load_model_config = {
            on: {
                success: Y.bind(
                    this.update_from_model, this, config.fetch_only),
