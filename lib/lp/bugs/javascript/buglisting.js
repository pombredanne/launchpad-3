...
Y.extend(namespace.ListingNavigator, Y.Base, {
    initializer: function(config) {
        ...
        var batch_key = this.handle_new_batch(cache);
    ...
    /**
     * Function called when history position changes.
     *
     * e is expected to be a history:change event.
     */
    ...
    update_from_new_model: function(model){
        this.update_from_model(model, this.handle_new_batch(model));
    },
     ...
     */
    update_from_model: function(model, batch_key) {
        var query = this.get_batch_query(model);
    ...
    update: function(config) {
        ...
        if (Y.Lang.isValue(cached_batch)) {
            this.update_from_model(cached_batch, key);
        }
        else {
    ...
    load_model: function(config) {
        ...
        var load_model_config = {
            on: {
                success: Y.bind(function(model){
                    var batch_key = this.handle_new_batch(model)
                    this.update_from_model(model, batch_key);
                 }, this)
            }
        };
