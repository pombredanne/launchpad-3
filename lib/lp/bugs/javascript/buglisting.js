...
namespace.search_params_to_query = function(search_params) {
    var param_name, param_value, field_name;
    var query = {};
    for (param_name in search_params){
        if (search_params.hasOwnProperty(param_name)){
            field_name = 'field.' + param_name;
            param_value = search_params[param_name];
            if (param_value === true || param_value === false){
                query[field_name + '.used'] = '';
            }
            if (!Y.Lang.isValue(param_value)){
                continue;
            }
...
namespace.update_listing = function(order_by, config){
    ...
    var query = namespace.search_params_to_query(LP.cache.search_params);
