...
Y.extend(namespace.ListingNavigator, Y.Base, {
    initializer: function(config) {
        ...
        this.set('history', new Y.History({
            initialState: Y.merge({
                batch_key: batch_key
            }, cache.field_visibility)
        }));
        this.get('history').after('change', this.history_changed, this);
        this.set('field_visibility_names', cache.field_visibility);
    },
    ...
    get_field_visibility: function(){
        ...
        var key_source = this.get('field_visibility_names');
    ...
    get_current_batch: function(){
        var batch_key = this.get('history').get('batch_key');
