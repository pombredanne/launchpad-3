...
suite.add(new Y.Test.Case({
    ...
    test_contentready_event: function() {
        // When initialize is done, 'bugnotificationlevel:contentready'
        // event is fired.
        var level_node = Y.Node.create('<div></div>');
        var node = Y.Node.create('<div></div>');
        node.appendChild(createRadioButton(this.MY_NAME, true));
        ...
        var state = module._initialize(radio_buttons, level_node);
        this.wait(function() {
            // We wait for the event just-in-case, though test passes for
            // me even without the wait.
