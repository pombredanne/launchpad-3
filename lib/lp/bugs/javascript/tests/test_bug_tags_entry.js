YUI().use('lp.testing.runner', 'lp.testing.mockio', 'test', 'console',
    ...
    suite.add(new Y.Test.Case({
        ...
        test_setup_tag_entry: function() {
            ...
            Y.Assert.isInstanceOf(Y.Node, form_node.one('#tag-input'));
            // The Autocompleter nodes are provideds.
    ...
    suite.add(new Y.Test.Case({
        ...
        test_setup_tag_completer: function() {
            // The Autocompleter nodes are provided.
            module.setup_tag_completer(
                'input[id=field.tag]',['project-tag'], true);
            ...
            Y.Assert.areEqual('off', input.get('autocomplete'));
        }
    }));
