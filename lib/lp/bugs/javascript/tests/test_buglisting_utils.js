...
var buglisting_display_utils_tests = new Y.Test.Case({
    ...
    getActualInputData: function() {
        ...
        var actual_checked = [];
        var inputs = Y.one(
            '.yui3-lp-ui-formoverlay-content form').all('input');
    ...
    test_click_icon_reveals_overlay: function() {
        ...
        Assert.isTrue(overlay.hasClass('yui3-lp-ui-formoverlay-hidden'));
        var config = Y.one('.config');
        config.simulate('click');
        Assert.isFalse(overlay.hasClass('yui3-lp-ui-formoverlay-hidden'));
    },
