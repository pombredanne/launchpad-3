...
/**
 * XXX gary 2011-05-26 bug 793579
...
suite.add(new Y.Test.Case({
    ...
    test_overlay_add_subscription: function() {
        ...
        // The action links are visible; the "You currently..." spans are not.
        Y.Assert.isFalse(overlay.one('#Discussion a').hasClass('hidden'));
        Y.Assert.isTrue(overlay.one('#Discussion span').hasClass('hidden'));
        Y.Assert.isFalse(overlay.one('#Details a').hasClass('hidden'));
        Y.Assert.isTrue(overlay.one('#Details span').hasClass('hidden'));
        Y.Assert.isFalse(overlay.one('#Lifecycle a').hasClass('hidden'));
        Y.Assert.isTrue(overlay.one('#Lifecycle span').hasClass('hidden'));
        // There are no other links (we test this by checking for a "remove"
        // link, which is always present in other cases).
    ...
    test_overlay_change_subscription: function() {
        ...
        Y.Assert.isTrue(overlay.one('#Discussion a').hasClass('hidden'));
        Y.Assert.isFalse(overlay.one('#Discussion span').hasClass('hidden'));
        Y.Assert.isFalse(overlay.one('#Details a').hasClass('hidden'));
        Y.Assert.isTrue(overlay.one('#Details span').hasClass('hidden'));
        Y.Assert.isFalse(overlay.one('#Lifecycle a').hasClass('hidden'));
        Y.Assert.isTrue(overlay.one('#Lifecycle span').hasClass('hidden'));
        ...
        // ...but not a mute link.
        Y.Assert.isFalse(Y.Lang.isValue(overlay.one('a.mute')));
        Y.Assert.areEqual(
            'Remove your direct subscription',
            overlay.one('div#subscription-levels+div').get('text'));
    },

    test_overlay_change_subscription_with_other_subscriptions: function() {
        // Starting with a DETAILS subscription, we get a "change" overlay
        // with extra warning text.
        ...
        Y.Assert.areEqual('Change your mail subscription for this bug',
                          overlay.one('h2').get('text'));
        // The action links are visible except for Details.
        Y.Assert.isFalse(overlay.one('#Discussion a').hasClass('hidden'));
        Y.Assert.isTrue(overlay.one('#Discussion span').hasClass('hidden'));
        Y.Assert.isTrue(overlay.one('#Details a').hasClass('hidden'));
        Y.Assert.isFalse(overlay.one('#Details span').hasClass('hidden'));
        Y.Assert.isFalse(overlay.one('#Lifecycle a').hasClass('hidden'));
        Y.Assert.isTrue(overlay.one('#Lifecycle span').hasClass('hidden'));
        // We have a remove link...
        Y.Assert.isTrue(Y.Lang.isValue(overlay.one('a.remove')));
        // ...a mute link...
        Y.Assert.isTrue(Y.Lang.isValue(overlay.one('a.mute')));
        // ...and an edit link.
        Y.Assert.isTrue(Y.Lang.isValue(overlay.one('a.edit')));
        Y.Assert.areEqual(
            'You may also remove your direct subscription, but this might '+
            'not stop all your email about the bug. Mute bug mail to make '+
            'sure you never receive email about it, or edit bug mail to see '+
            'details of all your subscriptions to this bug.',
            overlay.one('div#subscription-levels+div').get('text'));
    },

    test_overlay_with_lifecycle_subscription: function() {
        // The previous two tests were at DISCUSSION and DETAILS; now we'll
        // show LIFECYCLE for completeness.
        ...
        Y.Assert.isFalse(overlay.one('#Details a').hasClass('hidden'));
        Y.Assert.isTrue(overlay.one('#Details span').hasClass('hidden'));
        Y.Assert.isTrue(overlay.one('#Lifecycle a').hasClass('hidden'));
        Y.Assert.isFalse(overlay.one('#Lifecycle span').hasClass('hidden'));
    },
    ...
    test_subscribe_discussion: function() {
        ...
        this.link().simulate('click');
        var overlay = Y.one('.pretty-overlay-window');
        overlay.one('#Discussion a').simulate('click');
    ...
    test_subscribe_details: function() {
        ...
        this.link().simulate('click');
        var overlay = Y.one('.pretty-overlay-window');
        overlay.one('#Details a').simulate('click');
        // We got an appropriate call to the webservice.
        Y.Assert.areEqual('named_post', module._lp_client.received[0][0]);
        var args = module._lp_client.received[0][1];
        Y.Assert.areEqual(this.bug_link, args[0]);
    ...
    test_subscribe_lifecycle: function() {
        ...
        this.link().simulate('click');
        var overlay = Y.one('.pretty-overlay-window');
        overlay.one('#Lifecycle a').simulate('click');
        // We got an appropriate call to the webservice.
        Y.Assert.areEqual('named_post', module._lp_client.received[0][0]);
        var args = module._lp_client.received[0][1];
        Y.Assert.areEqual(this.bug_link, args[0]);
    ...
    test_mute: function() {
        ...
        this.link().simulate('click');
        var overlay = Y.one('.pretty-overlay-window');
        overlay.one('a.mute').simulate('click');
        // We got an appropriate call to the webservice.
        Y.Assert.areEqual('named_post', module._lp_client.received[0][0]);
        var args = module._lp_client.received[0][1];
        Y.Assert.areEqual(this.bug_link, args[0]);
        Y.Assert.areEqual('mute', args[1]);
        Y.ObjectAssert.areEqual({}, args[2].parameters);
        // The overlay has been destroyed.
        Y.Assert.isFalse(Y.Lang.isValue(Y.one('.pretty-overlay-window')));
        // The link has updated its text.
        Y.Assert.areEqual(
            "Your personal email notifications from this bug are muted.",
        ...
        // The mute node is not hidden...
        Y.Assert.isFalse(this.mute_node.hasClass('hidden'));
        // ...and it has changed to be an "unmute" link.
        Y.Assert.areEqual('Unmute bug mail', this.mute_node.get('text'));
        Y.Assert.isTrue(this.mute_link.hasClass('unmute'));
        Y.Assert.isFalse(this.mute_link.hasClass('mute'));
    },
    ...
    test_io_spinner_and_error: function() {
        ...
        this.link().simulate('click');
        var overlay = Y.one('.pretty-overlay-window');
        var inner_mute = overlay.one('a.mute')
        inner_mute.simulate('click');
        // Right now, this is as if we are waiting for the server to
        // reply. The link is spinning.
        Y.Assert.isTrue(inner_mute.hasClass('spinner'));
        Y.Assert.isFalse(inner_mute.hasClass(module.UNMUTED_CLASS));
        // Now the server replies with an error.
        module._lp_client.named_post.resume();
        // We have no spinner.
        Y.Assert.isTrue(inner_mute.hasClass(module.UNMUTED_CLASS));
        Y.Assert.isFalse(inner_mute.hasClass('spinner'));
        // The page has rendered the error overlay.
        var error_box = Y.one('.yui3-lazr-formoverlay-errors');
        Y.Assert.isTrue(Y.Lang.isValue(error_box));
