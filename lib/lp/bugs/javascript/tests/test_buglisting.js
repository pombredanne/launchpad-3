...
suite.add(new Y.Test.Case({
    ...
    test_get_prefech_queries: function(){
        var navigator = get_navigator();
        var configs = navigator.get_prefetch_configs();
        var batch_keys = []
        ...
        Y.Assert.areSame('["foo",467,true,500]', batch_keys[1]);
        Y.Assert.areSame('["foo",null,true,0]', batch_keys[2]);
    }
}));
