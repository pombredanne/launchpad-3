...
suite.add(new Y.Test.Case({
    ...
    test_change_ordering_uses_cache: function() {
        /* change_ordering will use the cached value instead of making a second
         * AJAX request. */
        var navigator = this.get_intensity_listing();
...
suite.add(new Y.Test.Case({
    ...
    test_last_batch: function(){
        var navigator = get_navigator('?memo=pi&direction=backwards&orderby=foo')
    ...
    test_first_batch: function(){
        var navigator = get_navigator('?memo=pi&orderby=foo')
