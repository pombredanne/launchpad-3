...
var subscriber_levels = {
    ...
    'Comments': 'Notified of all changes except comments',
 ...
 */
var subscriber_level_order = ['Details', 'Comments', 'Lifecycle', 'Maybe'];
...
SubscribersList.prototype.resetNoSubscribers = function() {
    ...
    if (has_sections) {
        ...
        var no_subs = this.container_node.one('.' + CSS_CLASSES.no_subscribers);
        if (no_subs !== null) {
            no_subs.remove();
        }
    } else {
        var no_subs = Y.Node.create('<div></div>')
...
SubscribersList.prototype._insertSectionNode = function(level, section_node) {
    ...
    for (index in subscriber_level_order) {
