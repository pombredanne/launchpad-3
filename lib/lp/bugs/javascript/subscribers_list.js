...
function setup_subscribe_someone_else_handler(subscribers_list) {
    ...
    config.save = function(result) {
        ...
        subscribe_someone_else(subscribers_list, person);
    };
...
function subscribe_someone_else(subscribers_list, person) {
    subscribers_list.addSubscriber(person, 'Discussion');
    subscribers_list.indicateSubscriberActivity(person);
    function on_success() {
        subscribers_list.stopSubscriberActivity(person, true);
    }
    function on_failure() {
        subscribers_list.stopSubscriberActivity(person, false, function() {
            subscribers_list.removeSubscriber(person);
        });
    }
    // Simulate the subscribing dance.
    setTimeout(on_failure, 2400);
}

var unsubscribe_callback = function(sl, subscriber) {
    sl.indicateSubscriberActivity(subscriber);
    // Simulated unsubscribing action for prototyping the UI.
    setTimeout(function() {
        sl.stopSubscriberActivity(subscriber, true, function() {
            sl.removeSubscriber(subscriber);
        });
    }, 2400);
};

function getSubscribersList(config) {
    var sl = new SubscribersList(config);
    ...
    function on_success(transactionid, response, args) {
        ...
        var my_unsubscribe_callback = undefined;
    ...
    function on_failure() {
        sl.stopActivity("Problem loading subscribers.");
    }
    var fetch_config = {on: {success: on_success, failure: on_failure}};

    sl.startActivity("Loading subscribers...");
    // Simulate loading taking 2s.
    setTimeout(function() {
        Y.io(uri, fetch_config);
    }, 2000);
    return sl;
}
namespace.getSubscribersList = getSubscribersList;
