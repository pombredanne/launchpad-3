...
SubscribersList.prototype.loadSubscribers = function(bug_data) {
    var bug = this.lp_client.wrap_resource(bug_data.self_link, bug_data);
    var sl = this;
    var config = {
        on: {
            success: function(subscribers) {
                var index, subscriber, level, person_link;
                var subscriptions = {};
                for (index = subscribers.start;
                     index < subscribers.total_size;
                     index++) {
                    subscriber = subscribers.entries[index];
                    level = subscriber.get('bug_notification_level');
                    person_link = subscriber.get('person_link');
                    var subscribed_by_link = subscriber.get(
                        'subscribed_by_link');
                    if (subscriptions.hasOwnProperty(person_link)) {
                        var existing_level = subscriptions[person_link];
                        if (Y.Array.indexOf(subscriber_level_order,
                                            existing_level) <
                            Y.Array.indexOf(subscriber_level_order, level)) {
                            // Override the existing subscription level.
                            subscriptions[person_link] = level;
                        }
                    } else {
                        subscriptions[person_link] = level;
                    }
                ...
                for (person_link in subscriptions) {
                    if (subscriptions.hasOwnProperty(person_link)) {
                        if (!failed_loading) {
                            // Fetch the person and add a subscription.
                            function on_success(transactionid, response, args) {
                                person = Y.JSON.parse(response.responseText);
                                sl.addSubscriber(person,
                                                 subscriptions[person_link]);
                            }
                            function on_failure() {
                                failed_loading = true;
                            };
                            var config = {on: {success: on_success}};
                            Y.io(person_link, config);
                        } else {
                            break;
                        }
                    }
    ...
    bug.follow_link('subscriptions', config);
};
