/* Copyright 2011 Canonical Ltd.  This software is licensed under the
 * GNU Affero General Public License version 3 (see the file LICENSE).
 *
 * Client-side rendering of the tag list portlet.
 *
 * @module bugs
 * @submodule bugtask
 */

YUI.add('lp.bugs.bugtask.taglist', function(Y) {

var namespace = Y.namespace('lp.bugs.bugtask.taglist');
var lp_client;
3
/**
 * Setup and start the AJAX request.
 */
namespace.setup_taglist = function() {
    Y.one('#tags-portlet-spinner').setStyle('display', 'block');
    var config = {on: {success: namespace.on_success,
                       failure: namespace.hide_spinner}};
    var url = Y.one('#tags-content-link').getAttribute('href').replace('bugs.', '');
    namespace.setup_lp_client();
    lp_client.io_provider.io(url, config);
}

/**
 * Set up lp_client
 */
namespace.setup_lp_client = function(config) {
    if (!lp_client) {
        lp_client = new Y.lp.client.Launchpad();
    }
}

/**
 * Hide the loading spinner.
 */
namespace.hide_spinner = function() {
    Y.one('#tags-portlet-spinner').setStyle('display', 'none');
}

/**
 * Display the tag list and set up events for showing more/less tags.
 */
namespace.on_success = function(transactionid, response, arguments) {
    namespace.hide_spinner();
    var portlet = Y.one('#portlet-tags');
    if (Y.Lang.trim(response.responseText).length == 0) {
        portlet.setStyle('display', 'none')
    }
    else {
        portlet.prepend(response.responseText);
        var show_more_link = Y.one('#show-more-tags-link');
        var show_less_link = Y.one('#show-less-tags-link');
        var tag_list = Y.all('.tag-list li');
        if (tag_list.size() > 20) {
            var extra_tags = tag_list.slice(20);
            extra_tags.addClass('hidden');
            show_more_link.removeClass('hidden');
            show_more_link.on('click', function(e) {
                e.halt();
                extra_tags.removeClass('hidden');
                show_more_link.addClass('hidden');
                show_less_link.removeClass('hidden');
            });
            show_less_link.on('click', function(e) {
                e.halt();
                extra_tags.addClass('hidden');
                show_more_link.removeClass('hidden');
                show_less_link.addClass('hidden');
            });
        }
    }
}

}, "0.1", {"requires": ["node", 'io-base']});
