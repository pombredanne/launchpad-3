...
namespace.MarkBugDuplicate = Y.Base.create("markBugDupeWidget", Y.Widget, [], {
    initializer: function(cfg) {
        ...
        if (update_dupe_link) {
            ...
            var duplicates_div = cfg.duplicates_div;
    ...
    update_bug_duplicate_success: function(updated_entry, new_dup_url,
        ...
        var update_dup_url = this.get('update_dupe_link').get('href');
    ...
    update_bug_duplicate_failure: function(request, dup_id, old_dup_url) {
        ...
        if (request.status === 400) {
            this.duplicate_form_overlay.showError(
                dup_id + ' is not a valid bug number or nickname.');
        } else {
            this.duplicate_form_overlay.showError(request.responseText);
        }
        ...
        on: {
            ...
            failure: function(id, request) {
                that.update_bug_duplicate_failure(
                    request, new_dup_id, old_dup_url);
            }
        }
    };
    ...
    HTML_PARSER: {
        ...
        duplicates_div: '#portlet-duplicates'
    },
    ATTRS: {
