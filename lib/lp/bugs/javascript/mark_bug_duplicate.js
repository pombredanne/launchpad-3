...
namespace.MarkBugDuplicate = Y.Base.create("markBugDupeWidget", Y.Widget, [], {
        ...
        if (update_dupe_link) {
            ...
            this.duplicate_form_overlay = new Y.lazr.FormOverlay({
                ...
                form_submit_callback:
                    Y.bind(this.find_duplicate_bug, this),
                visible: false,
    ...
    _showBugSpinner: function() {
        ...
        bug_id.insert(spinner_node, 'after');
    },
     ...
     */
    _hideBugSpinner: function() {
        var spinner = this.duplicate_form_overlay.get('contentBox')
            .one('.spinner');
        if (spinner !== null) {
            spinner.remove();
        }
     ...
     * they want.
     */
    find_duplicate_bug: function(data) {
        var new_dup_id = Y.Lang.trim(data['field.duplicateof'][0]);
        if (new_dup_id === '') {
            this.update_bug_duplicate(new_dup_id);
            return;
        }
        var that = this;
        var qs_data
        ...
        var config = {
            on: {
                start: function() {
                    that._showBugSpinner();
                },
                end: function() {
                    that._hideBugSpinner();
                },
                success: function(id, response) {
                    ...
                    that.confirm_duplicate_bug(bug_data);
                },
    ...
    confirm_duplicate_bug: function(bug_data) {
     ...
     */
    update_bug_duplicate: function(new_dup_id) {
