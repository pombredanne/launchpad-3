...
def group_comments_with_activity(
    ...
    def gen_comment_windows(actor_window):
        """Generate comment windows.

        A comment window is an uninterrupted run of activity and zero or one
        comments all made by the same person.

        Accepts an iterable of `(date, actor, kind, event)` tuples and
        generates `(index, date, kind, event)` tuples, where `index` is the
        comment window.
        """
        comment_count = 0
    ...
    def gen_date_windows(comment_window):
        ...
        A date window is a group of activity and/or comments all happening
        within a given (`window`) timeframe.

        Accepts an iterable of `(indexA, date, kind, event)` tuples and
        generates `(indexB, date, kind, event)` tuples, where `indexA` is the
        comment window (not used, but generated by `gen_comment_windows`) and
        `indexB` is the date window.
        """
    ...
    def combine_date_window(date_window):
        """Put all the events in the date window together.

        If there is a comment (there *must* be only zero or one comments in a
        date window) in the window, the activity in the window is added to its
        `activity` member.

        If there is no comment in the window, the activity is returned as a
        list.

        In both cases, ordering is preserved.
        """
        comment, activities = None, []
    ...
    records = sorted(chain(comments, activity), key=itemgetter(0, 1))
