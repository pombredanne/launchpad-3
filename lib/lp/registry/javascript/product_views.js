    ...
    ns.NewProduct = Y.Base.create('new-product-view', Y.Base, [], {
        ...
        _bind_information_type: function () {
            ...
            var widget = Y.lp.app.choice.addPopupChoiceForRadioButtons(
                'information_type',
                info_type.information_type_to_choicesource(
        ...
        _find_input_field_container: function (selector) {
            var container = Y.one(selector).ancestor('td');
            return container;
        },
        ...
        _information_type_change: function (value) {
            if (info_type.information_type_value_from_key)
            var driver = this._find_input_field_container(
                'input[name="field.driver"]');
            var bug_super = this._find_input_field_container(
                'input[name="field.bug_supervisor"]');
            driver.hide();
            bug_super.hide();
        },
         ...
         */
        bindUI: function () {
            if (Y.all('input[name="field.information_type"]')) {
            ...
            // If the information type changes, handle UI updates.
            this.on('information_type:changed', this._information_type_change, this);
        },
