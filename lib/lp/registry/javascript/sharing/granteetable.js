...
Y.extend(GranteeTableWidget, Y.Widget, {
    ...
    _grantee_table_empty_row: function() {
        var row_template = [
            ...
            '</td></tr>'].join('');
        var row_text_template = '{pillar_name}\'s ' +
            'private information is not shared with anyone.';
        var row_text = Y.Lang.sub(
                row_text_template, {pillar_name: this.get('pillar_name')});
        var row = Y.Node.create(row_template);
        row.one('td').set('text', row_text);
        return row;
    },
