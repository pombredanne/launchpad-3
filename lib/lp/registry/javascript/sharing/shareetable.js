...
Y.extend(ShareeTableWidget, Y.Widget, {
    ...
    make_navigator: function() {
        ...
        var navigator = new Y.lp.registry.sharing.shareelisting_navigator.ShareeListingNavigator({
            current_url: window.location,
            cache: LP.cache,
            target: Y.one('#sharee-table'),
            sharee_table_widget: this,
    ...
    renderUI: function() {
        this._renderSharees(this.get('sharees'));
    },
    
    _renderSharees: function(sharees) {
    ...
    update_sharees: function(sharees) {
        ...
        var sharee_table = this.get('sharee_table');
        var loading_row = sharee_table.one('tr#sharee-table-loading');
        if (Y.Lang.isValue(loading_row)) {
            loading_row.remove(true);
            this._renderSharees(sharees);
            this.bindUI();
            update_node_selectors.push('tr[id^=permission-]');
        } else {
            var self = this;
    ...
    delete_sharees: function(sharees, rows_are_remaining) {
...
namespace.ShareeTableWidget = ShareeTableWidget;
    ...
    'lp.app.batchnavigator', 'lp.app.listing_navigator',
