...
YUI.add('lp.registry.distroseries.differences.test', function(Y) {
    ...
    var TestLastChangedPicker = {
        ...
        makePicker: function() {
            var picker = differences.connect_last_changed_picker(
                this.origin, this.form);
            this.addCleanup(function() {
                picker.get("boundingBox").remove(true);
            });
            return picker;
        },

        test_linkify: function() {
            var picker = this.makePicker();
            var link = this.origin.one("a");
            Assert.isInstanceOf(Y.Node, link);
            Assert.areSame("Origin", link.get("text"));
            Assert.isTrue(link.hasClass("js-action"));
        },

        test_linkify_makes_picker_visible: function() {
            var picker = this.makePicker();
            Assert.isFalse(picker.get("visible"));
            this.origin.one("a").simulate("click");
            Assert.isTrue(picker.get("visible"));
        },
