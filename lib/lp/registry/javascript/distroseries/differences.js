...
var setupPackageSetPicker = function() {
    ...
    Y.each(packagesets_headers, function(packagesets_header) {
        ...
        var overlay = new Y.lazr.PrettyOverlay({
            ...
            bodyContent: packageset_picker_table,
            visible: false
        });
        /* This will render hidden but ready for use. */
        overlay.render();
        /* Make sure the overlay's bounding box resides in the DOM the
           filter form so that the select control value is sent along
           with the form. */
        var form = Y.one("form#distroseries-localdiff-search-filter");
        form.append(overlay.get("boundingBox"));
        ...
        /* HACK HACK HACK */
        packageset_picker_table.one("tbody").append(
            Y.Node.create('<tr><td><input type="submit" /></td></tr>'));
