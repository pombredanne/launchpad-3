 ...
 */
function collapse_div(expander, accordion) {
    if (accordion != undefined && super_sized && false) {
        var size = accordion.get('boundingBox').get('scrollHeight');
        Y.log(size);
        var bb = add_subscription_overlay.get('boundingBox');
        Y.log(bb);
        add_subscription_overlay.set('height', add_subscription_overlay.get('boundingBox').get('scrollHeight') - size)
    }
 ...
 * @param expander The expander to collapse.
 */
function expand_div(expander, accordion) {
...
/**
 * show_div and hide_div are alternate methods for expand_div and collapse_div
 * for debugging purposes and should be adopted or removed.
 */
function show_div(expander, accordion) {
    expander.setStyles({
            height: expander.get('scrollHeight'),
            overflow: 'visible',
            display: 'block'
            });
    expander.addClass('lazr-opened');
    expander.set(SRC, EXPANDER_EXPANDED);
    if (accordion != undefined)
        accordion.fire('contentUpdate');
}
function hide_div(expander, accordion) {
    expander.setStyles({
            height: 0,
            overflow: 'hidden',
            display: 'block'
            });
    expander.addClass('lazr-closed');
    expander.set(SRC, EXPANDER_EXPANDED);
    if (accordion != undefined)
        accordion.fire('contentUpdate');
}
...
function setup_overlay(content_box_id) {
    ...
    advanced_filter.on('change', function(e) {
        ...
        if (checked) {
            //show_div(div, accordion);
            expand_div(div);
        }
        else
            //hide_div(div, accordion);
            collapse_div(div);
        // Notify the overlay that content has changed.
        container.fire('contentUpdate');
    });
...
function fill_in_bug_subscriptions() {
    ...
    var html =
        '<div class="yui-g">'+
        '  <div id="structural-subscriptions">'+
        '    <ul>';

    for (i in subscription_info) {
        var sub = subscription_info[i];
        html +=
            '<li style="margin: 1em 0em; padding: 1em; border: 1px solid #ddd;">'+
            '  Subscriptions via <a href="'+sub.target_url+'">'+
            sub.target_title+'</a>'+
            '  <div style="padding: 1em 0em 0em 1em">'+
            '    <dl>';

        for (j in sub.filters) {
            var filter = sub.filters[j];
            ...
            html +=
                ...
                '  You are subscribed to bugs which:'+
                '  <ul class="bulleted">'

            if (filter.statuses.length != 0) {
                html += '<li> have status ' + filter.statuses.join(', ');
            }

            if (filter.importances.length != 0) {
                html += '<li> are of importance ' + filter.statuses.join(', ');
            }
        ...
        if (subscription_info[i].filters.length == 0) {
            html += '<dt><strong>All messages</strong></dt>';
        }
        html += '</dl></div></li>';
    }
    html += '</ul></div></div>';
