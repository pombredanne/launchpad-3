...
function add_recipient_picker(content_box, hide) {
    var container = content_box.one('#bug-mail-recipient');
...
function fill_in_bug_subscriptions(config, context) {
    ...
    function get_html(node) {
        temp_node = Y.Node.create('<div></div>')
        temp_node.appendChild(node);
        return temp_node.get('innerHTML');
    }
    var listing = Y.one('#subscription-listing');
    var subscription_info = LP.cache.subscription_info;
    var subs = Y.Node.create(
    ...
    for (i=0; i<subscription_info.length; i++) {
        var sub = subscription_info[i];
        subs.appendChild(Y.Node.create(
        ...
        for (j=0; j<sub.filters.length; j++) {
            ...
            LP.cache['structural-subscription-filter-'+filter_id.toString()] =
                filter;
            filter_name = render_filter_title(sub.filters[j], filter);
//            html +=
//                '<div style="margin: 1em 0em 0em 1em"'+
//                '      id="subscription-filter-'+filter_id.toString()+'"'+
//                '      class="subscription-filter">'+
//                '  <div style="margin-top: 1em">'+
//                '    <strong class="filter-name">'+
//                get_html(filter_name)+'</strong>';
...
//                        '<a href="#" class="sprite modify edit js-action edit-subscription">'+
//                        '  Edit this subscription</a> or '+
//                        '<a href="#" class="sprite modify remove js-action delete-subscription">'+
//                        '  Unsubscribe</a></span>'));
            } else {
...
//            html += '</div>';
            ...
            description.appendChild(create_filter_description(filter));
...
//            html += '<div <strong>All messages</strong>';
        }
//        html += '</div>';
    }
//    html += '</div></div>';
//    listing.appendChild(Y.Node.create(html));
    listing.appendChild(subs);
...
function create_filter_description(filter) {
    ...
    if (filter.tags.length !== 0) {
        if (filter.find_all_tags) {
            item.appendChild(Y.Node.create('<strong>all</strong>'));
        } else {
            item.appendChild(Y.Node.create('<strong>any</strong>'));
        }
        filter_items.push(Y.Node.create('<li>are tagged with </li>')
            .appendChild(Y.Node.create('<span></span>')
                .set('text', 'of these tags: '+filter.tags.join(', '))));
    }
