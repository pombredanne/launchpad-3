    ...
    var config = {
        on: {success: function (bug_filter) {
                ...
                var on = {failure: function () {
                    // We use the namespace bindings so tests can override
                    // these functions.
...
function setup_subscription_links(overlay_id, content_box_id) {
    ...
    if (!Y.Lang.isValue(link)) {
        alert('"Subscribe to bug mail" link not found.');
    }
...
function make_selector_controls(parent) {
    ...
    rv['html'] = '<div id="'+ parent + '-selectors" style="margin-left: 10px;margin-bottom: 10px">' +
...
function setup_overlay(content_box_id, hide_recipient_picker) {
    ...
    var no_recipient_picker =
        ...
        '      <label for="'+ADDED_OR_CLOSED+'">are added or closed</label>\n'+
        '      <br>\n' +
        '      <input type="radio" name="events"\n'+
...
namespace.setup_bug_subscriptions = function(config) {
    ...
    Y.on('domready', function() {
        ...
        add_input_validator(overlay_id, 'name',
                            get_error_for_subscription_name);
        add_input_validator(overlay_id, 'tags',
                            get_error_for_tags_list);
    }, window);
};
 ...
 * @param {String} node_name Form <input> 'name' to set up a validator for.
 * @param {String} validator Function which returns 'null' if there is
      no error in the field value, and an error message otherwise.
 */
function add_input_validator(overlay_id, node_name, validator) {
    var overlay = Y.one(overlay_id);
    var input = Y.one(overlay_id + ' input[name="'+node_name+'"]');
    ...
    input.on('change', function(e) {
        ...
        if (error_text !== null) {
            ...
            //field_container.get('parentNode').setStyles({height: '0px'});
            field_container.get('parentNode').get('parentNode').setStyles({height: 'auto'});
...
function get_error_for_tags_list(value) {
    ...
    // all tags to indicate exclusion of a tag.
...
function make_edit_handler(subscription, filter_info, filter_id,
    ...
    return function(e) {
        ...
        if (add_subscription_overlay) {
            ...
            // if we want to allow editing of the recipient, do this instead
            // of the if/else block below.
            // set_recipient(
            //     content_node, subscriber_is_team, team_link);
            if (filter_info.subscriber_is_team) {
                for (ix in teams) {
                    if (teams[ix].link == filter_info.subscriber_link){
                        recipient_label.set(INNER_HTML, teams[ix].title);
                        break;
                    }
                }
            } else {
            ...
            if (is_lifecycle) {
                ...
                if (statuses.length == 0) statuses = LP.cache['statuses'];
...
function fill_in_bug_subscriptions(config, context) {
    ...
    var filter_id = 0;
    for (var i in subscription_info) {
        var sub = subscription_info[i];
        html +=
...
namespace.setup = function(config) {
    ...
    // is idempotent.  Notice that this is old MochiKit code.
    forEach(findHelpLinks(), setupHelpTrigger);
    add_input_validator(overlay_id, 'name', get_error_for_subscription_name);
    add_input_validator(overlay_id, 'tags', get_error_for_tags_list);
