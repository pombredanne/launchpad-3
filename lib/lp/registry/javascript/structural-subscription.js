...
/*
 * External entry point for rendering a bug subscription listing.
 * @method render_bug_subscriptions
...
function get_checkbox(node, value) {
...
function set_checkboxes(node, all, checked) {
    ...
    Y.each(all, function (value) {
            get_checkbox(node, value).set('checked', false);
        });
    // Check the checkboxes that are supposed to be checked.
    Y.each(checked, function (value) {
            get_checkbox(node, value).set('checked', true)
        });
}
...
function set_recipient(node, is_team) {
    get_checkbox(node, 'yourself').set('checked', !is_team);
    get_checkbox(node, 'team').set('checked', is_team);
}
...
function wire_up_edit_links(config) {
    ...
    for (i in subscription_info) {
        ...
        for (j in sub.filters) {
            ...
            var team_title = sub.filters[j].subscriber_title;
            var edit_link = Y.one('#edit-'+link_id.toString());
            handler = (function(filter, subscriber_is_team, team_title) {
                return function(e) {
                    // Only proceed if the form content is already available.
                    if (add_subscription_overlay) {
                        e.preventDefault();
                        ...
                        set_recipient(
                            content_node, subscriber_is_team, team_title);
                ...
                }})(filter, subscriber_is_team, team_title);
            edit_link.on('click', handler);
            link_id += 1;
        }
    }
}
