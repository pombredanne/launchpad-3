    ...
    var config = {
        on: {
            success: function (bug_filter) {
                    patch_bug_filter(bug_filter.getAttrs(), form_data);
                },
            failure: failure_handler
            },
        parameters: {
...
function setup_subscription_links(overlay_id, content_box_id) {
    ...
    // TODO: using #involvement is a temporary hack.
    var portlet = Y.one('#global-actions') || Y.one('#involvement');
...
function setup_overlay(content_box_id, hide_recipient_picker) {
    ...
    var no_recipient_picker =
        ...
        '            id="'+FILTER_COMMENTS+'">\n' +
        '        <label for="'+FILTER_COMMENTS+'">filter out\n'+
        '          comments</label><br>\n' +
        '        <input type="checkbox" name="filters"\n' +
    ...
    return function(e) {
        ...
        if (add_subscription_overlay) {
            ...
            set_checkboxes(
                content_node,
                LP.cache['statuses'],
                is_lifecycle ? LP.cache['statuses'] : filter.statuses);
            set_checkboxes(
                content_node,
                LP.cache['importances'],
                is_lifecycle ? LP.cache['importances'] : filter.importances);
    ...
    for (i in subscription_info) {
        ...
        for (j in sub.filters) {
            ...
            var edit_handler = make_edit_handler(
                sub, sub.filters[j], filter_id, config, context);
            ...
            var delete_handler = make_delete_handler(
                sub.filters[j].filter, filter_id, i);
...
function render_filter_name(filter_info, filter) {
    ...
    if (filter_info.subscriber_is_team) {
        return filter_info.subscriber_title+" subscription: "+description;
    } else {
