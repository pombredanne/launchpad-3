...
function patch_bug_filter(bug_filter, form_data, on) {
    ...
    if (advanced_filter) {
        ...
        if (Y.Lang.isValue(tags) && tags != '') {
            ...
            patch_data.find_all_tags = match_all;
...
function clear_overlay(content_node) {
    ...
    fast_collapse_node(Y.one('#' + ACCORDION_WRAPPER));
    fast_collapse_node(Y.one('#' + FILTER_WRAPPER));
}
...
function fast_collapse_node(node) {
    node.setStyles({visibility: 'hidden', height: 0})
    node.set(SRC, EXPANDER_COLLAPSED);
}
...
function fast_expand_node(node) {
    node.setStyles({visibility: 'visible', height: 'auto'});
    node.set(SRC, EXPANDER_EXPANDED);
}
...
function make_edit_handler(filter_info, filter_id, subscriber_is_team,
                            team_link, config, context) {
    return function(e) {
        // Only proceed if the form content is already available.
        if (add_subscription_overlay) {
            e.preventDefault();
            ...
            content_node.one('[name="name"]').set('value',
                filter_info.filter.description);
            set_checkboxes(
                content_node,
                LP.cache['statuses'],
                filter_info.filter.statuses);
            set_checkboxes(
                content_node,
                LP.cache['importances'],
                filter_info.filter.importances);
            content_node.one('[name="tags"]').set(
                'value', filter_info.filter.tags.join(' '));
            set_radio_buttons(
                content_node, [MATCH_ALL, MATCH_ANY],
                filter_info.filter.find_all_tags ? MATCH_ALL : MATCH_ANY);
            var has_advanced_filters = (
                    filter_info.filter.statuses.length ||
                    filter_info.filter.importances.length ||
                    filter_info.filter.tags.length) > 0,
                filters = has_advanced_filters ? [ADVANCED_FILTER] : [],
                event = ADDED_OR_CHANGED;
            // Chattiness: Lifecycle < Details < Discussion.
            switch (filter_info.filter.bug_notification_level) {
                case 'Lifecycle':
            ...
            var accordion_node = Y.one('#' + ACCORDION_WRAPPER),
                filter_node = Y.one('#' + FILTER_WRAPPER);
            fast_collapse_node(accordion_node); // Hide usually.
            if (event == ADDED_OR_CHANGED) {
                // Show the filters section.
                fast_expand_node(filter_node);
                if (has_advanced_filters) {
                    // Show the accordion.
                    fast_expand_node(accordion_node);
                }
            } else {
                fast_collapse_node(filter_node);
            }
    ...
    for (i in subscription_info) {
        ...
        for (j in sub.filters) {
            ...
            var edit_handler = make_edit_handler(
                sub.filters[j], filter_id, subscriber_is_team, team_link,
                config, context);
