    ...
    return function() {
        var y_config = {
            ...
            on: {success: function(transactionid, response, args){
                    ...
                    var anim = Y.lazr.anim.green_flash({node: to_collapse});
                    anim.on("end", function() {
...
function show_add_overlay(config) {
    ...
    success_callback = function(form_data, filter) {
        if (config.add_filter_description === true) {
            ...
            var subscription_info = LP.cache.subscription_info[0];
            var subscriptions_list = Y.one('#subscription-0');
            var filter_id = subscription_info.filters.length;
            var filter_data = filter.getAttrs();
            var is_team = (form_data.recipient[0] == "team");
            var url, title, link;
            if (is_team) {
                var link = form_data.team[0];
                var team = get_team(link);
                if (team !== undefined) {
                    title = team.title;
                    url = team.url;
                } else {
            ...
            var filter_info = {
                ...
                user_is_team_admin: is_team,
                subscriber_url: url,
                subscriber_link: link,
                subscriber_title: title,
                target_title: subscription_info.target_title
            };
            ...
            Y.lazr.anim.green_flash({node: description_node}).run();
        }
    }
