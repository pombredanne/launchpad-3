 ...
 */
function setup_subscription_links(content_box, overlay_name) {
    ...
    add_subscription_overlay = new Y.lazr.PrettyOverlay({
        bodyContent: Y.one(overlay_name),
        centered: true,
        visible: false
    });
    Y.log('content_box ' + content_box);
    add_subscription_overlay.render(content_box);
 ...
 */
function setup_overlay(content_box) {
    // Create the accordion to go inside an overlay.
    var content_node = Y.one(content_box);
