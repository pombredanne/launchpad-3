...
function fill_in_bug_subscriptions() {
    ...
    for (i in subscription_info) {
        ...
        for (j in sub.filters) {
            ...
            // TODO add current user vs. team variation.
            html +=
                '<dt><strong>'+
            ...
            if (filter.importances.length != 0) {
                html += '<li> are of importance ' +
                    filter.importances.join(', ');
            }

            // Format tag conditions.
            if (filter.tags.length != 0) {
                html += '<li> is tagged with ';
                if (filter.find_all_tags) {
                    html += '<strong>all</strong>';
                } else {
                    html += '<strong>any</strong>';
                }
                html += ' of these tags: ' + filter.tags.join(', ');
            }
