...
Y.extend(MessageList, Y.Base, {
    ...
    _bind_nav: function () {
        ...
        forwards.on('click', function(e) {
            this.fire('messageList:forwards', e); 
        })

        backwards.on('click', function(e) {
            this.fire('messageList:backwards', e); 
        })
    },
    ...
    display_messages: function () {
        ...
        this._create_mustache_model(messages, mustache_model, processed_ids, 0)
        var content = Y.lp.mustache.to_html(LP.mustache_listings, {items: mustache_model});        
            ...
            if (processed_ids.indexOf(message.message_id) === -1) {
                ...
                if (message.nested_messages !== undefined) {
                    ...
                    filter_func = function(item) {
                        var nested_ids = message.nested_messages;
