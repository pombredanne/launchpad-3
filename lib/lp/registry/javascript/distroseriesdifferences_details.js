...
var set_package_diff_status = function(container, new_status) {
...
*
*/
namespace.get_selected_state = function(json_voc) {
...
* diff information.
*
* @api_uri {string} The uri for the distroseriesdifference object.
*/
namespace.setup_pending_package_diff = function(container, api_uri) {
    ...
    var build_package_diff_update_config = {
        ...
        */
        domUpdateFunction: function(container, data_object) {
            var state = namespace.get_selected_state(data_object);
            if (state === 'FAILED') {
                set_package_diff_status(container, 'FAILED');
            }
...
* @api_uri {string} The uri for the distroseriesdifference object.
*/
namespace.setup_request_derived_diff = function(container, api_uri) {
    // Setup handler for diff requests.
    container.one('.package-diff-button').append([
        '<button class="package-diff-button" ',
        'type="button">Compute diff</button>'].join(''));
    container.one('button').on('click', function(e) {
        compute_package_diff(container, api_uri);
    });
};
...
var compute_package_diff = function(container, api_uri) {
    ...
    var failure_cb = function(transaction_id, response, args) {
        container.one('p.update-in-progress-message').remove();
        var failure_message = Y.lp.soyuz.base.makeFailureNode(
            'Failed to compute package diff.');
    ...
    lp_client.named_post(api_uri, 'requestPackageDiffs', config);
};
