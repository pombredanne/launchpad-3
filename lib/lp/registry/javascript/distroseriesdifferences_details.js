...
namespace.setup_expandable_rows = function() {
    ...
    var add_comment_handler = function(placeholder, api_uri) {
        ...
        var success_handler = function(comment_entry) {
            ...
            config = {
                on: {
                    success: function(comment_html) {
                        ...
                        placeholder.insert(comment_node, 'after');
            ...
            placeholder.one('img').remove()
            Y.Event.simulate(
                Y.Node.getDOMNode(
                    placeholder.one('a.widget-hd')), 'click');
            placeholder.all('textarea,button').set(
                'disabled', '');
            placeholder.one('textarea').set('value', '');
        };
