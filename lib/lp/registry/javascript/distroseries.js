...
/**
 * A widget to encapsulate functionality around submitting the page.
 *
 * @class FormSubmissionWidget
 */
var FormSubmissionWidget = function() {
    PackagesetPickerWidget
        .superclass.constructor.apply(this, arguments);
};

Y.mix(FormSubmissionWidget, {

    NAME: 'formSubmissionWidget',
...
Y.extend(FormSubmissionWidget, Y.Widget, {
    ...
    /**
     * Display a message on success and fade out the form.
     *
     * @method showSuccess
     */
    showSuccess: function() {
        var messageNode = Y.Node.create(
            '<p class="informational message" />');
        messageNode.set(
            "text", [
                "The derivation of ", LP.cache.context.displayname,
                " has been scheduled. It should run shortly. Please ",
                "watch the XXX page to check on progress."].join(""));
        this.get("contentBox").ancestor("form")
            .insert(messageNode, "after")
            .transition({duration: 1, height: 0, opacity: 0});
    },
    ...
    submit: function(parameters) {
        ...
        var config = {
            on: {
                ...
                success: Y.bind(this.showSuccess, this),
                failure: this.error_handler.getFailureHandler(),
                end: Y.bind(this.hideSpinner, this)
            },
            parameters: {
                name: LP.cache.context.name,
                distribution: LP.cache.context.distribution_link,
                architectures: parameters.architectures,
                packagesets: parameters.packagesets,
                rebuild: parameters.rebuild
            }
        };
        this.client.named_post(
            parameters.parent, "deriveDistroSeries", config);
    }

});

namespace.FormSubmissionWidget = FormSubmissionWidget;
...
namespace.setup = function() {
    ...
    var package_copy_options = new ChoiceListWidget()
        ...
        .render(form_table_body);
    var form_submission = new FormSubmissionWidget(
    ...
    form_container.one("form").on(
        "submit", function(event) {
            event.halt();
            var parameters = {
                parent: derive_from_choice.get("value"),
                architectures: architecture_choice.get("choice"),
                packagesets: packageset_choice.get("choice"),
                rebuild: (
                    package_copy_options.get("choice") ==
                        "Copy Source and Rebuild")
            };
            form_submission.submit(parameters);
        }
    );
