...
Y.extend(ParentSeriesListWidget, FormRowWidget, {
    ...
    move_down: function(fieldNode, parent_id) {
        var node = fieldNode.one("tr#parent-" + parent_id);
        var other = node.next('tr.parent');
        if (other != null) { node.swap(other);}
        Y.lazr.anim.green_flash({node: node}).run();
    },

    move_up: function(fieldNode, parent_id) {
        var node = fieldNode.one("tr#parent-" + parent_id);
    ...
    add_parent: function(parent) {
        ...
        item.one('.move-up').on('click', function(e) {
            this.move_up(this.fieldNode, parent.value);
            e.preventDefault();
            return false;
        }, this);
        item.one('.move-down').on('click', function(e) {
            this.move_down(this.fieldNode, parent.value);
            e.preventDefault();
            return false;
        }, this);
    ...
    add_distroseries: function(distroseries) {
        ...
        var on = {
            success: function (results) {
                widget._distroseries[distroseries_id] = results;
