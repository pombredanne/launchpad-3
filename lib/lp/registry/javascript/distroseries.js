...
Y.extend(ParentSeriesListWidget, FormRowWidget, {
    ...
    move_down: function(fieldNode, value) {
        var node = fieldNode.one("#" + value.replace(/\//gi,'-'));
        var other = node.next('tr.parent');
        if (other != null) { node.swap(other);}
        Y.lazr.anim.green_flash({node: node}).run();
    },

    move_up: function(fieldNode, value) {
        var node = fieldNode.one("#" + value.replace('/','-'));
        var other = node.previous('tr.parent');
        if (other != null) { node.swap(other);}
        Y.lazr.anim.green_flash({node: node}).run();
    },

    add_parent: function(value) {
        if (this.get('parentSeries').length == 0) {
            this.init_display();
            this.renderUI();
        }
        var item =  Y.Node.create("<tr />")
            .addClass('parent')
            .set('id', value.api_uri.replace('/','-'))
        ...
        this.build_select(item, 'component',
            value.api_uri + '/component_names');
        this.build_select(item, 'pocket',
            value.api_uri + '/suite_names');
        var this_closure = this;
        item.one('.move-up').on('click', function(e) {
            this_closure.move_up(this_closure.fieldNode, value.api_uri);
            e.preventDefault();
            return false;
        });
        item.one('.move-down').on('click', function(e) {
            this_closure.move_down(this_closure.fieldNode, value.api_uri);
            e.preventDefault();
            return false;
        });
        item.one('.remove').on('click', function(e) {
            if (this_closure.get('parentSeries').length == 1) {
                this_closure.clean_display();
            }
            else {
                item.remove();
            }
            if (this_closure.get('parentSeries').length == 0) {
                this_closure.clean_display();
                this_closure.renderUI();
            }
            var parent = item.get('id').replace('-','/');
            Y.fire("parent_removed", parent);
            e.preventDefault();
            return false;
        });
    ...
    ATTRS: {
        ...
        choices: {
            ...
            setter: function(value, name) {
                ...
                choices.forEach(
                    function(choice) {
                       var item = widget._createChoice(choice);
                       list.append(item);
                    }
                );
...
Y.extend(ChoiceListWidget, FormRowWidget, {
     ...
     */
     add_choices: function(new_choices) {
        ...
        new_choices.forEach(
            function(choice) {
                if (widget.fieldNode.all(
                    "li > input[value=" + choice + "]").isEmpty()) {
                    var list = widget.fieldNode.one('ul');
                    if (list == null) {
                        var list = Y.Node.create("<ul />");
                        widget.fieldNode.empty().append(list);
                    }
                    list.append(widget._createChoice(choice));
                }
            }
        );
        Y.lazr.anim.green_flash({node: this.fieldNode}).run();
...
Y.extend(ArchitecturesChoiceListWidget, ChoiceListWidget, {
    ...
    add_distroseries: function(distroseries) {
        ...
        var on = {
            success: function (results) {
                widget._distroseries[distroseries] = results;
                widget.add_das(results);
            },
            failure: this.error_handler.getFailureHandler(),
        };
        this.client.get(path, {on: on});
    ...
    remove_distroseries: function(distroseries) {
        // Compute which das is only in the distroseries to be removed.
        arch_to_remove = []
        var das = this._distroseries[distroseries];
        for (i=0; i<das.entries.length; i++) {
            ...
            for (var ds in this._distroseries) {
                if (ds !== distroseries) {
                   ...
                   for (j=0; j<other_das.entries.length; j++) {
                       if (other_das.entries[j].get('architecture_tag') == das.entries[i].get('architecture_tag')) {
                           remove_das = false;
                       }
                   }
                }
            ...
            if (remove_das) {
                arch_to_remove.push(das.entries[i].get('architecture_tag'));
            }
        }
        delete this._distroseries[distroseries];
        this.remove_choices(arch_to_remove);
    },

    add_das: function(distroarchseries) {
...
Y.extend(PackagesetPickerWidget, SelectWidget, {
    ...
    add_distroseries: function(distroseries) {
        ...
        var on = {
            success: function (results) {
                widget.add_choices(results, distroseries);
            },
            failure: this.error_handler.getFailureHandler(),
        };
        var config = {
    ...
    add_choices: function(packagesets, distroseries) {
        packagesets.entries.forEach(
            function(packageset) {
                this.add_choice({
                    data: packageset,
                    value: packageset.get("name")+distroseries.api_uri,
                    text: (
                        packageset.get("name") + ": " +
                        packageset.get("description") +
                        " (" + distroseries.title + ") ")
    ...
    remove_distroseries: function(distroseries) {
        this.fieldNode.all('option[name$=' + distroseries + ']').remove();
    },
...
namespace.setup = function() {
    ...
    Y.on('add_parent', function(parent) {
        parents_selection.add_parent(parent);
        Y.fire("parent_added", parent);
    });

    Y.on('parent_added', function(parent) {
        architecture_choice.add_distroseries(parent.api_uri);
        packageset_choice.add_distroseries(parent);
    });

    Y.on('parent_removed', function(parent) {
        architecture_choice.remove_distroseries(parent);
        packageset_choice.remove_distroseries(parent);
    });
