...
Y.extend(ParentSeriesListWidget, FormRowWidget, {
    ...
    move_down: function(fieldNode, value) {
        var node = fieldNode.one("#" + uri_to_id(value));
    ...
    add_parent: function(value) {
        ...
        var item =  Y.Node.create("<tr />")
            .addClass('parent')
            .set('id', uri_to_id(value.api_uri))
        ...
        item.one('.move-up').on('click', function(e) {
            this.move_up(this.fieldNode, value.api_uri);
            e.preventDefault();
            return false;
        }, this);
        item.one('.move-down').on('click', function(e) {
            this.move_down(this.fieldNode, value.api_uri);
            e.preventDefault();
            return false;
        }, this);
        item.one('.remove').on('click', function(e) {
            ...
            var parent = id_to_uri(item.get('id'));
...
Y.extend(ArchitecturesChoiceListWidget, ChoiceListWidget, {
    ...
    add_distroseries: function(distroseries) {
        ...
        var on = {
            success: function (results) {
                widget._distroseries[uri_to_id(distroseries)] = results;
    ...
    remove_distroseries: function(distroseries) {
        ...
        delete this._distroseries[uri_to_id(distroseries)];
...
/**
 * Helper method to convert an uri to a valid html id
 * (to be used by CSS selectors).
 *
 */
var uri_to_id = function(uri) {
    return uri.replace(/\//g, '-');
};

/**
 * Helper method to convert an id back to the uri it was
 * calculated from.
 *
 */
var id_to_uri = function(id) {
    return id.replace(/-/g, '/');
};
...
Y.extend(PackagesetPickerWidget, SelectWidget, {
    ...
    add_choices: function(packagesets, distroseries) {
        packagesets.entries.forEach(
            function(packageset) {
                var value = uri_to_id(packageset.get("name")+distroseries.api_uri);
                this.add_choice({
                    data: packageset,
                    value: value,
                    text: (
    ...
    remove_distroseries: function(distroseries) {
        this.fieldNode.all(
            'option[value$=' + uri_to_id(distroseries) + ']').remove();
        Y.lazr.anim.green_flash({node: this.fieldNode}).run();
    },
