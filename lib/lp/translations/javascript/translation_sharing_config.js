...
Y.extend(TranslationSharingController, Y.Base, {
    ...
    select_branch: function(branch_summary){
        ...
        var save_config = get_config(Y.bind(
            get_branch, this, get_branch_config));
        var get_productseries_config = get_config(Y.bind(
            save_branch, this, save_config))
        var lp_client = new Y.lp.client.Launchpad();
        lp_client.get(
            this.get('productseries_link'), get_productseries_config);
    },
    update: function(){
