...
Y.extend(TranslationSharingConfig, Y.Base, {
    initializer: function(){
        ...
        var usage = new CheckItem({dependency: product_series});
        this.set('translation_usage', usage);
        var branch = new LinkCheckItem({dependency: this.get('product_series')})
        this.set('branch', branch);
        var autoimport = new CheckItem({dependency: branch})
