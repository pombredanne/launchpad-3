...
namespace.prepare = function(cache) {
    ...
    var usage_overlay = create_form_overlay(
        ...
        function load_model(config) {
            var source_package = sharing_controller.get('source_package');
            Y.lp.client.load_cache(
                source_package, '+sharing-details', config);
        }
        function refresh_from_model(cache) {
            sharing_controller.replace_productseries(cache.productseries);
            sharing_controller.replace_product(cache.product);
        ...
        var config = io_handler.chain_config(load_model, refresh_from_model);
        submit_form(
