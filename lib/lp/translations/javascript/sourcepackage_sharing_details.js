...
function create_remove_productseries_overlay(title_html, submit_callback){
    var submit_button = Y.Node.create(
            '<button type="submit" name="field.actions.create" ' +
            'value="Unlink">Unlink</button>'),
        cancel_button = Y.Node.create(
            '<button type="button" name="field.actions.cancel" ' +
            'value="Cancel">Cancel</button>'),
        content = Y.Node.create(
            '<span>Do you want to remove the upstream link?</span>');
    function hide_and_callback(){
        form_overlay.hide();
        submit_callback.apply(this, arguments);
    }
...
/*        function get_productseries(config) {
        ...
        var io_handler = new IOHandler(this, productseries_check);
        delete_packaging(io_handler.chain_config(set_usage));
    },
...
namespace.prepare = function(cache) {
    ...
    var remove_productseries_overlay = create_remove_productseries_overlay(
