...
namespace.prepare = function(cache) {
    ...
    var remove_productseries_overlay = create_form_overlay(
            '<h2>Unlink an upstream project<h2>', function(form_data) {
            Y.log('Unlinking.');
            sharing_controller.remove_productseries();
        });
    remove_productseries_overlay.set(
        'form_content',
        Y.Node.create('<p>This will remove the link to the upstream project series.</p>'));
