...
namespace.convert_cache = function(lp_client, cache){
    for (key in cache){
        cache[key] = new Y.lp.client.Entry(
            lp_client, cache[key], cache[key].self_link)
    }
}
...
Y.extend(TranslationSharingConfig, Y.Base, {
    ...
    configure: function(config){
        var productseries = config['productseries']
        this.get('product_series').set_link(
            productseries.get('title'), productseries.get('web_link'))
        var lp_client = new Y.lp.client.Launchpad();
        this.set_branch_link(config['upstream_branch']);
    },
    set_branch_link: function(branch){
        this.get('branch').set_link(
...
Y.extend(TranslationSharingController, Y.Base, {
    ...
    select_branch: function(branch_summary){
        ...
        function set_link(branch){
            that.get('tsconfig').set_branch_link(branch)
            that.update();
        }
