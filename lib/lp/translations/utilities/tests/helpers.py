...
def import_pofile_or_potemplate(file_contents, person,
    pofile=None, potemplate=None, from_maintainer=True):
    ...
    :param potemplate: if uploading a `POTemplate`, file to import to; None
        otherwise.
    :param from_maintainer: true if this file was uplaoded by the maintainer
    ...
    if pofile is not None:
        if pofile.potemplate.distroseries is None:
            entry = translation_import_queue.addOrUpdateEntry(
                pofile.path, file_contents, from_maintainer, person,
        ...
        # from_maintainer flag makes no difference.
