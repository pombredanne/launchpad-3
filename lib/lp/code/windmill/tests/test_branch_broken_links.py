...
class TestBranchBugLinks(WindmillTestCase):
    ...
    def test_invalid_url_rendering(self):
        ...
        valid_links = ['lp:firefox/trunk', 'lp:gnome-terminal/trunk']
        ...
        code = """
            var bad_a = document.getElementsByClassName('invalid-link', 'a');
            var bad_links = [];
            for( i=0; i<bad_a.length; i++ ) {
                result.push(bad_a[i].innerHTML);
            }

            var good_a = document.getElementsByClassName('branch-short-link', 'a');
            var good_links = [];
            for( i=0; i<good_a.length; i++ ) {
                result.push(good_a[i].innerHTML);
            }
