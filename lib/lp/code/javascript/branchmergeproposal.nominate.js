...
var setup_nominate_submit = function(io_provider) {
        ...
        on: {
            ...
            success:
                function(id, response) {
                    var visibility_info = Y.JSON.parse(response.responseText);
                    confirm_reviewer_nomination(visibility_info);
                }
        }
    };
    var form_data = {};
    ...
    form.all("[name^='field.']").each(function(field) {
        form_data[field.getAttribute('name')] = field.getAttribute('value');
    });
    form_data.id = form;
