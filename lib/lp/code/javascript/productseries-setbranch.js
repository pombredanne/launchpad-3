...
YUI.add('lp.code.productseries_setbranch', function(Y) {
    ...
    module.onclick_default_vcs = function(e) {
        ...
        var selectedDefaultVCS = Y.one("#test input[name=test1]:checked").get("value");
    };
    ...
    module.onclick_branch_type = function(e) {
     ...
     module.renderTabs = function() {
         var tabview = new Y.TabView({
             srcNode: '#push-instructions-tabview'
         });
         if (Y.one('#product-push-instructions')) {
             Y.one('.push-instructions-tabs').setStyle('display', 'block');
             tabview.render();
         }
     };
