...
YUI.add('lp.code.productseries_setbranch', function(Y) {
    ...
    module.onclick_default_vcs = function(e) {
        ...
        var git_form = Y.one('#form_git');
        var git_inputs = git_form.all('input');
        var bzr_form = Y.one('#form_bzr');
        var bzr_inputs = bzr_form.all('input');
        var branch_type = Y.one('[name="field.branch_type"]');
        if (selectedDefaultVCS === 'GIT') {
            git_form.show();
            git_inputs.set('disabled', false);
            bzr_form.hide();
            bzr_inputs.set('disabled', true);
        } else {
            bzr_form.show();
            bzr_inputs.set('disabled', false);
            git_form.hide(); 
            git_inputs.set('disabled', true);
        }
        module.renderPushInstructions();
    };
    ...
    module.onclick_branch_type = function(e) {
     ...
     module.renderPushInstructions = function() {
         if (Y.one('#product-push-instructions')) {
             var selectedVCS = module._get_selected_default_vcs();
             var git_instructions = Y.one('div#push-instructions-git');
             var bzr_instructions = Y.one('div#push-instructions-bzr');
             if (selectedVCS === 'GIT') {
                 git_instructions.show();
                 bzr_instructions.hide();
             } else {
                 bzr_instructions.show();
                 git_instructions.hide();
             }
         }
