...
Y.extend(DiffNav, Y.Widget, {
    ...
    _connectNavigator: function(navigator) {
        ...
        navigator.on('change', function() {
            var diff_id = Y.one('#available_diffs_choice').get('value');
    ...
    renderUI: function() {
        ...
        var config = {
		    ...
		    var node = Y.Node.create(content);
		    self.get('srcNode').insert(node, 2);
                    Y.lp.anim.green_flash({node: node}).run();
                    self._connectNavigator(node);
                },
                failure: function() {
		    console.log(content);
                    var node = Y.Node.create('<h4>Failed</h4>');
		    self.get('srcNode').insert(node, 2);
