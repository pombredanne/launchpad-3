    ...
    handler = function(e) {
        ...
        widget.editor.on('save', function() {
            ...
            if (this.get('value') === '') {
                ...
                var comments = draft_div.get('parentNode');
                draft_div.remove(true);
                if (!comments.hasChildNodes()) {
                    comments.ancestor('tr').remove(true);
                }
            }
            var config = {
                parameters: {
...
namespace.create_row = function(comment) {
    ...
    var comments_div = Y.one('#' + line_class + '-comments');
    if (comments_div === null) {
        var comments_row = Y.Node.create(
            '<tr class="inline-comments"><td class="line-no"></td>'
            + '<td><div id="' + line_class + '-comments"></div></td></tr>');
        Y.one('#' + line_class).insert(comments_row, 'after');
        comments_div = comments_row.one('div');
    }
    ...
    if (Y.Lang.isUndefined(comment.person)) {
        ...
        newrow.set('id', 'ict-' + ident).addClass('draft');
