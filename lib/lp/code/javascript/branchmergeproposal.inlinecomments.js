    ...
    handler = function(e) {
        ...
        widget.editor.on('save', function() {
            ...
            if (this.get('value') === '') {
                ...
                var comment_table = header_row.get('parentNode');
                header_row.remove(true);
                content_row.remove(true);
                if (!comment_table.hasChildNodes()) {
                    comment_table.ancestor('tr', false).remove(true);
                }
            }
            var config = {
                parameters: {
...
namespace.create_row = function(comment) {
    ...
    var line_class = 'diff-line-' + parseInt(comment.line_number, 10);
    var comments_row = Y.one('#' + line_class + '-comments');
    if (comments_row === null) {
        comments_row = Y.Node.create(
            '<tr><td class="line-no"></td>' +
            '<td id="' + line_class + '-comments"><table></table></td></tr>');
        Y.one('#' + line_class).insert(comments_row, 'after');
    }
    var comments_table = comments_row.one('table');
    ...
    if (Y.Lang.isUndefined(comment.person)) {
        ...
        headerrow.addClass('draft-header');
    ...
    // XXX: Quick hack to ensure that drafts end up at the end.
    var maybe_draft = comments_table.one('.draft-header');
    if (comment.person !== undefined && maybe_draft !== null) {
        maybe_draft.insert(headerrow, 'before');
        maybe_draft.insert(newrow, 'before');
    } else {
        comments_table.appendChild(headerrow);
        comments_table.appendChild(newrow);
    }
    return headerrow;
};
