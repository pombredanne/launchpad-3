...
namespace.headers = document.querySelectorAll('table.diff td.diff-file, table.diff td.diff-chunk');
namespace.scrollPos = -1;

namespace.getHeaderRow = function(headers, pos) {
    // Get parent row for scrollIntoView
    var id = headers[pos].parentNode.id;
    return document.getElementById(id);
};

namespace.updateScrollState = function(headers, pos) {
    namespace.scrollPos = pos;
    return namespace.getHeaderRow(headers, pos);
};
...
namespace.getNextHeader = function(headers, type, direction) {
    ...
    if (direction == 'forwards') {
        for (i=cursor + 1; i < headers.length; i++) {
            if (headers[i].className.indexOf(type) > -1) {
                return namespace.updateScrollState(headers, i);
            }
        }
    } else {
        if (cursor == 1) { cursor = headers.length;}
...
namespace.scrollToHeader = function(type, direction) {
    var header = namespace.getNextHeader(namespace.headers, type, direction);
