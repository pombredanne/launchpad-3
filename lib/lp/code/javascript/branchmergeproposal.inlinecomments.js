...
Y.extend(DiffNav, Y.Widget, {
    ...
    _connectScrollers: function() {
        ...
        Y.all('td[data-previewdiff-id]').each( function(td) {
            ...
            var scroller_id = 'ic-scroller-' + previewdiff_id;
            if (Y.one('#' + scroller_id)) {
                return;
            }
        ...
        if (Y.one('input[name="field.publish_inline_comments"]') === null) {
            var publish_label = Y.Node.create('<a />')
                .set('text', 'Publish inline comments: ');
            var publish_checkbox = Y.Node.create('<input />')
                .set('id', 'field.publish_inline_comments')
                .set('name', 'field.publish_inline_comments')
                .set('type', 'checkbox')
                .addClass('checkboxType');
            var src_node = Y.one('input[name="field.review_type"]')
            src_node.insert(publish_label, 'after');
            publish_label.insert(publish_checkbox, 'after');
        }
    },
