...
namespace.connect_requestdailybuild = function() {
    ...
    var request_daily_build_form = Y.one('#request-daily-build-form');

    // First we need to unhide the ajax link and hide and html button
    request_daily_build_handle.setStyle("display", "block");
    request_daily_build_form.setStyle("display", "none");

    if( request_daily_build_handle == null )
        return;
    request_daily_build_handle.addClass('js-action');
    request_daily_build_handle.on('click', function(e) {
        ...
        var y_config = {
            ...
            on: {
                ...
                success: request_daily_build_response_handler.getSuccessHandler(
                    function(handler, id, response) {
...
function validate(data) {
    ...
    if( Y.Object.size(distros) == 0 ) {
        request_build_response_handler.showError("You need to specify at least one " +
