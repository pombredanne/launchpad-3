...
var distro_node_html;

function get_distro_nodes() {
...
function ppa_changed(ppa_value, submit_button) {
    var distro_nodes = get_distro_nodes();
    for (var i=0; i<distro_nodes.size(); i++)
        distro_nodes.item(i).set("innerHTML", distro_node_html[i]);
    ...
    Y.Array.each(last_known_pending_builds, function(distroarchive) {
        ...
        for (var i=0; i<distro_nodes.size(); i++) {
            var distro_node = distro_nodes.item(i);
            var distro = distro_node.get("text").trim();
            if (distro == distroarchive[0] ) {
                nr_matches += 1;
                distro_node.set("innerHTML", distro + " (build pending)");
                break;
            }
        }
    });
    // If no distro series can be selected, no need to show the submit btn.
    if (nr_matches>0 && nr_matches == distro_nodes.size())
        submit_button.hide();
    else
        submit_button.show();
}
            ...
            success:
                function(id, response) {
                    ...
                    distro_nodes.each(function(distro_node) {
                        distro_node_html.push(distro_node.get("innerHTML"));
                    });
