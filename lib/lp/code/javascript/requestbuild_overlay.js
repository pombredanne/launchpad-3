        ...
        on: {
            success:
                function(id, response) {
                    ...
                    var nr_builds = 0;
                    var nr_series = 0;
                    builds.all('tr.package-build :nth-child(3)')
                            .each(function(build_row) {
                        nr_builds += 1;
                        nr_series = 0;
                        var series = build_row.get("text").trim();
                        request_build_overlay.form_node.all(
                                "label[for^='field.distros.']")
                                .each(function(distro) {
                            nr_series += 1;
                            if (distro.get("text").trim() == series) {
                                distro.set("text", series + " (build pending)");
                            }
                        });
                    });
                    if (nr_builds>0 && nr_builds == nr_series)
                        submit_button.hide();
                }
        }
    };
