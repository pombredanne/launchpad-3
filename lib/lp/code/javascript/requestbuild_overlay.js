...
RequestBuildResponseHandler = function () {};
RequestBuildResponseHandler.prototype = {
    ...
    getErrorHandler: function (errorCallback) {
        ...
        return function (id, response) {
            ...
            if(response.status == 503) {
                ...
                var page_content = response.responseText;
                if( errorCallback != null ) {
                    page_content = errorCallback(self, id, response);
                }
...
namespace.connect_requestbuild = function() {
    ...
    request_build_overlay = new Y.lazr.FormOverlay({
        headerContent: '<h2>Request builds for ' + recipe_name + ' </h2>',
    ...
    response_handler = new RequestBuildResponseHandler();
...
function do_request_builds(data) {
    ...
    var uri = '+builds'; //uri is used later
    var submit_url = base_url+"/"+uri;
    var query_str = '';
    query_str = LP.client.append_qs(query_str, 'field.archive', data['field.archive']);
    query_str = LP.client.append_qs(query_str, 'field.distros', Y.JSON.stringify(data['field.distros']));
