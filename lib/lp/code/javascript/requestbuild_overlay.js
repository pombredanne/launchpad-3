...
namespace.connect_requestbuilds = function(config) {
    ...
    var io_provider = Y.testing.mockio.io_provider_config(config);
    ...
    request_build_handle.on('click', function(e) {
        ...
        request_build_overlay.loadFormContentAndRender(
            '+builds/++form++', io_provider);
 ...
 */
function do_request_builds(data, io_provider) {
    if (!validate(data))
        return;
