...
namespace.connect_requestdailybuild = function() {
    ...
    var display_message = function (message, restore_handle){
        var build_message_node = Y.Node.create('<div></div>')
            .set('id', 'new-builds-info')
            .addClass('build-informational')
            .set('text', message)
            ...
            on: {
                ...
                success:
                    ...
                    function(handler, id, response) {
                        ...
                        display_message(new_builds_message);
                    }
                  )
            },
            data: qs
    ...
    request_daily_build_response_handler.showError = function(header, error) {
        ...
        if (error != null)
            error_msg += " " + error;
        display_message(error_msg, true);
        Y.log(error_msg);
    };
};
