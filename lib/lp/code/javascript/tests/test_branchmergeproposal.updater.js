...
suite.add(new Y.Test.Case({
    ...
    setUp: function() {
        ...
        LP.cache.context = {
            web_link: "https://code.launchpad.net/~foo/bar/foobr/+merge/123"};
    ...
    test__setup_diff_container: function() {
        ...
        Y.Assert.areEqual(
            ['<div id="review-diff"><h2>Preview Diff</h2>',
             '<div class="diff-content"></div></div>'].join(''),
            this.updater.get('srcNode').get('innerHTML'));
    },
    ...
    test_update_success: function() {
        ...
        module.lp_client.io_provider = mockio;
        Y.Assert.areEqual(
            '',
            this.updater.get('diff'));
        this.updater.update();
