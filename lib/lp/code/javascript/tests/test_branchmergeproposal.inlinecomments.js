...
YUI.add('lp.code.branchmergeproposal.inlinecomments.test', function (Y) {
    ...
    tests.suite.add(new Y.Test.Case({
	    ...
	    // But no inline_comments are set up.
        ...
        test_status_indicator: function() {
            var diff_nav = new module.DiffNav(
                {srcNode: Y.one('#review-diff')});
            // Indicating progress with the spinner image.
            diff_nav.set_status_updating();
            Y.Assert.areEqual(
                '/@@/spinner',
                Y.one('h2').one('img').getAttribute('src'));
            // Remove the spinner when the work is done.
            diff_nav.cleanup_status();
            Y.Assert.areEqual(
                'Preview Diff',
