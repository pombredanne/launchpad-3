...
YUI.add('lp.code.branchmergeproposal.inlinecomments.test', function (Y) {
    ...
    tests.suite.add(new Y.Test.Case({
        ...
        test_diff_nav_publish_scroller: function() {
            ...
            Y.Assert.areEqual('[id="field.comment"]', document.activeElement.id);
        },
        
        test_diff_nav_publish_scroller_with_diff: function() {
            // The Diff Navigator publish comment *scroller* is connected
            // upon widget rendering and when clicked triggers the browser
            // to scroll to the comment textbox.
            this.diffnav.render();
            ...
            Y.Assert.areEqual(Y.one('field.comment'), active);
            // Cause a inline comment trigger event
            Y.fire('inlinecomment.UPDATED');
            scroller.on('click', function() {
                var rc = Y.lp.code.branchmergeproposal.reviewcomment;
                rc.window_scroll_anim.fire('end');
            });
            // Now field.actions.add should be the focus element.
            var active = document.getElementById(document.activeElement);
            Y.Assert.areEqual('field.actions.add', document.activeElement.id);
        },
