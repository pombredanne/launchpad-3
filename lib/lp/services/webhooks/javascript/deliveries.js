...
Y.extend(WebhookDeliveries, Y.Widget, {
    ...
    initializer: function(config) {
        ...
        navigator.subscribe(
            namespace.WebhookDeliveriesListingNavigator.UPDATE_CONTENT,
            function(e) {
                LP.cache.deliveries = e.details[0];
                self.syncUI();
            });
    ...
    syncUI: function() {
        ...
        Y.Array.each(LP.cache.deliveries, function(delivery) {
            ...
            // load, but after batch navigation they are. Ensure we're
            // always dealing with wrapped ones.
            if (Y.Lang.isUndefined(delivery.get)) {
                delivery = self.lp_client.wrap_resource(null, delivery);
            }
