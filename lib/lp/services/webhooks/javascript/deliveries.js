...
Y.extend(WebhookDeliveries, Y.Widget, {
    ...
    initializer: function(config) {
        ...
        navigator.subscribe(
            namespace.WebhookDeliveriesListingNavigator.UPDATE_CONTENT,
            function(e) {
                self._set_deliveries(e.details[0]);
                self.syncUI();
            });
        this.navigator = navigator;
    },

    _set_deliveries: function(resources) {
        var self = this;
        Y.Array.each(resources, function(resource) {
            if (!Y.Object.owns(self.deliveries, resource.get("self_link"))) {
                self.deliveries[resource.get("self_link")] = {
                    resource: null, expanded: false, requesting_retry: false};
            }
            self.deliveries[resource.get("self_link")].resource = resource;
        });
        this.deliveries_displayed = Y.Array.map(
            resources, function(resource) {return resource.get("self_link")});
    },
    ...
    syncUI: function() {
        ...
        Y.Array.each(this.deliveries_displayed, function(delivery_url) {
            var delivery = self.deliveries[delivery_url];
            ...
            if (delivery.expanded) {
                ...
                if (retrying_now) {
                    ...
                    detail_node.one(".delivery-redeliver").addClass("hidden");
                    ...
                    detail_node.one(".delivery-redeliver").set("text", "Retry now");
                    detail_node.one(".delivery-retry-notice").removeClass("hidden");
                    detail_node.one(".delivery-redeliver").removeClass("hidden");
                    detail_node.one(".delivery-delivering-notice").addClass("hidden");
                } else {
                    var redeliver_text = resource.successful ? "Redeliver" : "Retry";
                    detail_node.one(".delivery-redeliver").set("text", redeliver_text);
                    detail_node.one(".delivery-retry-notice").addClass("hidden");
                    detail_node.one(".delivery-delivering-notice").addClass("hidden");
                    detail_node.one(".delivery-redeliver").removeClass("hidden");
                }
