...
Y.extend(WebhookDeliveries, Y.Widget, {
    ...
    _render_detail: function(delivery, delivery_node) {
        ...
        var relative_date_sent = "at an unknown time";
        if (delivery.resource.get("date_sent")) {
            relative_date_sent = Y.lp.app.date.approximatedate(
                Y.lp.app.date.parse_date(delivery.resource.get("date_sent")));
        }
        var status_text = null;
        if (delivery.resource.get("successful") === true) {
            status_text = "Delivered " + relative_date_sent + ".";
        } else if (delivery.resource.get("successful") === false) {
            status_text =
                "Failed " + relative_date_sent + ": "
                + delivery.resource.get("error_message");
        }
