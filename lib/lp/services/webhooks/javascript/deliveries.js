...
Y.extend(WebhookDeliveries, Y.Widget, {
    ...
    initializer: function(config) {
        ...
        this.expanded_deliveries = [];
        this.retrying_deliveries = [];
    ...
    syncUI: function() {
        ...
        Y.Object.each(this.deliveries, function(delivery) {
            ...
            if (Y.Array.indexOf(
                    self.expanded_deliveries, resource.get('self_link'))
                    !== -1) {
                ...
                if (Y.Array.indexOf(
                        self.retrying_deliveries, resource.get('self_link'))
                        !== -1) {
    ...
    _toggle_detail: function(e) {
        var delivery_node = e.currentTarget;
        var delivery_url = delivery_node.getData('delivery-url');
        if (delivery_url === undefined) {
            // Not actually a delivery row.
            return;
        }
        var index = Y.Array.indexOf(this.expanded_deliveries, delivery_url)
        if (index === -1) {
            this.expanded_deliveries.push(delivery_url);
        } else {
            delete this.expanded_deliveries[index];
        }
        this.syncUI();
    },

    _trigger_redeliver: function(e) {
        var link_node = e.currentTarget;
        var detail_node = link_node.ancestor('tr', false);
        var delivery_url =
            detail_node.getData('delivery-tr').getData('delivery-url');
        if (Y.Array.indexOf(this.retrying_deliveries, delivery_url) !== -1) {
            // Already in progress.
            return;
        }
        this.retrying_deliveries.push(delivery_url);
        this.syncUI();
        var self = this;
        var config = {
            on: {
                success: function() {
                    delete self.retrying_deliveries[
                        Y.Array.indexOf(
                            self.retrying_deliveries, delivery_url)];
                    self.syncUI();
                }
            }
        };
