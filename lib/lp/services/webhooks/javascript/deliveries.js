...
Y.extend(WebhookDeliveries, Y.Widget, {
    ...
    _toggle_detail: function(e) {
        ...
        var detail_node = delivery_node.getData('delivery-detail-tr');
        if (detail_node !== undefined) {
            detail_node.remove();
            detail_node = undefined;
        } else {
            detail_node = this._render_detail(delivery_node);
            delivery_node.insert(detail_node, 'after');
        }
