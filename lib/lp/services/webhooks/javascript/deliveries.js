...
Y.extend(WebhookDeliveries, Y.Widget, {
    ...
    _trigger_redeliver: function(e) {
        ...
        if (detail_node.getData("delivery-retrying")) {
            // Already in progress.
            return;
        }
        var delivery_url =
            detail_node.getData('delivery-tr').getData('delivery-url');
        var spinner = detail_node.one(".delivery-redeliver-spinner");
        detail_node.setData("delivery-retrying", true);
        spinner.removeClass("hidden");
        var config = {
            on: {
                success: function() {
                    spinner.addClass("hidden");
                    detail_node.setData("delivery-retrying", false);
                },
            }
        };
