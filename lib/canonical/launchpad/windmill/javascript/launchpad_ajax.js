...
var test_no_such_url = new SynchronizedTest('test_no_such_url', [
    function (test) {
        client.get("no-such-url", test.create_yui_sync_on()); 
    },
    'wait_action',
    function (test) {
...
var test_pagination_collection = new SynchronizedTest(
    ...
    function (test) {
        client.get("/people", test.create_yui_sync_on(), 2, 1);
    },
    'wait_action',
    function (test) {
        jum.assertEquals('success', test.result.callback);
...
var test_named_get = new SynchronizedTest('test_named_get', [
    function (test) {
        client.named_get(
            "people/", "find", test.create_yui_sync_on(), {text:"salgado"});
    },
    'wait_action',
    function (test) {
        jum.assertEquals('success', test.result.callback);
        var collection = test.result.args[0];
        jum.assertTrue(collection instanceof Collection);
...
var test_named_post = new SynchronizedTest('test_named_post', [
    function (test) {
        test.bugtask_uri = '/redfish/+bug/15';
        client.named_post(
            test.bugtask_uri, 'transitionToStatus', test.create_yui_sync_on(),
            {status: 'Confirmed'});
    },
    'wait_action',
    function (test) {
        test.add_cleanups([
            function (test) {
                client.named_post(
                    ...
                    test.create_yui_sync_on(), {'status': 'New'});
            },
            'wait_action']);
        jum.assertEquals('success', test.result.callback);
        test.clear();
        // Get the bugtask and make sure its status has changed.
        client.get(test.bugtask_uri, test.create_yui_sync_on());
    },
    'wait_action',
    function(test) {
        jum.assertEquals('success', test.result.callback);
...
var test_follow_link = new SynchronizedTest('test_follow_link', [
    ...
    function (test) {
        ...
        root_object.follow_link('people', test.create_yui_sync_on());
    },
    'wait_action',
    function (test) {
        jum.assertEquals('success', test.result.callback);
        test.clear();
        var people = test.result.args[0];
        jum.assertTrue(people instanceof Collection);
...
test_logged_in_as_foo_bar.test_follow_link_through_redirect = 
    new SynchronizedTest(
        ...
        function (test) {
            ...
            test.clear();
            var root_object = test.result.args[0];
            root_object.follow_link('me', test.create_yui_sync_on());
        },
        'wait_action',
        function (test) {
            jum.assertEquals('success', test.result.callback);
            ...
            jum.assertTrue(me instanceof Entry);
...
var test_entry_get = new SynchronizedTest('test_entry_get', [
    ...
    function (test) {
        ...
        jum.assertTrue(salgado instanceof Entry);
...
var test_collection = new SynchronizedTest('test_collection', [
    function (test) {
        client.get("people", test.create_yui_sync_on());
    },
    'wait_action',
    function (test) {
        jum.assertEquals('success', test.result.callback);
