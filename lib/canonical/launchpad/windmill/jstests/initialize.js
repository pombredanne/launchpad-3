...
function SynchronizedTest(name, body, debug) {
    ...
    /*
     * The tear down is also as an array, so that other cleanup
     * actions can be tacked to it using add_cleanups()
     */
    this.teardown = [];
    this.add_windmill_actions(this.teardown, [
        function (test) {
            test.clear();
        }]);
...
SynchronizedTest.prototype.Y = YUI().use('dump');
...
SynchronizedTest.prototype.sync = function (result) {
    ...
    if (document.getElementById(this.name)) {
        throw new Error('Synchronization node already present.');
    }
...
SynchronizedTest.prototype.add_windmill_actions = function (list, items) {
    ...
    for (var i=0; i< items.length; i++) {
        ...
        if (yui_lang.isFunction(test_item)) {
            ...
            //in the test body. Comme use case is to use 'wait_action' to
            //add a windmill wait action for the synchronizing elements to
            //appear.
            list.push(test[test_item]());
...
SynchronizedTest.prototype.wait_action = function () {
    return {method: "waits.forElement", params: {id: this.name}};
};

/* Clear the synchronization node */
SynchronizedTest.prototype.clear = function () {
    this.log('clear() called.');
    var sync_node = document.getElementById(this.name);
    if (sync_node) {
        this.log('removing synchronization node.');
...
SynchronizedTest.prototype.create_yui_sync_on = function () {
    ...
    return {
        success: function () {
            test.sync({callback: 'success', args: arguments});
        },
        failure: function () {
            test.sync({callback: 'failure', args: arguments});
        }
    };
};
