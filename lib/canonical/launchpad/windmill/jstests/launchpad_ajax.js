...
test_prepopulated_data_anonymous = new SynchronizedTest(
    ...
    // Don't proceed until the existing page has been unloaded. Otherwise
    // the following tests takes place in a doomed window scope.
    {"params": {"js": function () {return !document.body;}},
        "method": "waits.forJS"},
...
test_logged_in_as_foo_bar.test_prepopulated_data = new SynchronizedTest(
    ...
    function (test) {
        ...
        jum.assertNotEquals(bug.self_link.indexOf("/api/beta/bugs/1"), -1);
    }
    ]);
...
var test_normalize_uri = function() {
    ...
    jum.assertEquals(normalize("http://www.example.com/api/beta/foo"),
                     "/api/beta/foo");
    jum.assertEquals(normalize("http://www.example.com/foo/bar"), "/foo/bar");
    jum.assertEquals(normalize("/foo/bar"), "/api/beta/foo/bar");
    jum.assertEquals(normalize("/api/beta/foo/bar"), "/api/beta/foo/bar");
    jum.assertEquals(normalize("foo/bar"), "/api/beta/foo/bar");
    jum.assertEquals(normalize("api/beta/foo/bar"), "/api/beta/foo/bar");
};
...
// HTML snippet.
var test_entry_html_get = new SynchronizedTest('test_entry_xhtml_get', [
    function (test) {
        client.get_html('~salgado', {on: test.create_yui_sync_on()});
    },
    'wait_action',
    function (test) {
        jum.assertEquals('success', test.result.callback);
        var salgado = test.result.args[0];
        jum.assertEquals(salgado.firstChild.tagName, "dl");
    }
    ]);
...
test_logged_in_as_foo_bar.test_hosted_files = new SynchronizedTest(
    ...
    function(test) {
        ...
        // This should be a 405.
        var request = test.result.args[1];
        jum.assertEquals(500, request.status);
    }
    ]
);
