...
var test_named_get_returning_json = new SynchronizedTest(
    ...
    function (test) {
        client.get('~name18', {on: test.create_yui_sync_on()});
    },
    'wait_action',
    function (test) {
        jum.assertEquals('success', test.result.callback);
        var team = test.result.args[0];
        client.named_get("~salgado", 'inTeam',
                         {on: test.create_yui_sync_on(),
                                 parameters: {team: team.get('self_link')}});
    },
    'wait_action',
    function (test) {
        debugger;
        ...
        alert(structure);
        jum.assertEquals(structure, True);
    }
    ]);
