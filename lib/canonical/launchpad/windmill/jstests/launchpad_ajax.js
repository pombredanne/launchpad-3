...
// Test the LP.client.copy_object method.
var test_copy_object = new SynchronizedTest(
    'test_copy_object', [
    function (test) {
        obj = {1:2, "foo":"bar"};
        copy = LP.client.copy_object(obj);
        jum.assertEquals(copy[1], 2);
        jum.assertEquals(copy.foo, "bar");
    }
    ]);
...
test_logged_in_as_foo_bar.test_prepopulated_data = new SynchronizedTest(
    ...
    {"params": {"url": "/bugs/1"}, "method": "open"},
    {"params": {"timeout": 8000}, "method": "waits.forPageLoad"},
    function (test) {
        ...
        jum.assertNotEquals(context.related_tasks_collection_link.indexOf(
                            "/api/beta/firefox/+bug/1/related_tasks"), -1);
    }
    ]);
...
var test_relative_url = new SynchronizedTest('test_relative_url', [
    function (test) {
        test.config = {on: test.create_yui_sync_on()};
    ...
    // Run another test with the same configuration object
    // to make sure the test didn't change the configuration object.
    function (test) {
        client.get("/people", test.config);
    },
    'wait_action',
    function (test) {
        jum.assertEquals('success', test.result.callback);
...
var test_entry_get = new SynchronizedTest('test_entry_get', [
    ...
    function (test) {
        ...
        jum.assertEquals(salgado.get('name'), "salgado");
    }
    ]);
...
test_logged_in_as_foo_bar.test_entry_lp_save = new SynchronizedTest(
    ...
    function (test) {
        ...
        salgado.set('display_name', 'A new display name');
    ...
    function (test) {
        jum.assertEquals('success', test.result.callback);
        var salgado = test.result.args[0];
        jum.assertEquals('A new display name', salgado.get('display_name'));
...
test_logged_in_as_foo_bar.test_collection_named_post = new SynchronizedTest(
    ...
    function(test) {
        ...
        test.collection = test.result.args[0];
        test.config = {on: test.create_yui_sync_on(),
                       parameters: {display_name: 'My new team',
                                    name: name}};
        test.collection.named_post('newTeam', test.config);
    },
    'wait_action',
    function(test) {
    ...
    // Run the test again so we can make sure the call to named_post
    // didn't modify the configuration object.
    function(test) {
        test.config.parameters.name = test.config.parameters.name + '-second';
        test.collection.named_post('newTeam', test.config);
    },
    'wait_action',
    function(test) {
        jum.assertEquals("success", test.result.callback);
        var new_entry = test.result.args[0];
        var uri = new_entry.lp_original_uri;
        jum.assertTrue(new_entry instanceof LP.client.Entry);
        // Make sure the object is the one we created the second time,
        // not a holdover from the first named POST.
        jum.assertEquals("-second", uri.substring(uri.length-7));
    }
    ]);
