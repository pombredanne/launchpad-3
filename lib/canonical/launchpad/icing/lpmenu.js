...
window.lpmenu = new function() {
    ...
    Loader.prototype.load = function(mid, handler) {
        ...
        */
        ...
        // XXX note that this can potentially cause a URL to get requested
    ...
    Loader.prototype.errback = function(err) {
        ...
        // XXX in a bit of a hurry now, obviously this needs to be done
        // differently
    ...
    LPMenu.prototype.mouse_out_handler = function(event, el) {
        ...
        // XXX mouse out handling is very messy in browsers... 
        ...
        // XXX close on document mouse over instead?
    ...
    LPSubMenu.prototype.initialize = function(subel, parent, root) {
        ...
        if (!this.subel.handlers_set) {
            for (var i=0; i < this.subel.childNodes.length; i++) {
                // XXX currently we do caching in the loader, which means that
    ...
    LPSubMenu.prototype._render_scrolled_menu = function() {
        // XXX i'd rather use this.subel.ownerDocument here, but for some
    ...
    LPSubMenu.prototype.adjust_menu = function(direction) {
        // XXX not very optimal, some code duplication and such
        var items = this.items;
