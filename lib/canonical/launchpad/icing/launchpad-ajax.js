...
Resource.prototype = {
    ...
    'get_hosted_file': resource_get_hosted_file,
...
function resource_get_hosted_file(link_name) {
    var url = this[link_name + '_link'];
    if (url == undefined) {
        throw new Error("No such hosted file: " + link_name);
    }
    alert(new HostedFile(url));
    return new HostedFile(url);
}
...
function delete_hosted_file() {
    /* Delete this hosted file. */
    return doXHR(this.uri, {method: "DELETE"});
}