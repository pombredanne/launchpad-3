...
LP.client.Resource.prototype = {
    ...
    follow_link: function(link_name, config) {
        ...
        on = config.on;
        var uri = this[link_name + '_link'];
        if (uri === undefined) {
            uri = this[link_name + '_collection_link'];
        }
        if (uri === undefined) {
...
LP.client.Entry.prototype.lp_save = function(config) {
    ...
    var on = config.on;
    var representation = this._conf.data.value; // Hack around broken getAttrs().
    var entry = this;
    var data = Y.JSON.stringify(representation);
    args = [entry.lp_client, entry.get('self_link')];
    var y_config = { method: "PUT",
                     on: on,
                     headers: {"Content-Type": "application/json"},
                     'arguments': args,
                     data: data};
    Y.io(entry.get('self_link'), y_config);
};
