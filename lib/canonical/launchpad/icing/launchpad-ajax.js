...
function append_start_and_size(uri, start, size) {
    ...
    if (start != undefined) {
        uri = append_qs(uri, 'ws.start', start);
    }
    if (size != undefined) {
        uri = append_qs(uri, 'ws.size', size);
    }
    return uri;
}
...
function save_entry(callback, errback)
{
    ...
    var deferred = doXHR(this.self_link,
                         {method: "PUT",
...
function slice_collection(callback, errback, start, size) {
    this.lp_client.get(this.lp_original_uri, callback, errback, start, size);
}
