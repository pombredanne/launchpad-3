...
function ajax_test_suite() {
    ...
    var deferred = client.get("no-such-url");
    deferred.addCallback(expect_404_callback);
    deferred.addErrback(expect_404_errback);
    ...
    var deferred = client.get("");
    deferred.addCallback(relative_callback);
    deferred.addErrback(default_errback);

    function relative_callback(launchpad) {
        var success = (launchpad.people_collection_link
                       == "https://launchpad.dev/api/beta/people");
        report_test("Load a resource from relative URL", success);
    }

    var deferred = client.get("https://launchpad.dev/api/beta/");
    deferred.addCallback(absolute_callback);
    deferred.addErrback(default_errback);
    ...
    var deferred = client.get("/people", 2, 1);
    deferred.addCallback(uri_pagination_callback);
    deferred.addErrback(default_errback);
    ...
    var deferred = client.named_get('people/', 'find', {text:'salgado'});
    deferred.addCallback(uri_named_get_callback);
    deferred.addErrback(default_errback);
    ...
    function uri_named_post_callback(nothing) {
        ...
        var deferred = client.get(bugtask_uri);
        deferred.addCallback(uri_named_post_callback_check_bugtask);
        deferred.addErrback(default_errback);
    }
    ...
    var deferred = client.get('');
    deferred.addCallback(service_root_callback);
    deferred.addErrback(default_errback);
    ...
    function service_root_callback(root_object) {
        ...
        //var deferred = root_object.follow_link('people');
        //deferred.addCallback(follow_link_callback);
        //deferred.addErrback(default_errback);
    }
    ...
    var deferred = client.get('~salgado');
    deferred.addCallback(entry_callback);
    deferred.addErrback(default_errback);
    ...
    function entry_callback(entry) {
        ...
        // Get a hosted file that doesn't exist yet and run tests on it.
        var deferred = entry.follow_link('mugshot');
    ...
    function entry_save_callback(entry) {
        ...
        var deferred = client.get('~salgado');
        deferred.addCallback(modified_entry_callback);
        deferred.addErrback(default_errback);
    }
    ...
    // Test a HostedFile object that doesn't exist.
    function nonexistent_hosted_file_callback(hosted_file) {
        var success = (hosted_file instanceof HostedFile
                       && hosted_file.uri.indexOf("~salgado/mugshot" != -1));
        report_test("Following link to nonexistent hosted file yields " +
                    "HostedFile object", success);

        // Let's stick some data in it.
        // XXX: This doesn't work because XHR.send() won't send binary
        // data.
        // var deferred = doXHR("https://launchpad.dev/@@/person-mugshot");
        // deferred.addCallback(set_file_value, hosted_file);
        // deferred.addErrback(default_errback);
    }

    // Test a preexisting file object.

    var deferred = client.get("alsa-utils/trunk/1.0.9a/+file/README.txt");
    deferred.addCallback(set_file_value_callback);
    deferred.addErrback(default_errback);

    function set_file_value_callback(hosted_file) {
        hosted_file.contents = "Unit tester was here.";
        hosted_file.filename = "unittest.txt";
        hosted_file.content_type = "text/plain";

        var deferred = hosted_file.lp_save();
        deferred.addCallback(modified_hosted_file_callback);
        deferred.addErrback(default_errback);
    }


    function modified_hosted_file_callback(hosted_file) {
        var success = (hosted_file instanceof HostedFile)
        report_test("Following link to existing hosted file yields " +
                    "HostedFile object", success);

        var deferred = hosted_file.follow_link('file');
        deferred.addCallback(get_modified_hosted_file_value_callback);
        deferred.addErrback(default_errback);
    }


    function get_modified_hosted_file_value_callback(contents) {
        alert("Written!");
    }


    // Test a Collection object.
    var deferred = client.get('people/');
    deferred.addCallback(collection_callback);
    deferred.addErrback(default_errback);
    ...
    function collection_callback(collection) {
        ...
        // Now that we have the object, run some more tests.
        var deferred = collection.lp_slice(2,1);
        deferred.addCallback(collection_slice_callback);
        deferred.addErrback(default_errback);

        var deferred = collection.named_get('find', {text:'salgado'});
        deferred.addCallback(collection_named_get_callback);
        deferred.addErrback(default_errback);

        var deferred = collection.named_get('find', {text:'salgado'}, 10);
        deferred.addCallback(paged_collection_named_get_callback);
        deferred.addErrback(default_errback);
