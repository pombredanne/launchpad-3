...
function isInside(point, element) {
    /*
      ...
      Note that the element position is taken relative to the current
      viewport.
    */
    var viewport_pos = getViewportPosition();
    var element_pos = getElementPosition(element);
    var relative_pos = new Coordinates(
        element_pos.x - viewport_pos.x,
        element_pos.y - viewport_pos.y
    );

    return intersect(point, relative_pos, getElementDimensions(element));
}
