...
function setAddMoreLinesButton() {
    forEach(findAddMoreLinesButton(), showAddMoreLinesButton);
}

function findAddMoreLinesButton() {
    has_add_more_lines = function (elem) {
        return getNodeAttribute(elem, 'class') == 'translations_add_more_lines';
    };
    return filter(has_add_more_lines,
                  currentDocument().getElementsByTagName('span'));
}

function showAddMoreLinesButton(elem) {
    elem.innerHTML = '<img src="/+icing/translations-add-more-lines.gif">';
}
