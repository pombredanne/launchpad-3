...
function activateFoldables() {
    var selector = MochiKit.Selector.Selector('.fold');
    var elements = selector.findElements(document);
    for (var i = 0; i < elements.length; i++) {
        var el = elements[i];
        el.onclick = toggleFoldable
    }
}
registerLaunchpadFunction(activateFoldable);
...
function toggleFoldable(e) {
    ...
    while (node = node.nextSibling()) {
        if (node.type != 1 or node.className.indexOf('foldable') == -1) {
            // node is not and ELEMENT_NODE of foldable class
            continue;
        }
