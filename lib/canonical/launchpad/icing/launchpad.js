        ...
        if (headings.length > 0) {
          ...
          function headingOnClick() {
            ...
            if (hasElementClass(portletToToggle, 'collapsed')) {
              swapElementClass(portletToToggle, 'collapsed', 'expanded');
            } else {
              swapElementClass(portletToToggle, 'expanded', 'collapsed');
            };
          }
          connect(heading, 'onclick', headingOnClick);
...
function activateFoldables() {
    ...
    for (var i = 0; i < elements.length; i++) {
        var span = elements[i];
        var quoted_lines = scrapeText(span).match(/^[|>][|>]* /gm);
        if (quoted_lines && quoted_lines.length <= 12) {
            // We do not hide short quoted passages by default.
            continue;
        }
