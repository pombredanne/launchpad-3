...
function toggleExpandableTableRow(element_id) {
    // Used in sourcepackagepublishinghistory-listing-ppa-detailed
...
function setFocus() {
    ...
    var formnodes, formnode, divnodes, node, inputnode;
    ...
    for (var f = 0; (formnode = formnodes.item(f)); f++) {
        ...
        for (var j = 0; (node = inputnodes.item(j)); j++) {
...
function copyInnerHTMLById(from_id, to_id) {
    // Used by translationmessage-suggestions.pt.
...
function writeTextIntoPluralTranslationFields(from_id,
    ...
    // Used by currenttranslationmessage-translate-one.pt.
...
function expandRemoteCommentReply(comment_index) {
    ...
    $(prefix + 'reply_div_' + comment_index).style.display = 'block';
}

function toggleRemoteCommentReply(comment_index) {
    // Used by bugcomment-macros.pt.
    var prefix = 'remote_comment_reply_tree_icon_';
    var imgname = $(prefix + comment_index).src.split('/').pop();
...
function setLocation(lat, lng, tz_name, latname, lngname) {
    // Used by the location widget.
    $(latname).value = lat;
    $(lngname).value = lng;

    var request = new JSONScriptRequest();
    var url = 'http://ba-ws.geonames.net/timezoneJSON';
    url += "?username=launchpad&lat=" + lat.toString();
    url += "&lng=" + lng.toString();
...
function switchDisplay(tag_id1, tag_id2) {
    // Used by codereviewcomment-macros.pt.
