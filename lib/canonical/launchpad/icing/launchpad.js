...
var PortletManager = {
    ...
    writeCookie: function() {
        ...
        // XXX: 20070718 jamesh
        // Should we set an expiry on this cookie?
        document.cookie = 'expanded_portlets=' + value + this.cookie_scope;
    },

    isExpanded: function(portlet_name) {
        for (var i = 0; i < this.expanded_portlets.length; i++) {
            if (this.expanded_portlets[i] == portlet_name) {
                return true;
            }
        }
        return false;
    ...
    collapsePortlet: function(portlet) {
        ...
        var portlet_name = portlet.getAttribute('id');
        if (portlet_name) {
            this.expanded_portlets = filter(partial(compare, portlet_name),
                                            this.expanded_portlets);
            this.writeCookie();
        }
    },
    ...
    expandPortlet: function(portlet) {
        ...
        var portlet_name = portlet.getAttribute('id');
        if (portlet_name && !this.isExpanded(portlet_name)) {
            this.expanded_portlets.push(portlet_name);
            this.writeCookie();
        }
    },

    togglePortlet: function(ev) {
        portlet = getFirstParentByTagAndClassName(ev.src(), 'div', 'portlet');
        if (hasElementClass(portlet, 'collapsed')) {
            this.expandPortlet(portlet);
        } else {
            this.collapsePortlet(portlet);
        }
        return false;
    },
