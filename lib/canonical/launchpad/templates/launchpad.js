...
function convertTextInputToTextArea(current_text_input_id, rows) {
    var current_text_input = getElement(current_text_input_id)
    var new_text_area = document.createElement("textarea")
    var attributes

    attributes = {
        'id': current_text_input_id,
        'rows': rows,
...
function upgradeToTextAreaForTranslation(current_text_input_id) {
    current_text_input = getElement(current_text_input_id)
    current_text_input.parentNode.removeChild(
        getElement(current_text_input_id + '_to_textarea'))
    convertTextInputToTextArea(current_text_input_id, 6)
}
...
function writeTextIntoPluralTranslationFiels(from_id, to_id_pattern,
                                             nplurals) {
