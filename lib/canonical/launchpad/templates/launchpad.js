...
function convertInputIntoTextarea(replace_name, hide_name, rows) {
    // Change input widget 'replace_name' with a textarea of 'rows' rows.
    // The textarea will preserve the input widget attributes and content.
    // If 'hide_name' is given, we will remove the widget with that name. This
    // is used to remove the button that raises the converting.
    var replace = document.getElementById(replace_name)
    var newTextArea = document.createElement("textarea")

    if (rows) {
        newTextArea.setAttribute("rows", rows)
    }
    if (replace.hasAttribute("id")) {
        old_id = replace.getAttribute("id")
        replace.setAttribute("id", old_id + "_old")
        newTextArea.setAttribute("id", old_id)
    }
    if (replace.hasAttribute("name")) {
        newTextArea.setAttribute("name", replace.getAttribute("name"))
    }
    if (replace.hasAttribute("lang")) {
        newTextArea.setAttribute("lang", replace.getAttribute("lang"))
    }
    if (replace.hasAttribute("dir")) {
        newTextArea.setAttribute("dir", replace.getAttribute("dir"))
    }
    if (replace.hasAttribute("onKeyPress")) {
        newTextArea.setAttribute("onKeyPress", replace.getAttribute("onKeyPress"))
    }
    if (replace.hasAttribute("onChange")) {
        newTextArea.setAttribute("onChange", replace.getAttribute("onChange"))
    }

    newTextArea.value = replace.value
    if (hide_name) {
        replace.parentNode.removeChild(document.getElementById(hide_name))
    }

    replace.parentNode.replaceChild(newTextArea, replace)
}

/*
 * XXX: CarlosPerelloMarin 2006-12-21: This function is based on prototype's
 * unescapeHTML method. We should remove it once MochiKit implements it.
 * See bug #78788 for details.
 */
...
function copyInnerHTMLById(from_id, to_id) {
    ...
    // Remove any html tag from the text to copy and copy it.
    to.value = unescapeHTML(from.innerHTML.replace(/<\/?[^>]+>/gi,""));

}

function copyEnglishPluralIntoPluralTranslations(from_id, to_id_pattern,
                                                 nplurals) {
