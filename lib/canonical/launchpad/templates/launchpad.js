...
// Focus the first widget with an error.  Otherwise focus the named widget.
function setFocusById(name) {
    var error_re = new RegExp(/\berror\b/);

    // search for error widgets
    var formnodes = document.getElementsByTagName('form');
    for (var i = 0; i < formnodes.length; i++) {
        var formnode = formnodes[i];
        var divnodes = formnode.getElementsByTagName('div');
        for (var j = 0; j < divnodes.length; j++) {
            var node = divnodes[j];
            if (error_re.exec(node.className)) {
                var inputnode = node.getElementsByTagName('input').item(0);
                if (inputnode) {
                    inputnode.focus();
                    return;   
                }
    ...
    // fall back to focusing the named element
    if (name) {
