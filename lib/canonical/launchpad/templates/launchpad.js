...
function convertInputIntoTextarea(replace_name, hide_name, rows) {
    ...
    if (replace.hasAttribute("id")) {
        newTextArea.setAttribute("id", replace.getAttribute("id"))
    }
...
function copyInnerHTMLById(from_id, to_id) {
    ...
    // Remove any html tag from the text to copy
    content_to_copy = from.innerHTML.replace(/<[^>]+>/g,"");

    if (to.type == "text") {
        // It's an input field.
        to.setAttribute("value", content_to_copy);
    } else {
        // It's a textarea field.
        to.innerHTML = content_to_copy;
    }
}
