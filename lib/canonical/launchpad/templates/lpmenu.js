...
window.lpmenu = new function() {
    /* A simple dropdown menu for the LaunchPad website */
        ...
        */
        ...
        var ihandler = function(req) {
            ...
            if (req.status != 200 && req.status != 304) {
                alert('problem loading data: ' + req.status);
                return;
            };
            ...
            if (iscontainer) {
                ...
                if (!sub) {
                    alert('problem loading data from server: requested menu ' +
        ...
        var d = MochiKit.Async.doSimpleXMLHttpRequest(this.url+mid);
    ...
    Loader.prototype.errback = function(err) {
        ...
        alert('An error occurred when loading a menu item from the ' +
                'server: ' + err);
    };
        ...
        current_mouse_over_deferred = MochiKit.Async.callLater(
            ...
            function() {
                try {
                    ...
                    debug('exception in update_tree: ' + e.message);
                };
    ...
    LPMenu.prototype.update_tree = function(movedfrom, movedto) {
        ...
        if (movedfrom != movedto) {
            if (current_selected) {
                current_selected.className = previous_class;
                current_selected = movedto;
                previous_class = movedto.className;
                movedto.className += ' selected';
            };
        };
    };
    ...
    LPMenu.prototype.attach = function(parent, sub) {
        ...
        */
        ...
        if (lpmenu.offsetTop(parent) + menuheight > 
                    lpmenu.getAvailHeight()) {
            if (document.all) {
                rendered.style.marginTop = -(lpmenu.offsetTop(parent) -
                                                lpmenu.IE_OFFSET_CORRECTION);
            } else {
                rendered.style.top = this.firstlineempty ? 
                                        this.lineheight : '0px';
            };
        };

        // add 'selected' to the parent class so it can be styled
        previous_class = parent.className;
        current_selected = parent;
    ...
    LPMenu.prototype.close_all = function(event) {
        ...
        if (current_mouse_out_target || escape_clicked) {
            event.stopPropagation();
            event.preventDefault();
    ...
    LPMenu.prototype._issibling = function(el1, el2) {
        try {
            ...
            debug('exception in LPMenu._issibling(): ' + e.message);
        };
    };
    ...
    LPMenu.prototype._isparent = function(el1, el2) {
        try {
            ...
            debug('exception in LPMenu._isparent(): ' + e.message);
        };
    };
    ...
    LPMenu.prototype._ischild = function(el1, el2) {
        ...
        */
        try {
            ...
            debug('exception in LPMenu._ischild(): ' + e.message);
        };
    };
    ...
    LPSubMenu.prototype.initialize = function(subel, parent, root) {
        ...
        if (parent && parent.parentNode.parentNode.nodeName == 'LI') {
            if (document.all) {
                ...
                this.subel.style.marginTop = lpmenu.IE_TOP_CORRECTION;
                if (this.subel.parentNode != this.root.rootel) {
                    this.subel.style.left = parent.parentNode.maxWidth + 
                                                this.root.iconWidth;
                };
            } else {
                this.subel.style.marginLeft = parent.parentNode.maxWidth + 
                                                this.root.iconWidth +
                                                lpmenu.LEFT_CORRECTION;
            };
        } else if (document.all && parent) {
            // locate the first sub menus (below the root items)
            this.subel.style.left = parent.offsetLeft;
            this.subel.style.posTop = this.root.lineheight;
            this.subel.style.top = this.root.lineheight;
            this.subel.set_root_top = this.root.lineheight;
        } else {
            this.subel.style.left = '0px';
        };
    ...
    LPSubMenu.prototype.set_handlers = function(el) {
        ...
        MochiKit.Signal.connect(el, 'onmouseout', this, outhandler);
    ...
    LPSubMenu.prototype._render_scrolled_menu = function() {
        ...
        if (!document.all) {
            clone.style.top = this.root.firstlineempty ? 
                            this.root.lineheight : '0px';
        };
        return clone;
    };
    ...
    LPSubMenu.prototype.start_scroll_down = function(e) {
        ...
        try {
            ...
            debug('error in update tree: ' + e.message);
        };
        this.scroll_down();
    };
    ...
    LPSubMenu.prototype.scroll_down = function() {
        ...
        try {
            ...
            debug('exception in adjust_menu: ' + e.message);
        };
        var self = this;
    ...
    LPSubMenu.prototype.stop_scroll = function() {
        try {
            ...
            debug('exception during cancellation of deferred: ' + e.message);
        };
    ...
    LPSubMenu.prototype.create_scroll_button = function(direction) {
        ...
        */
        ...
        el.className = 'scrollbutton ' + direction;
    ...
    LPSubMenu.prototype.update_item_widths = function() {
        ...
        for (var i=0; i < this.menu.childNodes.length; i++) {
            ...
            child.style.width = this.maxWidth + this.root.iconWidth;
        };
    };
    ...
    this.offsetTop = function(el) {
        var curr = el;
        var offset = 0;
        while (curr.parentNode && curr.nodeName != 'BODY') {
