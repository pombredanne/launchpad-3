...
window.lpmenu = new function() {
        ...
        */
        ...
        var ihandler = function(req) {
            ...
            if (req.status != 200 && req.status != 304) {
                debug('ALERT: problem loading data: ' + req.status);
                return;
            };
            ...
            if (iscontainer) {
                ...
                if (!sub) {
                    debug('ALERT: problem loading data from server: requested menu ' +
    ...
    Loader.prototype.errback = function(err) {
        ...
        debug('ALERT: An error occurred when loading a menu item from the ' +
                'server: ' + err);
    };
        ...
        if (lpmenu.offsetTop(parent) + menuheight > 
            ...
            if (document.all) {
                rendered.style.marginTop = ensure_units(
                    -(lpmenu.offsetTop(parent) - lpmenu.IE_OFFSET_CORRECTION));
            } else {
    ...
    LPSubMenu.prototype.initialize = function(subel, parent, root) {
        ...
        if (parent && parent.parentNode.parentNode.nodeName == 'LI') {
            if (document.all) {
                ...
                this.subel.style.marginLeft = ensure_units(
                    parent.parentNode.maxWidth + this.root.iconWidth +
            ...
            this.subel.style.top = '46px';
            if (this.parent) {
                this.subel.style.offsetLeft = ensure_units(
        ...
        // attach mouse over and mouse out events for IE only
        /* works mostly but commenting out for now to persue using block level A tags
         * which support hover nativly -newz
        if(document.all) {
          obj = document.getElementById('menuroot');
          litems = obj.getElementsByTagName('li');
            ...
            menuitem.onmouseover = function() {
              ...
              for (i = 0; i < items.length; i++) {
                items[i].onmouseover = function () {
                  this.style.backgroundColor = '#8cacbb';
                };
                items[i].onmouseout = function () {
                  this.style.backgroundColor = '#dee7ec';
                };
              }
            };
          }
