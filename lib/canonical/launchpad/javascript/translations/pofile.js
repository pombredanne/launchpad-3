...
var WORKING_MODE_CURRENT_ID = "#translation-current-working-mode";
...
var setTranslatorModeText = function () {
    current_mode_text = Y.one(WORKING_MODE_CURRENT_ID);
    if (current_mode_text === null) {
        Y.log("Could not find " + WORKING_MODE_CURRENT_ID);
        return;
    }
    ...
    current_mode_text.set(
        'innerHTML',
        'Working in translator mode.');
...
var switchWorkingMode = function () {
    if (getWorkingMode() == WORKING_MODE_TRANSLATOR) {
        ...
        Y.Cookie.set(
            WORKING_MODE_COOKIE,
            WORKING_MODE_REVIEWER,
            {
                path: "/"
            });
    } else {
        setTranslatorModeText();
        Y.Cookie.set(
            WORKING_MODE_COOKIE,
            WORKING_MODE_TRANSLATOR,
            {
                path: "/"
            });
    }
...
var selectTranslation = function(e, widget_id) {
...
var initializeFieldsKeyBindings = function (fields) {
    for (var key = 0; key < fields.length; key++) {
        ...
        var translation_select_id = fields[key] + '_select';

        Y.on(
            'change', selectTranslation,
            '#' + fields[key], Y, translation_select_id);
        Y.on(
