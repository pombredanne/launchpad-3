...
var initializeWorkingMode = function () {
    ...
    if (working_mode_switch !== null) {
        working_mode_switch.addClass("sprite edit js-action widget-hd");

        var working_mode_switch_help = Y.one(WORKING_MODE_SWITCH_HELP_ID); 
        if (working_mode_switch_help !== null) {
            var what_is_this = (
                '(<a href="/+help/working-modes.html" target="help">' +
                'What\'s this?</a>)');
            working_mode_switch_help.set("innerHTML", what_is_this);
        }
        if (getWorkingMode() == WORKING_MODE_TRANSLATOR) {
            setTranslatorModeText();
        } else {
