    ...
    bug.lp_save({on : {
        success: function(updated_entry) {
            ...
            if (Y.Lang.trim(tags_html) == '') {
                Y.get('#bug-tags').removeClass(TAGS_SHOW);
                Y.get('#bug-tags').addClass(TAGS_HIDE);
                Y.get('#add-bug-tags').removeClass(TAGS_HIDE);
                Y.get('#add-bug-tags').addClass(TAGS_SHOW);
            }
        },
...
bugs.setup_tag_entry = function(available_official_tags) {
    ...
    add_tags_trigger = Y.get('#add-tags-trigger');
    ...
    add_tags_trigger.on('click', function(e) {
        e.halt();
        Y.get('#bug-tags').removeClass(TAGS_HIDE);
        Y.get('#bug-tags').addClass(TAGS_SHOW);
        Y.get('#add-bug-tags').removeClass(TAGS_SHOW);
        Y.get('#add-bug-tags').addClass(TAGS_HIDE);
        edit();
    });
