...
suite.add(new Y.Test.Case({
    ...
    test__getNewSourceNames: function() {
        var names;
        // No other users affected.
        names = this.choice_edit._getNewSourceNames(0);
        Assert.isUndefined(names[true]);
        Assert.isUndefined(names[false]);
        // 1 other user affected.
        names = this.choice_edit._getNewSourceNames(1);
        Assert.areEqual(
        ...
        names = this.choice_edit._getNewSourceNames(2);
        Assert.areEqual(
    ...
    test_new_source_names_are_applied: function() {
        var source_names = {};
        Y.each(this.choice_edit.get('items'), function(item) {
            source_names[item.value] = item.source_name;
        });
        Assert.areEqual(
            'This bug affects you and 5 other people', source_names[true]);
        Assert.areEqual(
            'This bug affects 5 people, but not you', source_names[false]);
    }
