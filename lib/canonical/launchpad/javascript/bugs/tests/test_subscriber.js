YUI({
    ...
    }).use('test', 'console', 'lp.subscriber', function(Y) {
...
Y.extend(APIStubSubscriber, Y.lp.Subscriber, {
...
suite.add(new Y.Test.Case({
    ...
    test_display_name_load_event: function() {
        ...
        subscriber.on('displaynameload', function(e) {
            // Resume the test and immediately signal success.
            test.resume(function() { Y.assert(true); });
        });
        subscriber.set('uri', '/~tester');
        subscriber.initializer();
        // Suspending the test isn't strictly necessary because the
        // event is sent synchronously, but this will also work even
        // if that changes.
        test.wait(2000);
    }
}));
