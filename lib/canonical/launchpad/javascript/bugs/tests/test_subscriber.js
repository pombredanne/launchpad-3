YUI({
    ...
    suite.add(new Y.Test.Case({
        ...
        test_subscription_config: function() {
            Y.Assert.isFalse(
                this.subscription.canBeUnsubscribedByUser(),
                'The subscription can not be unsubscribed.');
            Y.Assert.isTrue(
                this.subscription.isTeam(),
                'The subscription represents a team.');
            Y.Assert.isTrue(
                this.subscription.isDirectSubscription(),
                'This is a direct subscription.');
            // Also check that the defaults were set.
            Y.Assert.isNull(
                this.subscription.get('person'),
                'The subscription is not setup for a person.');
            Y.Assert.isNull(
                this.subscription.get('subscriber'),
                'The subscription is not setup for a subscriber.');
        },

        test_subscription_is_node: function() {
            Y.Assert.isFalse(
                this.subscription.isNode(),
                'Initially, no node is supplied to the config.');
            var link = Y.get('.menu-link-subscription');
            this.subscription.set('link', link);
            Y.Assert.isTrue(
                this.subscription.isNode(),
                'This subscription has a node for subscription link.');
        },
        ...
        test_already_subscribed: function() {
            ...
            Y.Assert.isTrue(
                this.subscription.isAlreadySubscribed(),
                'The JS Test User is already subscribed.');
        },
        ...
        test_is_current_user_subscribing: function() {
            ...
            Y.Assert.isTrue(
                this.subscription.isCurrentUserSubscribing(),
                'Current user is the same person being subscribed.');
        }
    }));
