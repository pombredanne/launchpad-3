 ...
 */
var get_official_bug_tags = function() {
    var tags = [];
 ...
 */
var get_other_bug_tags = function() {
    var tags = [];
...
var enable_arrows = function() {
    ...
    Y.get('#remove-official-tags').set('disabled', !official_cbs_checked);
    Y.get('#add-official-tags').set('disabled', !other_cbs_checked);
}
...
var make_tag_li = function(item) {
    ...
    li_node.query('input').on('click', function(e) {
      enable_arrows();
      var cb_node = li_node.query('input');
...
/**
 * Save the used tags to the databse.
 *
...
var on_new_tag_add = function() {
    ...
    if (!new_tag_already_official && !new_tag_already_used) {
        if (valid_name_re.test(new_tag)) {
            ...
            Y.get('#new-tag-text').set('value', '');
            Y.get('#new-tag-add').set('disabled', true);
            Y.get('#save-button').set('disabled', false);
        } else {
...
bugs.setup_official_bug_tag_management = function() {
    official_tags = get_official_bug_tags();
    other_tags = get_other_bug_tags();
    ...
    // pressed, try adding a new tag. If an error message is currently
    // displayed and the user edits the textbox, validate the currently entered
    // tag and if it's valid, hide the error message.
    Y.get('#new-tag-text').on('keypress', function(e) {
        var new_value = Y.Lang.trim(Y.get('#new-tag-text').get('value'));
        Y.get('#new-tag-add').set('disabled', new_value == '');
