...
bugs.setup_bugtask_row = function(row_id, bugtask_url, status_widget_items) {
    var tr = Y.get('#' + row_id);
    var td_status = tr.query('.status-content');
    var edit_icon = tr.query('.editicon');
    var status_choice_edit = new Y.ChoiceSource({
        contentBox: td_status,
        value: td_status.getAttribute('current_value'),
    ...
    td_status.on('mouseover', function(e) {
        edit_icon.set('src', '/@@/edit')
    });
    td_status.on('mouseout', function(e) {
        edit_icon.set('src', '/@@/edit-grey')
    });
    td_status.setStyle('cursor', 'pointer');
};
