...
function unsubscribe_user_via_icon(icon) {
    ...
    // Travese the DOM of the icon to get
    // the user name for this unsubscribe.
    var parent_div = icon.get('parentNode').get('parentNode');
    var link_uri = parent_div.query('a').getAttribute('href');
    // For the user name, strip the leading "/~".
    var icon_user_name = link_uri.substring(2);
    ...
    if (!current_user_unsubscribing) {
        config.parameters = {
            person: LP.client.get_absolute_uri(link_uri)
        }
    }
    ...
    var config = {
        on: {
            success: function(client) {
                ...
                // Set the click handler for the newly added remove icon.
                var remove_icon = Y.get('#unsubscribe-icon-' + user_name);
                if (remove_icon) {
