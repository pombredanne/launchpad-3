...
Y.bugs.portlet.subscribe('bugs:nameloaded', function(subscription) {
    ...
    error_handler.showError = function(error_msg) {
        Y.lp.display_error(Y.get('.menu-link-addsubscriber'), error_msg);
    };
...
function add_branch_to_linked_branches(branch_html) {
    ...
    existing_bug_branch_node = bug_branch_list.get(
        bug_branch_node.get('id'))
    if (!Y.Lang.isValue(existing_bug_branch_node)) {
        ...
        var anim = Y.lazr.anim.green_flash({node: existing_bug_branch_node});
    }
    anim.run();
}
