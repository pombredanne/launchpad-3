 ...
 * @param img_url {String} The URL of the icon for person or team.
 * @param can_be_unsubscribed {Boolean} Can the newly added subscription be
 *          unsubscribed by the current user?
 ...
 * the name appears depends on if the current user can unsubscribe the
 ...
 */
function add_user_name_link(name, full_name, img_url, can_be_unsubscribed) {
    ...
    if (current_user_subscribing) {
        ...
        // to have sortable lists of unsubscribable vs. not nunsubscribale
        // person links.
        ...
        if (all_subscribers) {
            ...
            if (next) {
                subscribers.insertBefore(link_node, nodes_by_name[next]);
            } else {
...
/*
 * Handle updating the UI after successfully subscribing someone else.
 *
 * This is a two step process -- get the list of subscripitions, then
 * find the current one and check canBeUnsubscribedByUser for how to handle
 * the subscription.
 *
 * @method handle_subscriptions_ui
 * @param someone_else_name {String} The subscribers username.
 * @param someone_else_display_name {String} The subscriber's display name.
 * @param someone_else_img_url {String} The URL for an icon of person or team.
 */
function handle_subscriptions_ui(
        ...
        on: {
            success: function(subscriptions) {
                ...
                lp_client.named_get(
                    ...
                    'canBeUnsubscribedByUser', final_config);
            }
        }
    };
