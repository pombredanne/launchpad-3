...
bugs.setup_me_too = function(user_is_affected) {
    ...
    if ((LP.client.links.me !== undefined) && (LP.client.links.me !== null)) {
        var me_too_edit = new Y.ChoiceSource({
            ...
            items: [
                {name: "Yes, this bug affects me", value: true,
                 style: '', help: "", disabled: false},
                {name: "No, this bug doesn't affect me", value: false,
                 style: '', help: "", disabled: false}
            ],
        ...
        me_too_edit.on('save', function(e) {
            alert(me_too_edit.get('value'));
        });
        me_too_edit.plug({
            fn: Y.lp.client.plugins.PATCHPlugin,
            cfg: {patch: 'foobar', resource: 'fred'}});
        me_too_edit.render();
    }
};
