...
Y.bugs.portlet.subscribe('bugs:nameloaded', function() {
    var subscriber = new Y.lp.Subscriber({uri: LP.client.links.me});
...
function setup_subscription_link_handlers() {
    ...
    fix_subscription_link_classes(subscriber);
    ...
    if (subscription_link) {
        subscription_link.on('click', function(e) {
            ...
            if (parent.hasClass('subscribed-false') &&
                ...
                subscribe_current_user(e.target, subscriber);
            }
            else {
                unsubscribe_current_user(e.target, subscriber);
            }
        });
        subscription_link.addClass('js-action');
    }

    setup_unsubscribe_icon_handlers(subscriber);
 ...
 */
function fix_subscription_link_classes(subscriber) {
 ...
 * @param subscriber {Object} A Y.lp.Subscriber object representing the
 *                            current user.
 */
function setup_unsubscribe_icon_handlers(subscriber) {
    var unsubscribe_icons = Y.all('.unsub-icon');
    if (unsubscribe_icons) {
        unsubscribe_icons.on('click', function(e) {
            e.halt();
            unsubscribe_user_via_icon(e.target, subscriber);
        });
    }
}
 ...
 */
function add_user_name_link(subscriber) {
    ...
    if (current_user_subscribing) {
        ...
        full_name = subscriber.get('display_name');
    } else {
        name = other_name;
 ...
 *                            current user.
*/
function unsubscribe_user_via_icon(icon, subscriber) {
    ...
    if (icon_user_name == subscriber.get('name')) {
        current_user_unsubscribing = true;
        unsubscribe_user = subscriber.get('name')
    } else {
 ...
 *                            current user.
 */
function subscribe_current_user(subscription_link, subscriber) {
 ...
 *                            current user.
 */
function unsubscribe_current_user(subscription_link, subscriber) {
    subscription_link.setStyle('display', 'none');
 ...
 *                            current user.
 */
function check_can_be_unsubscribed(subscriber) {
    var error_handler = new LP.client.ErrorHandler();
    error_handler.showError = function (error_msg) {
        display_error(Y.get('.menu-link-addsubscriber'), error_msg);
    };
...
function subscribe_someone_else(result) {
    ...
    if (already_subscribed) {
        ...
        check_can_be_unsubscribed(subscriber);
    }
}
