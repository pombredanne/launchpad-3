...
Y.bugs.setup_bugtask_index = function() {
    ...
    Y.on('load', function() {
        ...
        // Set up the 'link a related branch' overlay.
        link_branch_link = Y.get('.menu-link-addbranch');
        if (Y.Lang.isValue(link_branch_link)) {
            create_link_branch_form_overlay();

            link_branch_link.on('click', function(e) {
                if (Y.Lang.isValue(link_branch_form_overlay)) {
                    e.preventDefault();
                    link_branch_form_overlay.show();
                    link_branch_form_overlay.form_node.query(
                        '[id="field.branch"]').focus();
                }
                e.halt();
            });
...
var show_comment_on_duplicate_warning = function() {
    ...
    if (duplicate_warning === null) {
        var container = Y.get('#add-comment-form');
...
/**
 * Create the link-a-branch form overlay.
 */
function create_link_branch_form_overlay() {
    link_branch_form_overlay = new Y.lazr.FormOverlay({
        headerContent: '<h2>Link a branch</h2>',
        form_header: '',

        // We have to define the form content here rather than using
        // +addbranch/++form++ because the normal form includes a branch
        // picker, which should be taken out and shot.
        form_content:
            '<div>' +
            '  <p width="100%">' +
            '    Adding a Bazaar branch to a bug allows you to notify ' +
            '    others about where work is going on to fix this bug.' +
            '  </p>' +
            '  <p>' +
            '    <label for="field.branch">Branch</label>:' +
            '    <input type="text" class="" style="" onkeypress=""' +
            '        maxlength="" size="25" name="field.branch"' +
            '        id="field.branch" value=""/>' +
            '  </p>' +
            '</div>',
        form_submit_button: Y.Node.create(submit_button_html),
        form_cancel_button: Y.Node.create(cancel_button_html),
        centered: true,
        form_submit_callback: link_branch,
        visible: false
    });
    link_branch_form_overlay.render('#link-branch-form-container');
}

/**
 * Link a branch to the bug
 */
function link_branch(data) {
    link_branch_form_overlay.hide();
    ...
    error_handler.showError = function (error_msg) {
        Y.lazr.anim.red_flash({node: link_branch_link}).run();
        link_branch_form_overlay.showError(error_msg);
        link_branch_form_overlay.show();
    };

    function link_branch_to_bug(branch) {
        // lp:// URLs resolve to a branch's code.launchpad.net page but
        // are useless to us in this code. If the branch we've got isn't
        // something we can call .get() on we give up.
        if (!Y.Lang.isObject(branch.get)) {
            Y.lazr.anim.red_flash({node: link_branch_link}).run();
            link_branch_link.toggleClass('update-in-progress-message');
            link_branch_form_overlay.showError(
                "Branch " + branch_url + " could not be found.");
            link_branch_form_overlay.show();
        } else {
            // Otherwise we call linkBranch() on the bug and flash the
            // 'add a branch' link accordingly.
            config = {
                on: {
                    success: function(client) {
                        ...
                        // Clear the overlay form since it might be
                        // reused.
                        link_branch_form_overlay.form_node.reset();
                    },
                    failure: error_handler.getFailureHandler()
                },
                parameters: {
    ...
    var branch_url = '/' + data['field.branch'];
    config = {
        on: {
            ...
            failure: function(id, request) {
                ...
                if (request.status == 404) {
                    link_branch_form_overlay.showError(
                        "Branch " + branch_url + " doesn't exist.");
                } else {
                    link_branch_form_overlay.showError(request.responseText);
                }
                link_branch_form_overlay.show();
            }
        }
        ...
        on: {
            success: function(client) {
                ...
                anim.on('end', function(e) {
                    ...
                    subscription_link.removeClass('modify remove');
                    subscription_link.addClass('add');
                });
                anim.run();
            },
...
bugs.setup_bugtask_row = function(conf) {
    ...
    if (Y.Lang.isValue(milestone_content)) {
            ...
            inline_combo.set('value', null);
                Y.Array.each(milestone_choice_edit.get('items'), function(item) {
                    if (item.value == milestone_choice_edit.get('value')) {
                        inline_combo.queryAll('option').each(function(opt) {
...
MeTooChoiceSource.ATTRS = {
     ...
     * by HTML_PARSER.
     *
     * Setter function returns Y.get(parameter) so that you can pass
