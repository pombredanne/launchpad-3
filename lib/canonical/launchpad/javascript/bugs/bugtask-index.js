...
function set_none_for_empty_subscribers() {
    ...
    if (!subscriber_list.query('div') && !Y.get('#none-subscribers')) {
        var none_div = Y.Node.create('<div id="none-subscribers">None</div>');
        subscriber_list.appendChild(none_div);
    }

    // Check to see if the duplicate subscribers is empty, too.
    var dup_list = Y.get('#subscribers-from-duplicates');
    if (Y.Lang.isValue(dup_list) && !dup_list.query('div')) {
...
function unsubscribe_user_via_icon(icon) {
    ...
    var from_dupes;
    if (parent.hasClass('dup-subscribed-true')) {
        from_dupes = true;
    } else {
        from_dupes = false;
    }
