...
function setup_inline_commenting() {
    ...
    var file_input = Y.get(
        '#add-comment-form .fileType');
    var pristine_file_input = file_input.cloneNode();
    add_link.on('click', function(e) {
        e.halt();
        var comment_input = Y.get('#add-comment-form textarea');
        var patch_input = Y.get('#add-comment-form .checkboxType');
        var file_input = Y.get(
            '#add-comment-form .fileType');
        var inputs = Y.all('#add-comment-form input');
        var config = {
            on: {
                    ...
                    var config = {
                        on: {
                            success: function(something) {
                                ...
                                var comment = Y.Node.create(something);
                                fieldset.get('parentNode').insertBefore(
                                    comment, fieldset);
                                add_link.replaceClass(
                                    'update-in-progress-message', 'add');
                                ...
                                patch_input.set('checked', false);
                                // File inputs can't be reset, so
                                // replace it with a copy of the
                                // original (empty) one.
                                file_input.get('parentNode').replaceChild(
                                    pristine_file_input.cloneNode(),
                                    file_input);
                                toggleCollapsibleForNode(
                                    Y.Node.getDOMNode(legend));
                                Y.lazr.anim.green_flash({node: comment}).run();
                            }
                        },
                        accept: LP.client.XHTML
                    };
                    lp_client.get(something.lp_original_uri, config)
                },
