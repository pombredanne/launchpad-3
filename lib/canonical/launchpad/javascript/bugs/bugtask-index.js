 ...
 * The subscribe method cannot be attempted until this node
 * is added because this temp node has to be safely removed.
 *
 * There are a couple XHR requests in check_can_be_unsubscribed
 * so the subscribe work is done here, after bugs:nameloaded is fired.
 */
...
function build_user_link_html(name, full_name, current_user_subscribing) {
    ...
    if (is_team) {
        html = html.concat([
            '<img alt="" src="/@@/team" width="14" height="14" />',
            '&nbsp;' + full_name + '</a>']);
    } else {
        html = html.concat([
...
function get_next_subscriber_node(full_name) {
    // Pull all subscribers out from the DOM to have sortable
    // lists of unsubscribable vs. not nunsubscribale person links.
    ...
    if (all_subscribers) {
        ...
        all_subscribers.each(function(sub_link) {
            if (sub_link.getAttribute('id') != 'temp-username') {
                // The user's display name is found via a "name" attribute.
