...
function setup_unsubscribe_icon_handlers(subscription) {
    ...
    if (unsubscribe_icons) {
        unsubscribe_icons.on('click', function(e) {
            e.halt();
            unsubscribe_user_via_icon(
                e.target, subscription.get('subscriber'));
        });
    }
}
...
function unsubscribe_current_user(subscription) {
    ...
    var config = {
        on: {
            success: function(client) {
                if (subscription.isDirectSubscription() &&
                    subscription.hasDuplicateSubscriptions()) {
                ...
                } else if (subscription.isDirectSubscription() &&
                          !subscription.hasDuplicateSubscriptions()) {
