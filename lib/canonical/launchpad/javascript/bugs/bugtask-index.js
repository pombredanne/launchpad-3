...
bugs.setup_bugtask_row = function(conf) {
    ...
    if ((LP.client.links.me !== undefined) && (LP.client.links.me !== null))  {
        ...
        if (conf.user_can_edit_status) {
            var status_choice_edit = new BugStatusEdit({
                ...
                requires_confirmation: conf.status_requires_confirmation
            });
...
var BugStatusEdit = function() {
    BugStatusEdit.superclass.constructor.apply(this, arguments);
}

BugStatusEdit.NAME = 'bugstatusedit';
BugStatusEdit.NS = 'bugstatusedit';
...
Y.extend(BugStatusEdit, Y.ChoiceSource, {
    initializer: function(cfg) {
        BugStatusEdit.superclass.initializer.apply(this, arguments);
    },
    ...
    onClick: function(e) {
        ...
        this._choice_list.on("valueChosen", function(e) {
            if (that.get("requires_confirmation")) {
                // Ensure that the choicelist is hidden before the
                // confirmation box appears. Note: there must be
                // a better way to get the choice list?
                cl = Y.get("table.yui-ichoicelist-focused");
                cl.replaceClass(
                    'yui-ichoicelist-focused', 'yui-ichoicelist-hidden');
                var confirmed = confirm([
                    'Change bug status?',
                    '\n\n',
                    'Bug status is normally set by the project ',
                    'team, based on the current state of the work ',
                    'involved in diagnosing and fixing the bug.',
                    '\n\n',
                    'If you\'re not certain whether changing the status of ',
                    'this bug is correct, you can cancel and instead ',
                    'contact the project team.'].join(''));
            } else {
                var confirmed = true;
            }
            if (confirmed) {
...
function load_subscriber_ids() {
    ...
    Y.io(url, config);
                      ...
                      "lp.subscriber", "lp.errors", "bugs.status_edit"]});
