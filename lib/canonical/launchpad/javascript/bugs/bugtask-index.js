...
Y.bugs.setup_bugtask_index = function() {
    ...
    Y.on('load', function() {
        ...
        var link_branch_link = Y.get('.menu-link-addbranch');
...
function link_branch(data) {
    ...
    // First, grab the branch that's been entered.
    var branch_url = '/' + data['field.branch'];
    Y.log(branch_url);

    var error_handler = new LP.client.ErrorHandler();
    function on_success(branch) {
        Y.log(branch.get('self_link'));
        Y.log("Here...");
        config = {
            on: {
        ...
        Y.log(config);
        Y.log('Calling linkBranch');
        Y.log(lp_bug_entry.get('self_link'));
