 ...
 *
 * @param branch {Object} The branch to add to the list of linked
 *                        branches, as returned by the Launchpad API.
 */
function add_branch_to_linked_branches(branch) {
    Y.log(branch.getAtts());

    var branch_display_name = branch.get('display_name');
    var branch_status = branch.get('lifecycle_status');
    var branch_link = LP.client.normalize_uri(branch.get('self_link'));
    var status_class = 'branchstatus' + branch_status.toUpperCase();

    var bug_branch_node = Y.Node.create(
        '<div class="bug-branch-summary">' +
        '    <a class="sprite branch" ' +
        '       href="' + branch_link + '">' + branch_display_name  +
        '</a>' +
        '    (<span class="' + status_class + '">' + branch_status  +
        '</span>)' +
        '    <a href="$foo/+bug/15/+delete" title="Remove link">' +
        '      <img src="/@@/remove"/> ' +
        '    </a> ' +
        '</div>');
    ...
    if (!Y.Lang.isValue(bug_branch_list)) {
        ...
        var bug_branch_container = Y.get('#bug-branches-container');
        bug_branch_container.appendChild(bug_branch_list);

        var anim = Y.lazr.anim.green_flash({node: bug_branch_list});
    } else {
        var anim = Y.lazr.anim.green_flash({node: bug_branch_node});
    }
...
bugs.setup_bugtask_row = function(conf) {
    ...
    var assignee_content = Y.get('#assignee-picker-' + conf.row_id);
            ...
            if (conf.target_is_product) {
                if  (Y.UA.webkit) {
                    bugtarget_content.replaceChild(
                        ...
                        bugtarget_content.query('.yui-activator-act'));
                } else {
    ...
    if (Y.Lang.isValue(milestone_content) && conf.user_can_edit_milestone) {
        ...
        milestone_choice_edit.after('save', function() {
            ...
            if (Y.Lang.isValue(new_value)) {
                ...
                // fix this.
                milestone_content.query('.value').setAttribute(
        ...
        milestone_content.query('.nulltext').on(
            'mousedown',
    ...
    if (Y.Lang.isValue(assignee_content)) {
        var assignee_picker = Y.lp.picker.addPickerPatcher(
            ...
            {"step_title": "Search for people or teams",
                ...
                "remove_button_text": "Remove asignee",
