...
Y.bugs.setup_bugtask_index = function() {
    ...
    Y.on('load', function() {
        ...
        if (Y.Lang.isValue(link_branch_link)) {
            var link_branch_url =
                link_branch_link.getAttribute('href') + '/++form++';
            var link_branch_form_id = Y.io(link_branch_url);
            form_load_callbacks[link_branch_form_id.id] =
            ...
            link_branch_link.on('click', function(e) {
                if (Y.Lang.isValue(link_branch_form_overlay)) {
                    ...
                    document.getElementById('field.branch').focus();
                }
                e.halt();
            });
 ...
 * @param form_content {String} The HTML data of the form overlay.
 */
function create_link_branch_form_overlay(form_content) {
    ...
    link_branch_form_overlay.render('#link_branch-form-container');
    var node = Y.get('#form-container');
}
...
function link_branch(data) {
    ...
    // Create a "linking branch" node.
    var add_branch_container = Y.get('#add-branch-container');
    var linking_branch_text = Y.Node.create(
        '<span><img src="/@@/spinner" /> Linking branch</span>');

    var error_handler = new LP.client.ErrorHandler();

    error_handler.clearProgressUI = function () {
        add_branch_container.removeChild(linking_branch_text);
        link_branch_link.setStyle('display', 'block');
    };
    error_handler.showError = function (error_msg) {
        Y.lazr.anim.red_flash({node: add_branch_container}).run();
        link_branch_form_overlay.showError(error_msg);
        link_branch_form_overlay.show();
    };

    function on_success(branch) {
        config = {
            on: {
                success: function(client) {
                    var anim = Y.lazr.anim.green_flash(
                        {node: add_branch_container});
                    anim.on('end', function(e) {
                        add_branch_container.removeChild(
                            linking_branch_text);
                        link_branch_link.setStyle('display', 'block');
                    });
                    anim.run();
    ...
    link_branch_link.setStyle('display', 'none');
    add_branch_container.appendChild(linking_branch_text);
