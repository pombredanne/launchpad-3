...
bugs.setup_me_too = function(user_is_affected) {
    ...
    if ((LP.client.links.me !== undefined) && (LP.client.links.me !== null)) {
        var me_too_edit = new Y.ChoiceSource({
            contentBox: me_too_content,
            value: user_is_affected,
        ...
        me_too_edit.showError = function(err) {
            display_error(null, err);
        };
        // Insert a flame icon before the text if the user is affected
        // by this bug.
        Y.Do.after(
            function() {
                if (this.get('value')) {
                    var value_location = this.get('value_location');
                    var value_html = value_location.get('innerHTML');
                    var flame_icon = ('<img width="14" height="14" ' +
                                      'src="/@@/flame-icon" alt="" />');
                    value_location.set(
                        'innerHTML', flame_icon + ' ' + value_html);
                }
            },
            me_too_edit, 'syncUI', me_too_edit
        );
        ...
        Y.Do.after(
            ...
            me_too_edit, 'render', me_too_edit
        );
        me_too_edit.plug({fn: MeTooPlugin});
        me_too_edit.render();
    }
};

function MeTooPlugin() {
    MeTooPlugin.superclass.constructor.apply(this, arguments);
}
...
Y.extend(MeTooPlugin, Y.Plugin, {

    extra_config: null,

    initializer: function(config) {
        // Save the config object that the user passed in so that we
        // can pass any extra parameters through to the LP.client
        // constructor.
        ...
        // We want to run our markUserAffected code instead of the
        // original 'save' method.  Using doBefore() means that
        // unplugging our code will leave the original widget in a
        // clean state.
        this.doBefore("_saveData", this.markUserAffected);
    ...
    markUserAffected: function() {
        ...
        var value = owner.getInput();
        var client =  new LP.client.Launchpad();

        var cfg = Y.merge(this.extra_config, {
            on: {
                success: function(entry) {
                    owner._uiClearWaiting();
                    original_save.apply(owner, [value]);
                },
        ...
        client.named_post(
            LP.client.cache.bug.self_link, 'markUserAffected', cfg);
