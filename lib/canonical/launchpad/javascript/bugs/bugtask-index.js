...
function setup_subscribe_someone_else_handler() {
    ...
    subscription_link_someone_else.on('click', function(e) {
        e.halt();
        subscription_link_someone_else.setStyle('display', 'none');
...
function build_user_link_html(
    ...
    if (current_user_subscribing) {
        html = html.concat([
            '<a href="/~' + name + '" title="Subscribed themselves">']);
    } else {
        html = html.concat([
            '<a href="/~' + name + '" title="Subscribed by ' +
            display_name + '">']);
    }
 ...
 *
 * Returns the next node in the sort order for the subscribers
 * list based on the currently subscribed node.
 *
 * @method get_next_subscriber_node
 */
function get_next_subscriber_node(
    key, can_be_unsubscribed, unsubscribables, not_unsubscribables) {

    if (!unsubscribables && !not_unsubscribables) {
        // If neither list exists, this will be appended anyway.
        // No node reference is needed, just return.
        return;
    } else if (
        ...
        !can_be_unsubscribed && unsubscribables && !not_unsubscribables) {
        // If the user belongs in the second list, but the second list
        // doesn't exist, just return.  No node reference is needed.
        return;
        ...
        for (var i=0; i<not_unsubscribables.length; i++) {
            if (not_unsubscribables[i] == key) {
                if (i+1 < not_unsubscribables.length) {
                    return not_unsubscribables[i+1];
                } else if (i+1 >= not_unsubscribables.length) {
                    return;
                }
            }
        }
...
function add_user_name_link(
    ...
    if (current_user_subscribing) {
        ...
        all_subscribers.each(function(sub_link) {
            var sub_link_name = sub_link.getAttribute('id');
        ...
        // Use the ID form of "subscriber-NAME" as a sort key.
        var name_id = 'subscriber-' + name;
        if (can_be_unsubscribed) {
            unsubscribables.push(name_id);
        } else {
            not_unsubscribables.push(name_id);
        }
        unsubscribables.sort();
        not_unsubscribables.sort();
        var next = get_next_subscriber_node(
            name_id, can_be_unsubscribed, unsubscribables,
            not_unsubscribables);
...
function subscribe_someone_else(result) {
    ...
    var config = {
        on: {
            success: function() {
                spinner.setStyle('display', 'none');
                subscription_link_someone_else.setStyle('display', 'block');
