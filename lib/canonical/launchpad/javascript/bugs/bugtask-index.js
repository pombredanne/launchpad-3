...
function build_user_link_html(subscription) {
    ...
    if (subscription.isCurrentUserSubscribing()) {
...
function get_next_subscriber_node(subscription) {
    var full_name = subscription.get('person').get('full_display_name');
    var can_be_unsubscribed = subscription.canBeUnsubscribedByUser();
    var nodes_by_name = {};
    var unsubscribables = [];
 ...
 */
function subscribe_someone_else(result, config) {
    ...
    if (subscription.isAlreadySubscribed()) {
