...
function fix_subscription_link_classes(subscription) {
    ...
    var me_nodes = Y.all('.subscriber-' + subscriber.get('escaped_name'));
...
function build_user_link_html(subscription) {
    ...
    var escaped_name = subscription.get('person').get('escaped_name');
    ...
    var html = Y.Node.create(
        ...
        '&nbsp;</a></div>');
    html.addClass('subscriber-' + terms.escaped_name);
    ...
    if (subscription.can_be_unsubscribed_by_user()) {
        ...
        icon_html
            .set('id', 'unsubscribe-' + terms.escaped_name)
            .set('title', 'Unsubscribe ' + terms.full_name);
        icon_html.query('img')
            .set('id', 'unsubscribe-icon-' + terms.escaped_name);
...
function add_user_name_link(subscription) {
    ...
    if (subscription.can_be_unsubscribed_by_user()) {
        var remove_icon = Y.get('#unsubscribe-icon-' + person.get('escaped_name'));
...
function unsubscribe_user_via_icon(icon, subscription) {
    ...
    var dupe_id = 'dupe-subscriber-' + person.get('escaped_name');
    ...
    error_handler.clearProgressUI = function () {
        ...
        var unsubscribe_icon = Y.get(
            '#unsubscribe-icon-' + person.get('escaped_name'));
    ...
    error_handler.showError = function (error_msg) {
        var flash_node = Y.get('.subscriber-' + person.get('escaped_name'));
            ...
            success: function(client) {
                ...
                anim.on('end', function(e) {
                    ...
                    var person_link = Y.get(
                        '.subscriber-' + person.get('escaped_name'));
    ...
    var config = {
        on: {
            success: function(client) {
                ...
                var flash_node = Y.get(
                    '.subscriber-' + subscriber.get('escaped_name'));
