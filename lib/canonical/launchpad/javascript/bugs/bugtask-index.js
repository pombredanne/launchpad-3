    ...
    items: {
        value: [
            { name: 'Yes, it affects me', value: true,
              source_name: 'This bug affects you',
              disabled: false },
            { name: "No, it doesn't affect me", value: false,
              source_name: "This bug doesn't affect you",
              disabled: false }
        ]
    },
...
Y.extend(MeTooChoiceSource, Y.ChoiceSource, {
    initializer: function() {
        ...
        if (others_affected_count !== null) {
            var new_source_names =
                this._getNewSourceNames(others_affected_count);
            // Update source_names.
            Y.each(this.get('items'), function(item) {
                if (item.value in new_source_names) {
                    item.source_name = new_source_names[item.value];
                }
            });
        }
    },

    _getNewSourceNames: function(others_affected_count) {
        var new_source_names = {};
        // What to say when the user is marked as affected.
        if (others_affected_count == 1) {
            new_source_names[true] = (
