...
Y.on('load', function() {
    ...
    handle_unsubscribe_icon(subscription_link);

}, window);


/*
 * Hook up the unsubscribe icon for the current user,
 * if available.
*/
function handle_unsubscribe_icon(subscription_link) {
    var unsubscribe_icon = Y.get('#unsubscribe-icon-' + me.substring(2));
    if (unsubscribe_icon) {
        unsubscribe_icon.on('click', function(e) {
            e.halt();
            unsubscribe_current_user(subscription_link);
        });
    }
}
...
function subscribe_current_user(subscription_link) {
    ...
    lp_client.named_post(bug.self_link, 'subscribe', sub_config)
    var html = '<div id=\'subscriber-name16\'>' +
            '<a href=\'/%7Ename16\' title=\'Subscribed themselves\'>' +
              '<img alt=\'\' src=\'/@@/person\' width=\'14\' height=\'14\' />' +
              '&nbsp;Foo Bar&nbsp;' +
            '</a>' +
            '<a href=\'+subscribe\' id=\'unsubscribe-name16\' title=\'Unsubscribe Foo Bar\'>' +
              '<img src=\'/@@/remove\' id=\'unsubscribe-icon-name16\' />' +
            '</a>' +
        '</div>';
    ...
    handle_unsubscribe_icon(subscription_link);
    flash_node = Y.get('#subscriber-' + me.substring(2));
    anim = Y.lazr.anim.green_flash({ node: flash_node });
...
function unsubscribe_current_user(subscription_link) {
    ...
    spinner.setStyle('display', 'block');
    lp_client.named_post(bug.self_link, 'unsubscribe', unsub_config)
