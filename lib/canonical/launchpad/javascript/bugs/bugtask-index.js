...
/*
 * Page inline subscribe and unsubscribe from a bugtask.
 */
YUI({base: yui_base + '/'}).use(
    'io-base', 'node', 'lazr.anim', function(Y) {

/*
 * The initially hidden spinner element.
 */
var spinner;

/*
 * The LP API client, used for submitting the subscription.
 */
var lp_client;

/*
 * vars for tracking context across the object.
 */
var bug;
var me;
var display_name;
...
/*
 * Get the subscribe/unsubscribe links and icons,
 * and setup click event handlers.
 */
Y.on('load', function() {
    // If the user is not logged in, then we need to defer to the
    // default behaviour.
    ...
    config = {
        on: {
            ...
            failure: function(args) {
                alert('Yo, do something with this me error.');
            }
        }
    }
    ...
    if (unsubscribe_icon) {
        unsubscribe_icon.on('click', function(e) {
            e.halt();
            unsubscribeUserViaIcon(e.target, subscription_link);
        });
    }
}, window);
...
function unsubscribe_user_via_icon(icon, subscription_link) {
    ...
    var config = {
        on: {
            ...
            failure: function(some_int, response, args) {
                alert('UNSUB FAIL: yo, do something with an overlay here.');
            }
        },
    ...
    lp_client.named_post(bug.self_link, 'unsubscribe', config);

    var me_node = Y.get('#subscriber-' + me.substring(2));
    var parent = me_node.get('parentNode');
    var flash_node = Y.get('#subscriber-' + me.substring(2));
...
function subscribe_current_user(subscription_link) {
    ...
    var config = {
        on: {
            ...
            failure: function(some_int, response, args) {
                alert('SUB FAIL: yo, do something with an overlay here.');
            }
        },
    ...
    lp_client.named_post(bug.self_link, 'subscribe', config);

    var short_name = me.substring(2);
    var html = [
        '<div id="subscriber-' + short_name + '">',
        '<a href="' + me + '" title="Subscribed themselves">',
        '<img alt="" src="/@@/person" width="14" height="14" />',
        '&nbsp;' + display_name + '&nbsp;</a>',
        '<a href="+subscribe" id="unsubscribe-' + short_name,
        '" title="Unsubscribe ' + display_name + '">',
        '<img src="/@@/remove" id="unsubscribe-icon-' + short_name + '" />',
        '</a></div>'
    ].join('');
    ...
    // Set up the click handler for the newly added unsubscribe minus icon.
...
/*
 * Unubscribe the current user via the LP API.
 */
