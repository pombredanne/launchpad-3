...
YUI().use("attribute", "io-base", "json-parse", "json-stringify", function(Y) {
...
LP.client.normalize_uri = function(uri) {
    ...
    if (host_start != -1) {
        ...
        // eg. "http://www.example.com/api/beta/foo";
...
LP.client.wrap_resource_on_success = function(ignore, response, args) {
    ...
    if (old_on_success) {
        ...
        if (media_type == 'application/json') {
            ...
            representation = LP.client.parse_xml_entry(response.responseText);
            wrapped = client.wrap_resource(uri, representation);
            return old_on_success(wrapped);
        } else {
...
LP.client.parse_xml_entry = function (xml) {
    var xml_doc;
    try {
        //Internet Explorer
    ...
    for (var i=0; i<xml_doc.childNodes.length; i++) {
        var element = xml_doc.childNodes[i];
        if (element.localName == 'dl') {
            for (var j=0; j<element.childNodes.length; j++) {
                var subelement = element.childNodes[j];
                if (subelement.localName == 'dt') {
                    var attribute_name = subelement.innerHTML;
                } else if (subelement.localName == 'dd' && attribute_name) {
                    representation[attribute_name] = subelement.innerHTML;
                }
            }
        }
    }
...
LP.client.Entry = function(client, representation, uri) {
    ...
    for (key in representation) {
        if (representation.hasOwnProperty(key)) {
            this.addAtt(key, {value: representation[key]});
...
Y.extend(PATCHPlugin, Y.Plugin, {
    ...
    doPATCH: function() {
        ...
        patch_payload[attribute] = owner.getInput();
        client.patch(this.get('resource'), patch_payload, {
            on: {
                success: function (entry) {
                    owner._uiClearWaiting();
                    var new_value = entry.get(attribute);
                ...
                failure: function (ioId, o) {
                    ...
                    // If it was a timeout...
                    if(o.status == 503) {
                        owner.showError('Timeout error, please try again in a few minutes.');
                    // If it was a server error...
                        ...
                        owner.showError(server_error);
                    // Otherwise we send some sane text as an error
                    } else {
                        owner.showError(o.responseText);
                    }
              }}});
