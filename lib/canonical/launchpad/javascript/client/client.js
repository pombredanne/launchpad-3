...
LP.client.update_cache = function(entry) {
  // Now what?
  // ARSE: we need to check all the entries in the LP.client.cache.
  var original_uri = entry.lp_original_uri;
  var full_uri = LP.client.get_absolute_uri(original_uri);
  Y.log(entry);
  Y.log('full_uri: ' + full_uri);
  for (var name in LP.client.cache) {
    var cached_object = LP.client.cache[name];
  ...
  var old_context = LP.client.cache['context'];
  for (var name in old_context) {
    var old_value = old_context[name];
    var new_value = entry.get(name);
    if (name == 'lp_html') {
      for (var html_name in old_value) {
        Y.log('lp_html:' + html_name);
        Y.log('old: ' + old_value[html_name]);
        Y.log('new: ' + new_value[html_name]);
      }
    }
    else {
      Y.log(name);
...
LP.client.wrap_resource_on_success = function(ignore, response, args) {
    ...
    if (old_on_success) {
        ...
        if (media_type.substring(0,16) == 'application/json') {
            ...
            LP.client.update_cache(wrapped);
            return result;
        } else {
...
LP.client.Entry.prototype.getHTML = function(key) {
