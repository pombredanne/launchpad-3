...
/* If the argument is a YUI object, log all the attributes accessible via
 * o.get(key). Otherwise, log the normal attributes accessible via o[key].
...
LP.log_object = function(o, name) {
    ...
    if (typeof o == 'string' || typeof o == 'function') {
        result = format(o);
    } else if (o.getAttrs === undefined) {
        var items = [];
        for (var key in o) {
            items.push(key + '=' + format(o[key]));
        }
        result = '(direct-attributes)\n  ' + items.join(',\n  ');
    } else {
        var items = [];
        for (var key in o.getAttrs) {
            items.push(key + '=' + format(o.get(key)));
        }
