...
Y.extend(PATCHPlugin, Y.Plugin, {
    ...
    initializer: function(config) {
        if (!Y.Lang.isString(config.name)) {
            Y.fail("missing config: 'name' of the attribute");
        }

        if (!Y.Lang.isString(config.url)) {
            Y.fail("missing config: 'url' of the object");
        }
    ...
    doPATCH: function() {
        ...
        var patch_payload = {};
        patch_payload[this.get('name')] = owner.getInput();
        client.patch(this.get('url'), patch_payload, {
            on: {
                success: function () {
                    owner._uiClearWaiting();
                    original_save.apply(owner);
                },
                failure: function (ioId, o) {
                    Y.log('PATCH failure:' + o.responseText, 'error');
                    owner._uiClearWaiting();
