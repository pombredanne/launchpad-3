...
function update_cached_object(cache_name, cache, entry)
{
  ...
  if (fields_changed.length > 0) {
    ...
    Y.log('Firing <' + event_name + '>');
    //var publisher = new Y.EventTarget();
    //p/ublisher.name = event_name;
    //publisher.publish(event_name, { broadcast: 2 });
    //publisher.fire(entry, fields_changed)
    //Y.log(publisher);
    Y.fire(event_name, fields_changed, entry);
    Y.log('Fired <' + event_name + '>');
  }
}
...
}, "0.1", {"requires": ["event-custom", "plugin", "dump", "lazr.editor"]});
