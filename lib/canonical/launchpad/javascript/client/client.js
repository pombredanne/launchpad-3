...
LP.client.Entry.prototype.lp_save = function(config) {
    ...
    this.lp_client.patch(uri, representation, {on: on},
                         headers);
    this.dirty_attributes = [];
};
...
Y.extend(PATCHPlugin, Y.Plugin, {
    ...
    doPATCH: function() {
        ...
        var patch_payload = {};
        patch_payload[this.get('patch')] = owner.getInput();
        client.patch(this.get('resource'), patch_payload, {
            on: {
                success: function () {
                    owner._uiClearWaiting();
                    // XXX flacoste 2009/02/11 Use the actual updated value
                    // from the server here.
                    original_save.apply(owner, [owner.getInput()]);
                },
