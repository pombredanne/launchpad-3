...
YUI().use("node", "lazr.anim", "anim", function(Y) {
     ...
     * when passed the DOM subtree.
     */
    var shouldStopUpdatingStatusSummary = function(table_node){
     ...
     */
    var getBuildSummaryParameters = function(table_node){
     ...
     */
    var shouldStopUpdatingSourcePkgsTable = function(table_node){
        // Stop updating only when there aren't any sources to update:
        return getBuildSummaryParameters(table_node) == null;
    };
    ...
    Y.on("contentready", function(){
        ...
        var config = {
            ...
            parameter_evaluator_function: getBuildSummaryParameters,
            stop_updates_check: shouldStopUpdatingSourcePkgsTable,
            interval: 3000
        };
        table.plug(LP.DynamicDomUpdater, config);
    }, "table#packages_list");
