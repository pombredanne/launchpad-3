/** Copyright (c) 2008, Canonical Ltd. All rights reserved.
 *
 * The Launchpad DynamicDomUpdater module provides a plugin class that
 * can be plugged in to a DOM subtree, so that the subtree can update itself
 * regularly using the Launchpad API.
 *
 * @module DynamicDomUpdater
 * @requires yahoo, node, plugin, LP
 */
YUI.add('soyuz.dynamic_dom_updater', function(Y) {

    /**
     * The DomUpdater class provides the ability to plugin functionality
     * to a DOM subtree so that it can update itself when given data in an
     * expected format.
     *
     * For example:
     *     var table = Y.get('table#build-count-table');
     *     var config = {
     *         domUpdateFunction: updateArchiveBuildStatusSummary
     *     }
     *     table.plug(LP.DomUpdater, config);
     *
     *     // Now updating the table is as simple as:
     *     table.updater.update({total:3, failed: 1});
     *
     * @class DomUpdater
     * @extends Plugin
     * @constructor
     */
    var DomUpdater = function(config){
        DomUpdater.superclass.constructor.apply(this, arguments);
    };

    DomUpdater.NAME = 'domupdater';
    DomUpdater.NS = 'updater';

    DomUpdater.ATTRS = {
        /**
         * The dom node to which this plugin instance is attached.
         *
         * @attribute owner
         * @type Node
         */
        owner: {
            value: null
        },

        /**
         * The function that updates the owner's dom subtree.
         *
         * @attribute domUpdateFunction
         * @type Function
         * @default null
         */
        domUpdateFunction: {
            value: null
        }
    };

    Y.extend(DomUpdater, Y.Plugin, {

        /**
         * The update method simply passes through to the user provided
         * update function.
         *
         * @method update
         * @param update_data {Object} The user defined data that will be
         *        passed to the users domUpdateFunction.
         */
        update: function(update_data) {
            Y.log("Updating Dom subtree for " + this.get("owner"),
                  "info", "DomUpdater");
            var domUpdateFunction = this.get("domUpdateFunction");
            if (domUpdateFunction !== null){
                domUpdateFunction(this.get("owner"), update_data);
            }
        }

    });


    /**
     * The base class for DynamicDomUpdater and ApiDynamicDomUpdater.
     *
     * @class BaseDynamicDomUpdater
     * @extends DomUpdater
     * @constructor
     */
    var BaseDynamicDomUpdater = function(config){
        BaseDynamicDomUpdater.superclass.constructor.apply(this, arguments);
    };

    BaseDynamicDomUpdater.NAME = 'basedynamicdomupdater';
    BaseDynamicDomUpdater.NS = 'baseupdater';
    BaseDynamicDomUpdater.ATTRS = {
        /**
         * The uri to use for the LP.get request.
         *
         * @attribute uri
         * @type String
         */
        uri: {
            value: null
        },

        /**
         * The interval (in ms) with which the subtree should be updated.
         *
         * @attribute interval
         * @type NUM
         * @default 60000
         */
        interval: {
            value: 60000
        },

        /**
         * The function used to determine whether updates should stop.
         *
         * If it is not included, we use a default function that always
         * returns false so the updates will continue infinitely.
         *
         * Once this function returns true, updates will stop and not
         * be restarted.
         *
         * @attribute stopUpdatesCheckFunction
         * @type Function
         */
        stopUpdatesCheckFunction: {
            value: function(data){return false;}
        },

        /**
         * A function to be called when it fails to retrieve the dynamic
         * data required for the update.
         *
         * @attribute updateFailed
         * @type Function
         */
        updateFailed: {
            value: function(dom_object){return;}
        }

    };

    Y.extend(BaseDynamicDomUpdater, DomUpdater, {
        /**
         * The initializer method that is called from the base Plugin class.
         *
         * @method initializer
         * @protected
         */
        initializer: function(){
            Y.log("Initializing updater for " + this.get("owner") +
                  " with an interval of " + this.get("interval") + "ms.",
                  "info", "LPDynamicDomUpdater");

            this.initialize();

            // Finally, setup our timeout interval to update ourselves.
            this.intervalID = setInterval(
                Y.bind(this.startUpdate, this),
                this.get('interval')
                );
        },

        /**
         * Local initialization code, called by
         * BaseDynamicDomUpdater.initializer.
         *
         * @method initialize
         */
	initialize: function(){
            return;
        },

        /**
         * Called for staring a update chain:
         *
         * Check Interval -> Fetch data -> Update
         *
         * It probes 'stopUpdatesCheckFunction' before dispatching the
         * 'dynamicUpdate'.
         *
         * @method initializer
         * @protected
         */
        startUpdate: function () {
            Y.log("Starting update for " + this.get("owner"),
                  "info", "LP.DynamicDomUpdater");
            var uri = this.get("uri");

            // Check whether we should stop updating now...
            if (this.get("stopUpdatesCheckFunction")(this.get("owner"))){
                Y.log(
                    "Cancelling updates for " + this.get("owner") +
                    " after stopUpdatesCheckFunction returned true.", "info",
                    "LP DynamicDomUpdater");
                clearInterval(this.intervalID);
                return;
            }

            // Finally, call the LP api method as required...
            if (!uri){
                return;
            }

            this.dynamicUpdate();
	},

        /**
         * Failure handler for the call to XHR or LP API.
         *
         * @method _handleFailure
         * @private
         */
        _handleFailure: function(id, request) {
            Y.log("LP.DynamicDomUpdater for " + this.get("owner") +
                  " failed to get dynamic data. (" +
                  id + ', ' + request + ")");
	    this.get("updateFailed")(this.get("owner"));
        }

    });


    /**
     * The DynamicDomUpdater class provides the ability to plug
     * functionality into a DOM subtree so that it can update itself
     * using an plain XHR method
     *
     * For example:
     *     var size_section = Y.get('#package_counters');
     *     var config = {
     *         domUpdateFunction: updateArchiveRepositorySize,
     *         uri: '+repository-size',
     *     }
     *     size_section.plug(LP.DynamicDomUpdater, config);
     *
     * Once configured, the 'size_section' dom subtree will now update itself
     * by calling the user defined domUpdateFunction (with a default interval
     * of 60000ms) with the result of the LPs api call.
     *
     * @class DynamicDomUpdater
     * @extends BaseDynamicDomUpdater
     * @constructor
     */
    LP.DynamicDomUpdater = function(config){
        LP.ApiDynamicDomUpdater.superclass.constructor.apply(this, arguments);
    };
    LP.DynamicDomUpdater.NAME = 'dynamicdomupdater';
    LP.DynamicDomUpdater.NS = 'updater';

    Y.extend(LP.DynamicDomUpdater, BaseDynamicDomUpdater, {
        /**
         * See BaseDynamicDomUpdater.initialize.
         *
         * @method initialize
         */
        initialize: function(){
            // Create the configuration for the LP client request:
            this._xhr_config = {
                on: {
                    success: Y.bind(this._handleSuccess, this),
                    failure: Y.bind(this._handleFailure, this)
                }
            };

        },

        /**
         * The dynamicUpdate method is responsible for updating the DOM
         * subtree with data from a dynamic source.
         *
         * @method dynamicUpdate
         */
        dynamicUpdate: function() {
            var uri = this.get("uri");
            Y.io(uri, this._xhr_config);
        },

        /**
         * Success handler for the call to the LP API.
         *
         * @method _handleSuccess
         * @private
         */
	 _handleSuccess: function(id, response) {
            Y.log("Data received for " + this.get("owner"),
                   " info", "LP.DynamicDomUpdater");
            // Call our parent class's update method to update the DOM
            // subtree with the returned data.
            this.update(response);
        }
    });

    /**
     * The ApiDynamicDomUpdater class provides the ability to plug
     * functionality into a DOM subtree so that it can update itself
     * using an LP api method.
     *
     * For example:
     *     var table = Y.get('table#build-count-table');
     *     var config = {
     *         domUpdateFunction: updateArchiveBuildStatusSummary,
     *         uri: LP.client.cache.context.self_link,
     *         api_method_name: 'getBuildCounters'
     *     }
     *     table.plug(LP.ApiDynamicDomUpdater, config);
     *
     * Once configured, the 'table' dom subtree will now update itself
     * by calling the user defined domUpdateFunction (with a default interval
     * of 60000ms) with the result of the LPs api call.
     *
     * @class ApiDynamicDomUpdater
     * @extends BaseDynamicDomUpdater
     * @constructor
     */
    LP.ApiDynamicDomUpdater = function(config){
        LP.ApiDynamicDomUpdater.superclass.constructor.apply(this, arguments);
    };
    LP.ApiDynamicDomUpdater.NAME = 'apidynamicdomupdater';
    LP.ApiDynamicDomUpdater.NS = 'apiupdater';
    LP.ApiDynamicDomUpdater.ATTRS = {
        /**
         * The LP client to use. If none is provided, one will be
         * created during initialization.
         *
         * @attribute lp_client
         */
        lp_client: {
            value: null
        },

        /**
         * The LP api method name (if applicable).
         *
         * @attribute api_method_name
         * @type String
         */
        api_method_name: {
            value: null
        },

        /**
         * The function that provides the parameters for the API call.based
         * on the current state of the subtree.
         *
         * If this is not specified, no parameters will be included.
         *
         * @attribute parameterEvaluatorFunction
         * @type Function
         */
        parameterEvaluatorFunction: {
            value: null
        }
    };

    Y.extend(LP.ApiDynamicDomUpdater, BaseDynamicDomUpdater, {
        /**
         * See BaseDynamicDomUpdater.initialize.
         *
         * @method initialize
         */
        initialize: function(){
            // Create the configuration for the LP client request:
            this._lp_api_config = {
                on: {
                    success: Y.bind(this._handleSuccess, this),
                    failure: Y.bind(this._handleFailure, this)
                }
            };

            // If we have not been provided with a Launchpad Client, then
            // create one now:
            if (null === this.get("lp_client")){
                // Create our own instance of the LP client.
                this.set("lp_client", new LP.client.Launchpad());
            }

        },

        /**
         * The dynamicUpdate method is responsible for updating the DOM
         * subtree with data from a dynamic source.
         *
         * @method dynamicUpdate
         */
        dynamicUpdate: function() {
            var uri = this.get("uri");

            // Set any parameters for the API call:
            var parameterEvaluatorFunction = this.get(
                "parameterEvaluatorFunction");

            if (parameterEvaluatorFunction !== null){
                this._lp_api_config.parameters = parameterEvaluatorFunction(
                    this.get("owner"));
            }

            if (api_method_name){
                this.get("lp_client").named_get(uri,
                    api_method_name, this._lp_api_config);
            }
            else {
                this.get("lp_client").get(uri, this._lp_api_config);
            }
        },

        /**
         * Success handler for the call to the LP API.
         *
         * @method _handleSuccess
         * @private
         */
        _handleSuccess: function(data, something) {
            Y.log("Data received for " + this.get("owner"),
                   " info", "LP.ApiDynamicDomUpdater");
            // Call our parent class's update method to update the DOM
            // subtree with the returned data.
            this.update(data);
        }
    });

}, '0.1', {requires:['node', 'plugin', 'io-base']});
