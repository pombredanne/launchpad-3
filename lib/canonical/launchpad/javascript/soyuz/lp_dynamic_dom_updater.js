 ...
 *
 * @module DynamicDomUpdater, ApiDynamicDomUpdater
 * @requires yahoo, node, io-base, plugin, LP
 */
YUI.add('soyuz.dynamic_dom_updater', function(Y) {
    ...
    /**
     * The DynamicDomUpdater class provides the ability to plug
     * functionality into a DOM subtree so that it can update itself
     * using an plain XHR method
     *
     * For example:
     *     var size_section = Y.get('#package_counters');
     *     var config = {
     *         domUpdateFunction: updateArchiveRepositorySize,
     *         uri: '+repository-size',
     *     }
     *     size_section.plug(LP.DynamicDomUpdater, config);
     *
     * Once configured, the 'size_section' dom subtree will now update itself
     * by calling the user defined domUpdateFunction (with a default interval
     * of 60000ms) with the result of the LPs api call.
     *
    ...
    LP.DynamicDomUpdater.ATTRS = {
        ...
        /**
         * A function to be called when it fails to retrieve the dynamic
         ...
         */
        updateFailed: {
            value: function(dom_object){return;}
        }
    ...
    Y.extend(LP.DynamicDomUpdater, DomUpdater, {
        ...
        initializer: function(){
            ...
            // Create the configuration for XHR or LP API requests.
            this._xhr_config = {
                on: {
            ...
            this.intervalID = setInterval(
                Y.bind(this.startUpdate, this),
                this.get('interval')
                );
        },

        /**
         * Local initialization code, called by DynamicDomUpdater.initializer.
         *
         * @method initialize
         */
	initialize: function(){
        ...
        /**
         * Called for starting a update chain:
         *
         * Check Interval -> Fetch data -> Update
         *
         * It probes 'stopUpdatesCheckFunction' before dispatching the
         * 'dynamicUpdate'.
         *
         * @method initializer
         * @protected
        ...
        startUpdate: function () {
            ...
            if (this.get("stopUpdatesCheckFunction")(this.get("owner"))){
                Y.log(
                    "Cancelling updates for " + this.get("owner") +
                    " after stopUpdatesCheckFunction returned true.",
                    "info", this.NAME);
        ...
        dynamicUpdate: function() {
            var uri = this.get("uri");
            Y.io(uri, this._xhr_config);
        },

        /**
         * Success handler for the plain XHR call.
         *
         * @method _handleSuccess
         * @private
         */
	_handleSuccess: function(id, response) {
            Y.log("Data received for " + this.get("owner"),
                   "info", this.NAME);
            // Call our parent class's update method to update the DOM
            // subtree with the returned response.
            this.update(response);
	},

        /**
         * Failure handler for the call to XHR or LP API.
         *
         * @method _handleFailure
         * @private
         */
        _handleFailure: function(id, request) {
            Y.log("Failed to get dynamic data for " + this.get("owner") +
                  " (" + id + ', ' + request + ")",
                  "info", this.NAME);
            // Call user-defined method for dealing with failures passing
            // the response object.
	    this.get("updateFailed")(this.get("owner"));
        }
    ...
    /**
     * The ApiDynamicDomUpdater class provides the ability to plug
     * functionality into a DOM subtree so that it can update itself
     * using an LP api method.
     *
     * For example:
     *     var table = Y.get('table#build-count-table');
     *     var config = {
     ...
     *     }
     *     table.plug(LP.ApiDynamicDomUpdater, config);
     *
     * Once configured, the 'table' dom subtree will now update itself
     * by calling the user defined domUpdateFunction (with a default interval
     * of 60000ms) with the result of the LPs api call.
     *
     * @class ApiDynamicDomUpdater
     * @extends BaseDynamicDomUpdater
     * @constructor
     */
    LP.ApiDynamicDomUpdater = function(config){
        LP.ApiDynamicDomUpdater.superclass.constructor.apply(this, arguments);
    };
    LP.ApiDynamicDomUpdater.NAME = 'apidynamicdomupdater';
    LP.ApiDynamicDomUpdater.NS = 'apiupdater';
    LP.ApiDynamicDomUpdater.ATTRS = {
        /**
    ...
    Y.extend(LP.ApiDynamicDomUpdater, LP.DynamicDomUpdater, {
        /**
         * See DynamicDomUpdater.initialize. Creates a LP client instance
         * if it wasn't passed.
         *
         * @method initialize
         */
        ...
        dynamicUpdate: function() {
            ...
            if (api_method_name){
                this.get("lp_client").named_get(uri,
                    api_method_name, this._xhr_config);
            }
            else {
                this.get("lp_client").get(uri, this._xhr_config);
            }
        },

        /**
         * Success handler for the call to the LP API.
         * It has to be refined because API responses do not return the usual
         * XHR arguments (transaction_id, response_object). Maybe it should
         * be fixed in the LP API JS client.
         *
         * @method _handleSuccess
         * @private
         */
...
}, '0.1', {requires:['node', 'plugin', 'io-base']});
