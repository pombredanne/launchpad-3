...
YUI().use("node", "plugin", function(Y) {
    ...
    LP.DynamicDomUpdater.ATTRS = {
         ...
         *
         * If it is not included, the updates will continue infinitely.
         */
        stop_updates_check: {
            valueFn: null
        },
    ...
    Y.extend(LP.DynamicDomUpdater, DomUpdater, {
        ...
        initializer: function(config){
            ...
            // If there wasn't any stop_updates_check function provided,
            // create a function that always returns false, so updates
            // will never stop.
            if (null == this.get("stop_updates_check")){
                this.set("stop_updates_check", function(data){
                    return false;
                })
            }
