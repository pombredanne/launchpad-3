...
YUI.add('lazr.dynamic_dom_updater', function(Y) {

    /**
     * This class provides the ability to plugin functionality to a DOM
     * subtree so that it can update itself when given data in an expected
     * format.
     *
     * For example:
    ...
    Y.extend(DomUpdater, Y.Plugin, {
        ...
        update: function(update_data) {
            ...
            var dom_update_function = this.get("dom_update_function");
            if (dom_update_function !== null){
                dom_update_function(this.get("owner"), update_data);
            }
        }
     ...
     * (with a default interval of 60000ms) with the result of the LP
     * api call.
     * 
    ...
    Y.extend(LP.DynamicDomUpdater, DomUpdater, {
        ...
        _handleSuccess: function(data, something) {
            ...
            // Check whether updates should stop:
            if (this.get("stop_updates_check")(data)){
                Y.log(
                    "Cancelling updates after stop_updates_check " + 
                    "returned true.", "info", "LP DynamicDomUpdater");
                clearInterval(this.intervalID);
            }
        },

        _handleFailure: function(id, request) {
            Y.fail("LP.DynamicDomUpdater failed to get dynamic data.");
        }
    });
