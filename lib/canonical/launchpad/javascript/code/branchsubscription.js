 ...
 */
Y.branchsubscription.update_subscriber_list = function(highlight) {
    ...
    Y.io('+branch-portlet-subscriber-content', {
        on: {
            success: function(id, response) {
                Y.one('#subscriber-list').set(
                    'innerHTML', response.responseText);
                Y.one('#subscriber-list').setStyle('display', 'block');
                if (highlight) {
...
function subscribe_yourself_inline(data) {
    ...
    var selfsubscriptioncontainer = Y.one('#selfsubscriptioncontainer');
    selfsubscriptioncontainer.set('innerHTML',
        '<img src="/@@/spinner">Subscribing...');
    ...
    var notification_level_update = notification_level.options[notification_level.selectedIndex].text;
    var max_diff_lines = document.getElementById(
        'field.max_diff_lines');
    var max_diff_lines_update = max_diff_lines.options[max_diff_lines.selectedIndex].text;
    var review_level = document.getElementById(
        'field.review_level');
    var review_level_update = review_level.options[review_level.selectedIndex].text;

    config = {
        on: {
            success: function(updated_entry) {
                var lp_subscription_entry = updated_entry;

                // XXX: rockstar: bug=336866 The etag returned by lp_save() is
                // totally wrong.
                lp_subscription_entry.removeAttr('http_etag');

                selfsubscriptioncontainer.setStyle('display', 'none');
                Y.code.branchsubscription.update_subscriber_list(true);
            },
            failure: function(id, response) {
...
function set_up_self_subscription_formoverlay(element_id) {
    ...
    var subscribe_yourself = Y.one('#selfsubscription');
    ...
    if (subscribe_yourself !== null) {
        ...
        subscription_form_overlay.loadFormContentAndRender(
         ...
         */
        Y.one('#shadow').setStyle('width', '562px');
        Y.one('div#yui-pretty-overlay-modal.content_box_container').setStyle(
        'width', '500px');
        subscribe_yourself.addClass('js-action');
...
function hide_edit_your_subscription() {
    var element = Y.one('#selfsubscription');
    if (element !== null) {
...
function add_temp_user_name() {
    ...
    var subscribers = Y.one('#subscriber-list');
    ...
    config = {
        on: {
            success: function(person) {
                ...
                username = person.lookup_value('user_name');
            }
        }
    };
...
function animate_subscription_update() {
    var element = Y.one('#subscriber-' + user_name);
