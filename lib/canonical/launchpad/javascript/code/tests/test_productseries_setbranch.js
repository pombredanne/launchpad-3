...
YUI({
    ...
    suite.add(new Y.Test.Case({
        ...
        test_handlers_connected: function() {
           ...
           alert(custom_events);
           custom_events = Y.Event.getListeners(Y.one('#field.branch_type.0'), 'click');
           alert(custom_events);
           var click_event = custom_events[0];
           var subscribers = click_event.subscribers;
           for (var sub in subscribers) {
              Y.Assert.isTrue(subscribers[sub].contains(module.onclick_branch_type),
                              'branch_type_onclick handler setup');
           }
           Y.Assert.areEqual('onclick_branch_type()',
