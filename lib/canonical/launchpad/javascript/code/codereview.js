...
Y.codereview.connect_links = function() {
    ...
    link = Y.get('.menu-link-set_commit_message');
    if (link !== null) {
        link.addClass('js-action');
...
function edit_commit_message(e) {
    e.preventDefault();
    Y.log('edit_commit_message');
    // Make the edit button unseen.
    var link = Y.get('#edit-commit-message');
    link.removeClass('unseen');
    // Remove the unseen class from the commit message.
    link = Y.get('.menu-link-set_commit_message');
    link.addClass('unseen');
    // Simulate a click on the edit link.
    link = Y.get('#edit-commit-message .edit-controls a');
    Y.log(link);
    link.simulate('click');
}
...
Y.extend(NumberToggle, Y.Widget, {
    ...
    bindUI: function(){
        Y.one('#show-no').on('click', update_nos);
    }
});
