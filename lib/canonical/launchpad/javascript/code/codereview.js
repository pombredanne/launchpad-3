...
Y.codereview.connect_links = function() {
    ...
    link = Y.one('.menu-link-add_comment');
    if (link) {
        link.addClass('js-action');
        link.on('click', function(e) {
                e.halt();
                var comment_block = Y.one('#add-comment');
                var comment_top = comment_block.getY() - 5;
                anim.set('to', {scroll: [0, comment_top] });
    ...
    link = Y.one('#proposal-summary a.diff-link');
    if (link) {
        link.addClass('js-action');
        link.on('click', function(e) {
                e.halt();
                var comment_block = Y.one('#review-diff');
                var comment_top = comment_block.getY() - 5;
                anim.set('to', {scroll: [0, comment_top] });
                anim.on('end', function() { });
                anim.run();
