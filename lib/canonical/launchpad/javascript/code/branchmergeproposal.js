...
function update_summary(status_content) {
    Y.log('update_summary');
    SUMMARY_SNIPPET = '++pagelet-summary';
    Y.io(SUMMARY_SNIPPET, {
            on: {
                success: function(id, response) {
                    Y.log('success');
                    var summary = Node.create(response.resposneText);
                    summary.all('tr').each(function(row) {
                            Y.log(row);
                        });
                    Y.log('done.');
                }
