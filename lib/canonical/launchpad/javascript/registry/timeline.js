...
var ANGLE_X_SPACING = 0.1 * Y_SPACING;
// Font size in em's.
var FONT_SIZE = 1;
// Colors.
...
SeriesLine = function(timeline_graph, series, start) {
    ...
    } else if (this.series.status == OBSOLETE_SERIES_STATUS) {
        tooltip = 'Obsolete Series';
    }
    ...
    for (var i in this.series.landmarks) {
        ...
        if (Y.Lang.isString(this.timeline_graph.resize_frame)) {
            cfg = {second_line: landmark.date, id: landmark.name};
        }
...
ProjectLine = function(timeline_graph, timeline) {
    ...
    this.start = new Position(
        MARGIN_LEFT,
        (this.timeline.length-1) * Y_SPACING + MARGIN_TOP);
...
ProjectLine.prototype = {
    ...
    get_height: function() {
        ...
        return this.start.y + bottom_label_height + RELEASE_RADIUS;
    },
...
Y.extend(module.TimelineGraph, Y.Widget, {
    ...
    calculate_landmark_spacing: function() {
        ...
        for (var i in this.project_line.series_lines) {
            ...
            series_max_label_width = Math.max(
                ...
                series_line.series_label.get('offsetWidth'));
    ...
    scroll_to_last_development_focus_landmark: function(label) {
        ...
        if (landmark) {
            ...
            var scroll_x = landmark_label.get('offsetLeft') +
                          landmark_label.get('offsetWidth') -
