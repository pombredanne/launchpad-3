...
var C_ZOOM_BOX = Y.ClassNameManager.getClassName(TIMELINE_GRAPH, 'zoom-box');
...
SeriesLine = function(timeline_graph, series, start) {
    ...
    this.series_label = this.timeline_graph.make_label(
        '<b>' + series.name + '</b>', tooltip, this.series.uri,
        {id: series.name});
    this.labels = {}
    for (var i in this.series.landmarks) {
        ...
        this.labels[landmark.name] = this.timeline_graph.make_label(
            ...
            {second_line: landmark.date, id: landmark.name});
    }
}
...
ProjectLine = function(timeline_graph, timeline) {
    ...
    this.start = new Position(MARGIN_LEFT, this.timeline.length * Y_SPACING);
...
ProjectLine.prototype = {
    ...
    get_height: function() {
        ...
        return this.start.y + bottom_label_height + RELEASE_SIZE;
    },
...
Y.extend(module.TimelineGraph, Y.Widget, {
    ...
    initializer: function(cfg) {
        ...
        if (cfg === undefined || cfg.hide_zoom_buttons !== true) {
            this.hide_zoom_buttons = false;
        } else {
            this.hide_zoom_buttons = true;
        }
        this.graph_scale = 1;
    },
    ...
    recreate_canvas: function() {
        var width = this.graph_scale * Math.max(
            this.project_line.get_width() + MARGIN_LEFT,
            this.get_legend_width());
        var height = this.graph_scale * this.project_line.get_height()
                     + MARGIN_BOTTOM;
        if (this.canvas) {
     ...
     */
    renderUI: function() {
        if (this.hide_zoom_buttons === false) {
    ...
    get_legend_width: function() {
        ...
        return this.compute_legend(false);
    },
     ...
     * @method draw_legend
     */
    draw_legend: function(draw) {
        this.compute_legend(true);
    },

    /**
     * Calculate width or draw legend.
     *
     * @method compute_legend
     */
    compute_legend: function(draw) {
        var line_height =
        ...
        rectangle_start.y -= line_height + 2;

        var rectangle_width =
            (landmark_position.x + RELEASE_SIZE + LEGEND_BOX_PADDING)
            - rectangle_start.x;
        var rectangle_height = line_height + 4;
        if (draw === true) {
        ...
        return rectangle_start.x + rectangle_width + 2;
    },
