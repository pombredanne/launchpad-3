...
ProjectLine.prototype = {
    ...
    draw: function() {
        ...
        // Anchor circle.
        var series_line = this.series_lines[0];
        context.fillStyle = anchor_fill_color;
        context.beginPath();
        context.arc(
            series_line.start.x, series_line.start.y, release_size, 0,
            (Math.PI*2), true);
        context.fill();
        context.stroke();
    }
...
Y.extend(module.TimelineGraph, Y.Widget, {
    ...
    draw_legend: function() {
        ...
        var line_height = this.milestone_legend.get('offsetHeight')
                          / this.graph_scale;
        var legend_position = new Position(
            100 / this.graph_scale,
            (this.canvas.get('offsetHeight') - line_height * 1.5)
            / this.graph_scale);
        ...
        // Release.
        legend_position.x = 20 + release_size + landmark_position.x;
        this.place_label(
        ...
        var rectangle_stop = new Position(
            landmark_position.x + 15,
            rectangle_start.y + line_height + 3);
    ...
    make_label: function(text, tooltip, uri, second_line) {
        ...
        if (uri) {
            label.appendChild(
                Y.Node.create('<a href="' + uri + '">' + text + '</a>'));
        } else {
            label.appendChild(
