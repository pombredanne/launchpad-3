...
SeriesLine = function(timeline_graph, series, start) {
    ...
    if (!Y.Lang.isString(this.timeline_graph.resize_frame)) {
        var reversed_landmarks = this.series.landmarks.slice().reverse();
        for (var i in this.series.landmarks) {
            landmark = this.series.landmarks[i];
...
SeriesLine.prototype = {
    ...
    get_y_spacing: function() {
        ...
        return max_y + 2 * RELEASE_RADIUS
            + this.series_label.get('offsetHeight');
    },
...
ProjectLine.prototype = {
    ...
    initSeries: function() {
        ...
        for (var i in reversed_timeline) {
            var series = reversed_timeline[i];
        ...
        if (current.x < MARGIN_LEFT) {
            ...
            for (var i in this.series_lines) {
                this.series_lines[i].start.x += shift_x;
            }
        }
    },
