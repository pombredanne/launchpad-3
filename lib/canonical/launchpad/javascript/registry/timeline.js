...
Y.extend(module.TimelineGraph, Y.Widget, {
    ...
    create_canvas: function() {
        ...
        var width = contentBox.get('offsetWidth');
        var height =
            (this.project_line.series_lines.length + 1) * y_spacing
            * this.graph_scale + margin;
    ...
    renderUI: function() {
        ...
        this.zoom_out_button = Y.Node.create(
                ...
                '          position: static; height: 14px;' +
                '          top: 25px; left: 20px"/>');
        var zoom_box = Y.Node.create('<div style="z-index: 500; border: 2px solid red; position: fixed"/>');
