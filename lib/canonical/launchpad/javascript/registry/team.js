    ...
    config = {
        on: {
            success: function(member_added) {
                if (!member_added) {
                    disable_spinner();
                    Y.lp.display_error(addmember_link,
                        selected_person.title + ' is already a member.');
                    return;
                }
                var members_section, members_ul, count_elem;
                var is_team = selected_person.css.match("team")
                if (is_team) {
                ...
                lp_client.get(selected_person.api_uri, config);
            },
            failure: error_handler.getFailureHandler()
        },
        parameters: {
