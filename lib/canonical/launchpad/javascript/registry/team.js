...
function add_member(result) {
    ...
    if (false/* TODO: result is direct *and* active member of team */) {
        // Display message saying we won't do anything.
        return;
    }

    /*
    error_handler.showError = function(error_msg) {
        Y.lp.display_error(Y.one('.menu-link-addmember'), error_msg);
    */
    config = {
        on: {
            success: function(member_added) {
                ...
                if ( result.css.match("team") ) {
                    disable_spinner();
                    // Display message saying membership is still pending.
