...
YUI({
    ...
    Y.on('domready', function() {
        python_log_node = Y.Node.create(
            '<pre id="python-log"></pre>');
        Y.get('body').appendChild(python_log_node);
        var handle_test_result = function(data) {
            status_node = Y.Node.create(
                '||' +
                data.type + '::' +
                data.testName  + '::' +
                data.testCase.name);
            Y.get('#python-log').appendChild(status_node);
            };
        Y.Test.Runner.subscribe(
            Y.Test.Runner.TEST_FAIL_EVENT, handle_test_result);
        Y.Test.Runner.subscribe(
            Y.Test.Runner.TEST_IGNORE_EVENT, handle_test_result);
        Y.Test.Runner.subscribe(
            Y.Test.Runner.TEST_PASS_EVENT, handle_test_result);
