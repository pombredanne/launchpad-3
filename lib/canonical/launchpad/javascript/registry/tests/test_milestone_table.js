...
YUI({
    ...
    }).use('yuitest', 'console', 'lp.milestonetable', function(Y) {

    var milestonetable = Y.lp.milestonetable;
    ...
    suite.add(new Y.Test.Case({
        ...
        test_config_property_undefined: function() {
            // Verify an error is thrown whenthe config is incomplete.
            var config = {
                milestone_row_uri_template: '/uri'
                };
            milestonetable.setup(config);
        ...
        test_missing_tbody_is_an_error: function() {
            ...
            var config = {
                milestone_row_uri_template: '/uri',
                milestone_rows_id:  'does-not-exis'
                };
            milestonetable.setup(config);
            }
        }));
    ...
    suite.add(new Y.Test.Case({
        ...
        setUp: function() {
            ...
            this.data =  milestonetable._setup_milestone_event_data(
                {name: '0.1'}, Y.get('#milestone-rows'));
            // Monkey patch Y.lazr.anim objects.
        ...
        test_sucess_milestone_prepended: function() {
    ...
    suite.add(new Y.Test.Case({
        ...
        setUp: function() {
            milestonetable.setup({
                milestone_row_uri_template: 'milestone_table.data',
                milestone_rows_id: '#milestone-rows'
                });
        ...
        test_get_milestone_row: function() {
            ...
            Y.Assert.isTrue(this.record.called);
            }
        }));
