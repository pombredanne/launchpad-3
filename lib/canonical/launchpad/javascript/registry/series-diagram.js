...
function make_landmark(canvas_context, x_pos, y_pos, type, label) {
    if(type == 'milestone') {
        ...
        canvas_context.fillRect (
            ...
            milestone_size);
        make_label(x_pos, y_pos + label_spacing, label);
...
function draw_graph(timeline, canvas) {
    ...
    var x_start_vertical  = x_spacing;
    var x_end_vertical = x_start_vertical
        + (timeline.length * angle_x_spacing);

    var y_end_vertical = 0;
    var y_start_vertical = timeline.length * y_spacing;

    canvas.width = x_end_vertical + 9 * x_spacing;
    canvas.height = y_start_vertical + 9 * y_spacing;
    // Evil global.
    ...
    var nodes = [
        {x: x_start_vertical,  y: y_start_vertical},
        {x: x_end_vertical,  y: y_end_vertical}
        ];
    ...
    for(var i in timeline) {
        ...
        i = parseInt(i);
        var series = timeline[i];

        // Horizontal lines
        var x_start_series = x_start_vertical + (i * angle_x_spacing);
        var x_end_series =
            x_start_series + (series.landmarks.length * x_spacing);

        var y_series = y_start_vertical - (i * y_spacing);
