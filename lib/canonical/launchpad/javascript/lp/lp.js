    ...
    Y.lp.activate_collapsibles = function() {
        ...
        if (collapsibles !== null) {
            Y.each(collapsibles, function(collapsible) {
                ...
                // Move the contents of the legend into the span.
                var span = Y.Node.create('<span />');
                var legend_children = legend.get('children');

                // XXX 2009-07-06 gmb
                // Account for oddness from Node.get('children'); (see
                // YUI ticket 2528028 for details).
                var len = legend_children.size ?
                ...
                if (len > 0) {
                    ...
                    Y.each(legend_children, function(child_node) {
                        if (child_node.get('tagName') == 'A') {
                            ...
                            // Otherwise, add the node to the span as it is.
