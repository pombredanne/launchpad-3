...
YUI.add('lp', function(Y) {
    ...
    Y.lp.activate_collapsibles = function() {
        ...
        Y.each(collapsibles, function(collapsible) {
            ...
            if (legend === null) {
                // If there's no legend there's not much we can do, so
                // just exit this iteration.
                return;
            }
            ...
            var inner = legend.get('innerHTML');
            span.set('innerHTML', inner);
            ...
            // Put a wrapper around the fieldset contents for ease of hiding.
            var wrapper_div = Y.Node.create('<div class="collapseWrapper" />');

            // Loop over the children of the collapsible and move them
            // into the wrapper div, leaving the <legend> outside.
            Y.each(collapsible.get('children'), function(child_node) {
                if (child_node != legend) {
                    collapsible.removeChild(child_node);
                    wrapper_div.appendChild(child_node);
                }
            });

            collapsible.appendChild(wrapper_div);

            // If the collapsible is to be collapsed on pageload, do so.
            if (collapsible.hasClass('collapsed')) {
                // Strip out the 'collapsed' class as it's no longer
                // needed.
                collapsible.removeClass('collapsed');

                // We use the slide_in effect to hide the collapsible
                // because it sets up all the properties and classes for
                // the element properly and saves us from embarrasement
                // later on.
            ...
            // Finally, add toggle_collapsible() as an onclick handler
            // to the anchor.
