YUI.add('lp.mapping', function(Y) {
    ...
    mapping.has_gmaps = (typeof(GBrowserIsCompatible) != 'undefined');

    // jslint does not like functions that look like classes.
    var gBrowserIsCompatible = mapping.has_gmaps ?
        GBrowserIsCompatible : function() {return false;};
    var gDownloadUrl =  mapping.has_gmaps ?
        GDownloadUrl : function() {return null;};
    ...
    mapping.toggleShowSmallMaps = function (checkbox) {
        ...
        lp.launchpad_views.set('small_maps', is_shown);
        ...
        if (is_shown && !mapping.has_gmaps) {
            // The server must add the mapping dependancies.
     ...
     * @param {String} div_id the id of the map div that this control follows.
     */
    mapping.setupShowSmallMapsControl = function (div_id) {
        var show_small_maps = lp.launchpad_views.get('small_maps');
        ...
        var label_text = Y.Node.create('Show small maps');
        var label = Y.Node.create('<label></label>');
        label.appendChild(checkbox);
        label.appendChild(label_text);
        var map_div = Y.get('#' + div_id);
        var ancestor =  map_div.ancestor();
        // Note the order is backward because we want to add after the map.
        ancestor.insertBefore(label, map_div);
        ancestor.insertBefore(map_div, label);
        if (!show_small_maps) {
    ...
    mapping.renderPersonMapSmall = function(center_lat, center_lng) {
        mapping.setupShowSmallMapsControl('person_map_div');
    ...
    mapping.renderTeamMapSmall = function(center_lat, center_lng) {
        mapping.setupShowSmallMapsControl('team_map_div');
        if (!gBrowserIsCompatible()) {
            return;
        }
