...
Y.extend(Comment, Y.Widget, {
    ...
    addComment: function(e){
        ...
        var config = {
            ...
            parameters: this.getCommentParams()
        };
       this.lp_client.named_post(
            this.getPostUrl(), this.post_command, config);
    },
    getCommentParams: function() {
        return {content: this.comment_input.get('value')}
    },
    getPostUrl: function(){
...
Y.extend(CodeReviewComment, Comment, {
    ...
    addComment: function(e){
       ...
       this.lp_client.named_post(
            this.getPostUrl(), 'createComment', config);
    },
    getPostUrl: function(){
        return LP.client.cache.context.self_link
    },
    addCommentHTML: function(comment_entry) {
        var fieldset = Y.get('#launchpad-form-actions');
        var keys = ['title', 'message_body', 'vote', 'vote_tag'];
        var body = Y.Node.create('<div class="boardCommentBody"/>');
        for (var i in keys) {
            body.appendChild(document.createTextNode(keys[i]));
            body.appendChild(document.createTextNode(': '));
            body.appendChild(
                document.createTextNode(comment_entry.get(keys[i])));
            body.appendChild(Y.Node.create('<br/>'));
        }
        var comment = Y.Node.create(
