...
/**
  ...
  *                        config.step_title overrides the subtitle.
  *                        description strings.
  * @param {Function} save Function (optional) which takes a single string
  *                        argument.
  */
Y.lp.picker.create = function (vocabulary, config, save) {
    if (Y.UA.ie) {
        return;
    }
    if (config !== undefined) {
    ...
    picker.subscribe('save', function (e) {
        ...
        if (save !== undefined) {
            save(e.details[Y.lazr.Picker.SAVE_RESULT]);
        }
    });
