...
Y.lp.picker.addPickerPatcher = function (
    ...
    var remove = function () {
        ...
        var success_handler = function (e) {
            activator.renderSuccess(Y.Node.create('*removed*'));
            show_hide_buttons();
        };
    ...
    var form_extras = Y.Node.create('<span style="white-space: nowrap"/>');
    var remove_button = Y.Node.create(
        '<button style="background: white; margin: 3px;' +
        'background: url(/@@/remove) 0.2em 0 no-repeat">' +
        '<span style="margin-left: 1.3em">Remove Assignee</span></button>');
    remove_button.on('click', remove);
    form_extras.appendChild(remove_button);
    var assign_me_button = Y.Node.create(
        '<button style="background: white; margin: 3px;' +
        'background: url(/@@/person) 0.2em 0 no-repeat">' +
        '<span style="margin-left: 1.3em">Assign Me</span></button>');
    assign_me_button.on('click', assign_me);
    form_extras.appendChild(assign_me_button);
    picker.set('form_extras', form_extras);
