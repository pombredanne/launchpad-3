...
class TestBzrSync(BzrSyncTestCase):
    ...
    def test_get_revisions_branched(self):
        # Create a branch, branch from it, commit to both branches, merge the
        # second branch into the first, then commit. This creates a branch with
        # a mainline revision history which is distinct from its ancestry. The
        # commits to the second branch are in the first branch's ancestry, but
        # not its history.
        #
