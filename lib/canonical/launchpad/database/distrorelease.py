        ...
        cur.execute('''
            ...
            FROM
                ...
                LEFT OUTER JOIN POTemplate AS pt_from_other ON
                    ...
                    pt_from_other.distrorelease = %s
            WHERE
                pt1.distrorelease = %s AND ps2.id IS NULL AND
                (pt_from_other.id IS NULL OR %s)''' % sqlvalues(
            self, self.parentrelease, self.parentrelease, full_copy))
            ...
            cur.execute('''
                UPDATE POSelection SET activesubmission = ps2.id
                    ...
                    WHERE
                        ...
                        POSelection.activesubmission <> ps2.id AND
        ...
        cur.execute('''
            ...
            FROM
                ...
                LEFT OUTER JOIN POTemplate AS pt_from_other ON
                    ...
                    pt_from_other.distrorelease = %s
            WHERE
                pt1.distrorelease = %s AND psel2.id IS NULL AND
                (pt_from_other.id IS NULL OR %s)''' % (
            (poselection_publishedsubmission_value, ) +
            sqlvalues(self, self.parentrelease, self.parentrelease, full_copy)))
