 ...
 *             reverse) inside table cells. Removed tabs.
 *
 */
...
function ts_makeSortable(table) {
    ...
    for (var i=0; i<firstRow.cells.length; i++) {
	    ...
	    continue;
        lnk = ts_firstChildByName(cell, 'a');
        ts_resortTable(lnk);
        break;
    }
}
...
function ts_resortTable(lnk) {
    ...
    for (var ci=0; ci<lnk.childNodes.length; ci++) {
        if (lnk.childNodes[ci].tagName && 
            lnk.childNodes[ci].tagName.toLowerCase() == 'img') 
                img = lnk.childNodes[ci];
    }
...
function ts_stableSort(sortfn) {
    ...
    function stableSort(a, b) {
        cmp = sortfn(a, b);
        if (cmp != 0) return cmp;
        else return a.oldPosition - b.oldPosition;
    }
    return stableSort;
}
