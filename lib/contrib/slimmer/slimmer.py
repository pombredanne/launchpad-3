...
def slimmer(code, syntax=XHTML, hardcore=False):
    ...
    if syntax == XHTML:
        ...
        if hardcore:
            return _js_slimmer(code, slim_functions=True)
        else:
...
def js_slimmer(js, slim_functions=False):
    return _js_slimmer(js, slim_functions=slim_functions)
