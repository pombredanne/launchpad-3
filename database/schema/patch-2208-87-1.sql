...
CREATE OR REPLACE FUNCTION update_transitively_private(
    root int, _root_transitively_private boolean = NULL) RETURNS VOID
...
BEGIN
    ...
    PERFORM update_transitively_private(
        id, GREATEST(private, root_transitively_private))
    FROM Branch WHERE stacked_on = root;
END;
$$;

COMMENT ON FUNCTION update_transitively_private(int, boolean) IS
