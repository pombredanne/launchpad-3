...
CREATE OR REPLACE FUNCTION shipment_maintain_shipped_flag()
RETURNS TRIGGER VOLATILE SECURITY DEFINER AS $$
    if not SD.has_key("update"):
        SD["update_plan"] = plpy.prepare(
            "UPDATE ShippingRequest SET shipped=$1 WHERE id=$2",
            ["bool", "int4"]
            )
            
    new = TD["new"]
    old = TD["old"]
    update_plan = SD["update_plan"]

    # INSERT
    if old is None:
        plpy.execute(update_plan, [True, new["request"]])

    # DELETE
    elif new is None:
        plpy.execute(update_plan, [False, old["request"]])

    # UPDATE
    elif old["request"] != new["request"]:
        plpy.execute(update_plan, [False, old["request"]])
        plpy.execute(update_plan, [True, new["request"]])
$$ LANGUAGE plpythonu;

COMMENT ON FUNCTION shipment_maintain_shipped_flag() IS 'A trigger function on the Shipment table to maintain the ShippingRequest.shipped flag. This flag is required to enforce the "only one unshipped request at a time" constraint until we collapse the ShippingRequest and Shipment tables into one.';
