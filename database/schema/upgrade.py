...
def apply_comments(con):
    if not options.slonik:
        ...
        while comments:
            ...
            chunk_file.close()
            counter += 1
            slonik_execute_script(chunk_path)
...
def slonik_preamble():
    ...
    print >> options.slonik, "# slonik script for %s" % (
            connect_string('slony'),)
    print >> options.slonik, "# generated by %s %s\n" % (
            os.path.basename(sys.argv[0]), time.ctime())
    print >> options.slonik, dedent("""\
            cluster name = %s;
            node 1 admin conninfo='%s';
            """ % (SLONY_CLUSTER, connect_string('slony')))


def slonik_execute_script(script_filename):
    replication_set = 1
    master_node = 1
    print >> options.slonik, dedent("""\
