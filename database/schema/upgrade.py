...
def main():
    ...
    for (major, minor, patch), patch_file in patches:
        if options.slonik:
            slonik_execute_script(outf, patch_file)
        else:
...
def slonik_preamble(outf):
    print >> outf, preamble()
    print >> outf, dedent("""\
        # slonik script for upgrading %s
        # generated by %s %s
        try {
        """ % (
            connect_string('slony'),
            os.path.basename(sys.argv[0]), time.ctime(),))

def slonik_afterword(outf):
    print >> outf, dedent("""\
        }
        on error { echo "Failed. Rolling back."; exit 1; }
