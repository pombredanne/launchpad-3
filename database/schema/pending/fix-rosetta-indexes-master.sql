...
CREATE TEMPORARY TABLE NotCurrentTM AS
SELECT DISTINCT bad.id
FROM TranslationMessage AS bad, TranslationMessage AS good
WHERE
    bad.id <> good.id AND
    good.potemplate IS NOT NULL AND
    bad.potemplate IS NOT NULL AND
    ...
    (COALESCE(good.date_reviewed, good.date_created) >
        COALESCE(bad.date_reviewed, bad.date_created) OR
     (COALESCE(good.date_reviewed, good.date_created) =
         COALESCE(bad.date_reviewed, bad.date_created) AND
         good.id > bad.id));

CREATE UNIQUE INDEX notcurrenttm__id__key ON NotCurrentTM(id);

UPDATE TranslationMessage SET is_current = FALSE
FROM NotCurrentTM
WHERE TranslationMessage.id = NotCurrentTM.id;

DROP TABLE NotCurrentTM;


CREATE TEMPORARY TABLE NotImportedTM AS 
SELECT DISTINCT bad.id
FROM TranslationMessage AS bad, TranslationMessage AS good
WHERE
    bad.id <> good.id AND
...
CREATE UNIQUE INDEX notimportedtm__id__key ON NotImportedTM(id);

UPDATE TranslationMessage
SET is_imported = FALSE
FROM NotImportedTM
WHERE
    TranslationMessage.id = NotImportedTM.id;
